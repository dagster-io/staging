...
def test_execute_on_celery_k8s_run_queued(  # pylint: disable=redefined-outer-name
    ...
    run_config = merge_dicts(
        ...
        get_celery_engine_config(
            ...
            job_namespace=helm_namespace_for_run_coordinator_queue,
        ),
    )
    ...
    dagster_instance_for_run_coordinator_queue.launch_run(
        run.run_id,
        ReOriginatedExternalPipelineForTest(get_test_project_external_pipeline(pipeline_name)),
    )

    result = wait_for_job_and_get_raw_logs(
        job_name='dagster-run-%s' % run.run_id, namespace=helm_namespace_for_run_coordinator_queue
    )
