...
def test_execute_on_celery_k8s_run_queued(  # pylint: disable=redefined-outer-name
    dagster_docker_image, dagster_instance, helm_namespace_run_launcher_queued
):
    ...
    run_config = merge_dicts(
        ...
        get_celery_engine_config(
            ...
            job_namespace=helm_namespace_run_launcher_queued,
        ),
    )
    ...
    result = wait_for_job_and_get_raw_logs(
        job_name='dagster-run-%s' % run.run_id, namespace=helm_namespace_run_launcher_queued
    )
