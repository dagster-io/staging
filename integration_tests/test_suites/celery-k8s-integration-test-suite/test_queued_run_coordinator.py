...
from marks import mark_run_coordiantor
...
def test_execute_on_celery_k8s(  # pylint: disable=redefined-outer-name
    ...
    dagster_instance_for_run_coordinator.launch_run(
        run.run_id,
    ...
    assert "PIPELINE_ENQUEUED" in result, "no match, result: {}".format(result)
    assert "PIPELINE_DEQUEUED" in result, "no match, result: {}".format(result)
