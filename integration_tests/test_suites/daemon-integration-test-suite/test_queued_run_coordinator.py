...
WAIT_FOR_RUN_CHECK_INTERVAL_SECONDS = 2
WAIT_FOR_RUN_TIMEOUT_SECONDS = 20
    ...
    with setup_instance(dagster_home_path):
        with start_daemon():
            ...
            with create_run(instance, foo_pipeline_handle) as run:
                ...
                start = time.time()
                while time.time() - start < WAIT_FOR_RUN_TIMEOUT_SECONDS:
                    time.sleep(WAIT_FOR_RUN_CHECK_INTERVAL_SECONDS)
                    run_status = instance.get_run_by_id(run.run_id).status
                    if run_status == PipelineRunStatus.SUCCESS:
                        break
                    assert run_status != PipelineRunStatus.FAILURE
                    print("Run status {}, waiting".format(run_status))  # pylint: disable=print-call
