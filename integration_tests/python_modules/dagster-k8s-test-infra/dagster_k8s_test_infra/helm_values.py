...
def create_helm_config(
    namespace, docker_image, enable_user_deployments=False, enable_run_launcher_queued=False
):
    check.str_param(namespace, 'namespace')
    check.str_param(docker_image, 'docker_image')
    check.bool_param(enable_user_deployments, 'user_deployments')
    check.bool_param(enable_run_launcher_queued, 'run_launcher_queued')
    ...
    helm_config['celery'] = get_celery_config(
        repository, tag, pull_policy, enable_run_launcher_queued
    )
...
def get_celery_config(repository, tag, pull_policy, enable_run_launcher_queued):
    config = {
        'image': {'repository': repository, 'tag': tag, 'pullPolicy': pull_policy},
