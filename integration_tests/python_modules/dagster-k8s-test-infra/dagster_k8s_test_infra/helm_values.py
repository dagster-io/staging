...
from .helm import TEST_CONFIGMAP_NAME, TEST_SECRET_NAME
...
def create_helm_config(namespace, docker_image, user_deployments=False, run_launcher_queued=False):
    ...
    helm_config['celery'] = celery_config(repository, tag, pull_policy)
...
def celery_config(repository, tag, pull_policy):
    return {
        'image': {'repository': repository, 'tag': tag, 'pullPolicy': pull_policy},
