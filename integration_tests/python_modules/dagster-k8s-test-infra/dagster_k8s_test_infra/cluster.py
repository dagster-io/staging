...
def dagster_instance_with_k8s_scheduler(
    helm_for_k8s_run_launcher, run_launcher, k8s_scheduler, schedule_tempdir
):
    with local_port_forward_postgres(namespace=helm_for_k8s_run_launcher) as local_forward_port:
...
def dagster_instance_for_user_deployments(
    helm_for_k8s_run_launcher_for_user_deployments, run_launcher
):  # pylint: disable=redefined-outer-name
    tempdir = DagsterInstance.temp_storage()

    with local_port_forward_postgres(
        namespace=helm_for_k8s_run_launcher_for_user_deployments
...
def dagster_instance_for_k8s_run_launcher(
    helm_for_k8s_run_launcher_for_k8s_run_launcher, run_launcher
):
    tempdir = DagsterInstance.temp_storage()

    with local_port_forward_postgres(
        namespace=helm_for_k8s_run_launcher_for_k8s_run_launcher
    ) as local_forward_port:
        postgres_url = 'postgresql://test:test@localhost:{local_forward_port}/test'.format(
            local_forward_port=local_forward_port
        )
...
def dagster_instance(
    helm_for_k8s_run_launcher, run_launcher
):  # pylint: disable=redefined-outer-name
    tempdir = DagsterInstance.temp_storage()
