...
@pytest.fixture(scope='session')
def dagster_instance_for_run_queue(
    helm_namespace_for_run_queue, run_launcher
):  # pylint: disable=redefined-outer-name
    tempdir = DagsterInstance.temp_storage()

    with local_port_forward_postgres(namespace=helm_namespace_for_run_queue) as local_forward_port:
        postgres_url = 'postgresql://test:test@localhost:{local_forward_port}/test'.format(
            local_forward_port=local_forward_port
        )
        print('Local Postgres forwarding URL: ', postgres_url)
