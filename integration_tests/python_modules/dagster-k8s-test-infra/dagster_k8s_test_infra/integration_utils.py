...
class ReOriginatedGrpcExternalPipelineForTest(ExternalPipeline):
    def __init__(
        ...
        grpc_repository_location_handle = GrpcServerRepositoryLocationHandle(
            port=3030,
            socket=None,
            host='user-code-deployment-1',
            location_name='user-code-deployment-1',
            client=DagsterGrpcClient(port=3030, socket=None, host='user-code-deployment-1'),
            repository_names=set('demo_execution_repo'),
        )
        repository_handle = RepositoryHandle(
            repository_name='demo_execution_repo',
            repository_location_handle=grpc_repository_location_handle,
        )
        ...
        #     port=3030, socket=None, host='user-code-deployment-1', location_name='user-code-deployment-1',
        # )
        super(ReOriginatedGrpcExternalPipelineForTest, self).__init__(
            external_pipeline.external_pipeline_data, repository_handle,
        )
