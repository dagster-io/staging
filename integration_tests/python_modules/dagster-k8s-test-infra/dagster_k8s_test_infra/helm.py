    ...
    try:
        ...
        if helm_config.get("runLauncher").get("type") == "CeleryK8sRunLauncher":
            ...
            for queue in worker_queues:
                ...
                if labels:
                    all_deployments = kubernetes.client.AppsV1Api().list_namespaced_deployment(namespace=namespace)
                    target_deployments = list(filter(lambda item: "{queue.get('name')}" in item.metadata.name, all_deployments.items))
