    ...
    try:
        ...
        if helm_config.get("runLauncher").get("type") == "CeleryK8sRunLauncher":
            ...
            for queue in worker_queues:
                ...
                if labels:
                    print("deployments.items", deployments.items) # pylint: disable=print-call
                    target_deployments = list(filter(lambda item: "{queue.get('name')}" in item.metadata.name, deployments.items))
