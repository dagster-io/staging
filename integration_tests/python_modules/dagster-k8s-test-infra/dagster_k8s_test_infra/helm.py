...
def helm_namespace_for_user_deployments(
    ...
    yield _helm_namespace_helper(helm_chart_provider(enable_user_deployments=True), request)
...
@pytest.fixture(scope='session')
def helm_namespace_for_run_coordinator_queue(
    cluster_provider, request
):  # pylint: disable=unused-argument, redefined-outer-name
    yield _helm_namespace_helper(helm_chart_provider(enable_run_coordinator_queue=True), request)
