...
@pytest.fixture(scope='session')
def helm_namespace_run_launcher_queued(
    cluster_provider, request
):  # pylint: disable=unused-argument, redefined-outer-name
    yield _helm_namespace_helper(helm_chart_provider(run_launcher_queued=True), request)
...
#         yield _helm_namespace_helper(helm_chart_provider(**kwargs), request)
