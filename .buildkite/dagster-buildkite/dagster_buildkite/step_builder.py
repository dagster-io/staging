...
class StepBuilder:
    ...
    def on_python_image(self, image, env=None, extra_docker_settings=None):
        docker_settings = {
            "image": f"{AWS_ACCOUNT_ID}.dkr.ecr.us-west-2.amazonaws.com/{image}",
            "environment": ["BUILDKITE"] + (env or []),
            **(extra_docker_settings or {}),
            **DOCKER_SETTINGS,
        }
    ...
    def on_integration_image(self, python_version: str, env=None):
        ...
        return self.on_python_image(
            f"buildkite-integration:py{python_version}-{INTEGRATION_IMAGE_VERSION}",
            env,
            extra_docker_settings,
        )
