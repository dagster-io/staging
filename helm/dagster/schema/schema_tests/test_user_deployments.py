...
@pytest.fixture(name="user_deployment_one")
def simple_user_deployment_one() -> UserDeployment:
    return UserDeployment(
        name="deployment-one",
        image=kubernetes.Image(repository="repo/image-one", tag="tag1", pullPolicy="Always"),
        dagsterApiGrpcArgs=["-m", "deployment_one"],
        port=3030,
    )


@pytest.fixture(name="user_deployment_two")
def simple_user_deployment_two() -> UserDeployment:
    return UserDeployment(
        name="deployment-two",
        image=kubernetes.Image(repository="repo/image-two", tag="tag2", pullPolicy="Always"),
        dagsterApiGrpcArgs=["-m", "deployment_two"],
        port=3030,
    )


@pytest.fixture(name="user_deployment_three")
def complex_user_deployment_one() -> UserDeployment:
    return UserDeployment(
        name="complex-deployment-one",
        image=kubernetes.Image(
            repository="repo/complex-image-one", tag="tag1", pullPolicy="Always"
        ),
        dagsterApiGrpcArgs=["-m", "complex_deployment_one"],
        port=3030,
...
@pytest.fixture(name="user_deployment_four")
def complex_user_deployment_two() -> UserDeployment:
    return UserDeployment(
        name="complex-deployment-two",
        image=kubernetes.Image(
            repository="repo/complex-image-two", tag="tag1", pullPolicy="Always"
        ),
        dagsterApiGrpcArgs=["-m", "complex_deployment_two"],
        port=3030,
        annotations=kubernetes.Annotations.parse_obj(
            {"annotation_1": "an_annotation_1", "annotation_2": "an_annotation_2"}
        ),
...
def helm_values_single_user_deployment(
    user_deployment_one: UserDeployment,
) -> HelmValues:
    return HelmValues.construct(
        userDeployments=UserDeployments(
            enabled=True,
            deployments=[user_deployment_one],
        )
    )


@pytest.fixture(name="single_complex_user_deployment_values")
def helm_values_single_complex_user_deployment(
    user_deployment_three: UserDeployment,
) -> HelmValues:
    return HelmValues.construct(
        userDeployments=UserDeployments(
            enabled=True,
            deployments=[user_deployment_three],
        )
    )


@pytest.fixture(name="multi_user_deployment_values")
def helm_values_multi_user_deployment(
    user_deployment_one: UserDeployment, user_deployment_two: UserDeployment
) -> HelmValues:
    return HelmValues.construct(
        userDeployments=UserDeployments(
            enabled=True,
            deployments=[user_deployment_one, user_deployment_two],
        )
    )


@pytest.fixture(name="multi_complex_user_deployment_values")
def helm_values_multi_complex_user_deployment(
    user_deployment_two: UserDeployment,
    user_deployment_three: UserDeployment,
    user_deployment_four: UserDeployment,
) -> HelmValues:
    return HelmValues.construct(
        userDeployments=UserDeployments(
            enabled=True,
            deployments=[user_deployment_three, user_deployment_four, user_deployment_two],
        )
    )
