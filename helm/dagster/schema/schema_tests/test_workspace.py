...
def test_workspace_renders_from_helm_dagit(template: HelmTemplate):
    servers = [
        Server(name="another-deployment-one", port=4000),
        Server(name="another-deployment-two", port=4001),
        Server(name="another-deployment-three", port=4002),
    ]
    helm_values = DagsterHelmValues.construct(
    ...
    for grpc_server, server in zip(grpc_servers, servers):
        assert grpc_server["grpc_server"]["host"] == server.name
