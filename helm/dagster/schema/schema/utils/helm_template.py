...
class HelmTemplate:
    ...
    def render(  # pylint: disable=W0102
        self,
        values: DagsterHelmValues,
        chart_version: Optional[str] = None,
        helm_dir_path: str = "helm/dagster",
        subchart_paths: List[str] = ["charts/dagster-user-deployments"],
    ) -> List[Any]:
        with NamedTemporaryFile() as tmp_file:
            helm_dir_path = os.path.join(git_repo_root(), helm_dir_path)
            ...
            with self._with_chart_yaml(helm_dir_path, chart_version, subchart_paths):
    ...
    @contextmanager
    def _with_chart_yaml(
        self, helm_dir_path: str, chart_version: Optional[str], subchart_paths: List[str]
    ):
        if not chart_version:
            ...
            subchart_chart_paths = [
                ...
                for subchart_path in subchart_paths
            ]
