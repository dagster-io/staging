...
@pipeline
def hammer_pipeline():
    out_1, out_2, out_3, out_4 = chase_taker()
    reducer(
...
@schedule(
    cron_schedule="* * * * *", pipeline_name="hammer_pipeline", execution_timezone="US/Central"
)
def every_minute_schedule(_context):
    return {}


@sensor(pipeline_name="hammer_pipeline", minimum_interval_seconds=30)
def always_on_sensor(_):
    return RunRequest(run_key=None, run_config={}, tags={})


@repository
def dagster_cloud_repository():
    return [hammer_pipeline, every_minute_schedule, always_on_sensor]
