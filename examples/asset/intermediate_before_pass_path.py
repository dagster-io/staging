...
def data_materializer(context, path, value):
    dt_string = datetime.now().strftime("%d-%m-%Y-%H-%M-%S")
    csv_path = os.path.join(
        os.path.dirname(__file__), "{path}_{dt}.csv".format(path=path, dt=dt_string)
    )
...
def raw_data(context, source_data_path):
    ...
    csv_path = data_materializer(context, "cache/source_data", source_data)
...
def preprocess(context, source_data_path):
    ...
    csv_path = data_materializer(context, "cache/processed", processed)
    return csv_path
...
def daily_highs(context, data_path):
    ...
    csv_path = data_materializer(context, "cache/highs", sorted(highs, key=lambda x: x["date"]))
