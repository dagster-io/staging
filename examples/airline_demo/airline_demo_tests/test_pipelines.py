...
def test_ingest_pipeline_fast(postgres, pg_hostname):
    with tempfile.TemporaryDirectory() as temp_dir:
        with instance_for_test() as instance:
            ...
            ingest_config_dict["resources"]["object_manager"] = {"config": {"base_dir": temp_dir}}
            ingest_config_dict["resources"]["pyspark_object_manager"] = {
...
def test_airline_pipeline_1_warehouse(postgres, pg_hostname):
    with tempfile.TemporaryDirectory() as temp_dir:
        with instance_for_test() as instance:
            ...
            warehouse_config_object["resources"]["object_manager"] = {
                "config": {"base_dir": temp_dir}
            }
            warehouse_config_object["resources"]["pyspark_object_manager"] = {
                "config": {"base_dir": temp_dir}
            }
