...
def overridden_dagster_yaml(reference_deployment):
    # Override dagster.yaml to use DefaultRunLauncher
...
@pytest.fixture
def network(monkeypatch, docker_compose, testrun_uid):
    # When running locally, we don't need to jump through
    # any special networking hoops; we can talk to dagit on localhost
    ...
    # Only this time, do it from within our test fixtures
    ...
    # Connect the two
    subprocess.call(["docker", "network", "connect", network_name, container_name])

    # Return our container's IP Address
    info = json.loads(subprocess.check_output(["docker", "inspect", container_name]))[0]
    ip_address = info["NetworkSettings"]["Networks"][network_name]["IPAddress"]
    return ip_address


def test_deploy(network, session):
    assert session.get(f"http://{network}:3000/dagit_info").ok
