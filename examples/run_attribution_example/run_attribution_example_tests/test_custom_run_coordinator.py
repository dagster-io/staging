...
from dagster_tests.core_tests.run_coordinator_tests.test_queued_run_coordinator import (
    QueuedRunCoordinatorTests,
)
from mock import patch
from run_attribution_example.custom_run_coordinator import CustomRunCoordinator


class CustomRunCoordinatorTests(QueuedRunCoordinatorTests):
    @pytest.fixture(scope="function")
    def coordinator(self, instance):
        coordinator = CustomRunCoordinator()
        coordinator.register_instance(instance)
        # silence warnings when not applicable since execution is in a test context
        with patch("run_attribution.custom_run_coordinator.warnings"):
            yield coordinator
    ...
    def test_session_header_decode_failure(self, instance, coordinator):
        ...
        with get_foo_external_pipeline() as external_pipeline, patch(
            ...
            "run_attribution.custom_run_coordinator.warnings"
        ) as mock_warnings:
