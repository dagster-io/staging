...
from dagster import Field, ModeDefinition, Noneable, pipeline, resource, solid
...
@resource(
    config_schema={
        "db_url": str,
        "create_db_schema": Field(Noneable(str), is_required=False, default_value=None),
    }
)
def postgres(context):
    create_db_schema = context.resource_config["create_db_schema"]
    if create_db_schema is not None:
        engine = sqlalchemy.create_engine(
            context.resource_config["db_url"],
            execution_options={"schema_translate_map": {None: create_db_schema}},
        )
        with engine.connect() as conn:
            if not conn.dialect.has_schema(engine, create_db_schema):
                conn.execute(CreateSchema(create_db_schema))
    else:
...
def load_cereals_from_csv(context, csv_file_path):
    ...
    with context.resources.db.connect() as conn:
        cereals_df.to_sql(
            name="cereals", con=conn, if_exists="replace", schema="dbt-example-schema"
        )
