...
def remove_files_from_s3(s3_dir):
    ...
    print(f"Removing files from S3 path: {s3_dir}")
...
def upload_files_to_s3(source_dir):
    ...
    for root, _, files in os.walk(source_dir):
        ...
        for filename in files:
            ...
            print(f"Uploading file {source_path} to S3 path: {s3_path}")
...
def main(version, overwrite):
    ...
    if version in versions:
        ...
        if value == version:
            ...
            print(f'✅ successfully removed versioned content for version "{version}"')
            remove_files_from_s3(os.path.join("versioned_images", version))
            print(f'✅ successfully removed versioned images for version "{version}"')
    ...
    print(f"✅ successfully wrote versioned content for to path: {version_content_directory}")
    # Upload versioned content to s3
    upload_files_to_s3(version_content_directory)
    print(f'✅ successfully uploaded versioned content for version "{version}"')
    ...
    print(f"✅ successfully wrote versioned images for to path: {version_image_directory}")
    # Upload versioned images to s3
    upload_files_to_s3(version_image_directory)
    print(f'✅ successfully uploaded versioned images for version "{version}"')

    # Update versions file
    versions.append(version)
    write_json(os.path.join(VERSIONED_DIR, "_versions.json"), versions)
    print(f'✅ successfully updated {os.path.join(VERSIONED_DIR, "_versions.json")}')
