...
describe("for the config pipeline in config", () => {
  ...
  it("renders the playground view with the mini schema", async () => {
    await page.goto("http://localhost:3000/workspace/__repository__config_pipeline@config.py:config_pipeline/pipelines/config_pipeline/playground", {
      waitUntil: "networkidle2",
    });
    await sleep(500);
    await page.type(
    ...
    await page.type(".CodeMirror textarea", "  sort_by_calories\n" + "config:\n" + '  reverse: True');
    await page.waitForSelector('button[type="button"][status="ready"]')
    const image = await page.screenshot({
...
describe("for the custom types pipeline in metadata", () => {
  ...
  it("renders a pipeline run with the sort_by_calories.compute metadata", async () => {
    await page.goto("http://localhost:3000/workspace/__repository__custom_type_pipeline@custom_types.py:custom_type_pipeline/pipelines/custom_type_pipeline/playground", {
      waitUntil: "networkidle0",
    });
    await sleep(1000);
    await page.waitForSelector(".CodeMirror textarea", { timeout: 0 });
...
describe("for the custom types pipeline in expectations", () => {
  ...
  beforeAll(async (done) => {
    // dagit -f custom_types_5.py -a custom_type_pipeline
...
describe("for the materialization pipeline in materializations", () => {
  ...
  it("renders execution results for the materializations pipeline", async () => {
    await page.goto("http://localhost:3000/workspace/__repository__materialization_pipeline@materializations.py:materialization_pipeline/pipelines/materialization_pipeline/playgroun", {
      waitUntil: "networkidle0",
    });
    await sleep(500);
