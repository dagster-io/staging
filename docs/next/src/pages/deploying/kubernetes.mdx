...
The K8sRunLauncher is used in the [Default Deployment](/deploying/kubernetes#default-deployment) example. It creates a new K8s Job per pipeline run.

<PyObject module="dagster_celery_k8s" object="CeleryK8sRunLauncher" />:

The `CeleryK8sRunLauncher` is used in the [Advanced Deployment](/deploying/kubernetes#advanced-deployment) example. It creates a new K8s Job per step
