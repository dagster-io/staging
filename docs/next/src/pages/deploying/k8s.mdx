...
* [Default Architecture and Walk Through →](/deploying/k8s#architecture-default)
...
In this section, we cover the simplest deployment option from [quickstart](/deploying/k8s#quickstart-default), 
to [architecture](/deploying/k8s#architecture-default), to individual [components](/deploying/k8s#components-default).

### Quickstart (Default) 

The following steps are how to get up and running with the [default architecture](/deploying/k8s#architecture-default). This example requires a Kubernetes cluster.
...
The following steps set up the [advanced architecture](/deploying/k8s#architecture-adv). This example requires a Kubernetes cluster and a persistent file store (we use S3 
...
We briefly gloss over User Code Deployment and Dagit since these are the same as in the [Default Deployment](/deploying/k8s#default-deployment). 
...
The CeleryK8sRunLauncher is used in the [Advanced Deployment](/deploying/k8s#advanced-deployment) example. It creates a new K8s Job per step for step-level isolation and handling concurrency limits.
```shell
