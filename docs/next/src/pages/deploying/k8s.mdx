...
* [Quickstart: Running Dagster on K8s! →](/deploying/k8s#quickstart-default)
* [Default Architecture and Walk Through →](/deploying/k8s#default-deployment)
* [Default Components →](/deploying/k8s#components-default)
...
* [Quickstart: Running Advanced Dagster in Production →](/deploying/k8s#quickstart-adv)
* [Advanced Architecture and Walk Through →](/deploying/k8s#architecture-adv)
* [Advanced Components →](/deploying/k8s#components-adv)
* [Helm Configuration Options →](/deploying/k8s#configuration-adv)
...
In this section, we cover the simplest deployment option: from quickstart, to architecture, to individual components.
...
Within the `userDeployments.deployments` section of `values.yaml`, set `enabled: true` and add your deployment. 
...
starts the gRPC server. To find the applicable arguments, [read more here](/overview/repositories-workspaces/workspaces#grpc-server). 
...
--set k8sRunLauncher.enabled=true \
...
| -------------- | ----------- | ----------- |
| User Code Deployment  | [K8s Deployment](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/) behind a [K8s Service](https://kubernetes.io/docs/concepts/services-networking/service/) | User-provided (example: [dagster/k8s-example](https://hub.docker.com/r/dagster/k8s-example)) |
| Dagit       | [K8s Deployment](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/) behind a [K8s Service](https://kubernetes.io/docs/concepts/services-networking/service/)       |  [dagster/k8s-dagit](https://hub.docker.com/r/dagster/k8s-dagit) _(released weekly)_ |
| Daemon (Scheduler)       | [Deployment](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/)       | [dagster/k8s-dagit](https://hub.docker.com/r/dagster/k8s-dagit) _(released weekly)_ |
| Run Worker       | [K8s Job](https://kubernetes.io/docs/concepts/workloads/controllers/job/)       |  User-provided (same image as User Code Deployment)|
| Database       | PostgreSQL       |  (Opt) [postgres](https://hub.docker.com/_/postgres)|
...
[repository definition](/overview/repositories-workspaces/repositories#main) 
...
####  Database
The user can connect an external database (ie from a cloud provider) or run PostgreSQL on Kubernetes. This
...
The following steps set up the advanced architecture. This example requires a Kubernetes cluster and a persistent file store (we use S3 
in the example). Read about [GCP documentation here](/deploying/gcp).

**Step 1-4:** [Follow Step 1-4 in the Default Quickstart](/deploying/k8s#quickstart-default)
...
--set k8sRunLauncher.enabled=false \
--set userDeployments.enabled=true \
...
| User Code Deployment  | [K8s Deployment](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/) behind a [K8s Service](https://kubernetes.io/docs/concepts/services-networking/service/) | User-provided (example: [dagster/k8s-example](https://hub.docker.com/r/dagster/k8s-example)) |
| Run Worker       | [K8s Job](https://kubernetes.io/docs/concepts/workloads/controllers/job/)       |  User-provided (same image as User Code Deployment)|
| Step Job       | [K8s Job](https://kubernetes.io/docs/concepts/workloads/controllers/job/)       |  User-provided (same image as User Code Deployment)|
| Dagit       | [K8s Deployment](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/) behind a [K8s Service](https://kubernetes.io/docs/concepts/services-networking/service/)       |  [dagster/k8s-dagit](https://hub.docker.com/r/dagster/k8s-dagit) _(released weekly)_ |
| Daemon       | [Deployment](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/)       | [dagster/k8s-dagit](https://hub.docker.com/r/dagster/k8s-dagit) _(released weekly)_ |
| Database       | PostgreSQL       |  (Opt) [postgres](https://hub.docker.com/_/postgres)|
| Celery       | [K8s Deployment](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/)       |  [dagster/k8s-celery-worker](https://hub.docker.com/r/dagster/k8s-celery-worker) _(released weekly)_|
| Flower (Opt)       | [K8s Deployment](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/) behind a  [K8s Service](https://kubernetes.io/docs/concepts/services-networking/service/) | [mher/flower](https://hub.docker.com/r/mher/flower)
...
Using Celery requires configuring the CeleryK8sRunLauncher and CeleryK8sExecutor ([See Configuration](/deploying/k8s#configuration-adv)).
...
####  Run Launcher
The [Run Launcher](/overview/run-launchers-executors/run-launcher#main) is responsible for 
allocating the computational resources for run execution.

[K8sRunLauncher](/_apidocs/libraries/dagster_k8s#dagster_k8s.K8sRunLauncher) [Default]: The K8sRunLauncher
...
[CeleryK8sRunLauncher](/_apidocs/libraries/dagster_celery_k8s#dagster_celery_k8s.CeleryK8sRunLauncher): 
The CeleryK8sRunLauncher is used in the _Advanced Deployment_ example. It creates a new K8s Job per step for step-level isolation and handling concurrency limits.
```shell
...
With the K8sRunLauncher, users can utilize the [In Process Executor](/_apidocs/execution#dagster.in_process_executor), the 
the [Multiprocess Executor](/_apidocs/execution#dagster.multiprocess_executor), 
[Celery Executor](/_apidocs/libraries/dagster_celery#dagster_celery.celery_executor), 
[Celery Docker Executor](/_apidocs/libraries/dagster_celery_docker#dagster_celery_docker.celery_docker_executor), 
or a custom Executor.

With the CeleryK8sRunLauncher, user can only use the [CeleryK8sJobExecutor](/_apidocs/libraries/dagster_celery_k8s#dagster_celery_k8s.celery_k8s_job_executor). 
As seen in the Advanced Deployment example, this creates a Celery task for each Step, allowing for step-level isolation and concurrency control.
...
and managing the queue of runs. This is used in both the _Default Deployment_ and _Advanced Deployment_ examples and is our recommended
approach.
