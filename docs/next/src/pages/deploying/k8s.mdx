...
The following steps are how to get up and running with the production-grade architecture. This example requires a Kubernetes cluster and S3.
...
```shell
helm repo add dagster https://dagster-io.github.io/helm
helm install dagster dagster/dagster -f /path/to/values.yaml --set k8sRunLauncher.enabled=false --set userDeployments.enabled=true --set celery.enabled=true --set rabbitmq.enabled=false
```

To make changes to "dagster" release:

```shell
helm upgrade dagster dagster/dagster -f helm/dagster/values.yaml  --set k8sRunLauncher.enabled=false --set userDeployments.enabled=true --set celery.enabled=true --set rabbitmq.enabled=false
```
...
We cover options that are supported out-of-the-box. We encourage users to write custom Run Launcher and Executor if other execution
substrates are needed. If you have questions, please reach out to us on [Slack](https://dagster-slackin.herokuapp.com/)!
