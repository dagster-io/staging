...
In the _Default Architecture_ section, we go over the simplest way to get started:
...
In the _Production-Grade Architecture_ section, we build off of the _Default Architecture_ and add more sophisticated features including 
solid-level isolation and queuing mechanisms.
* Production-Grade Architecture [→](https://docs.dagster.io/deploying/k8s#production-architecture)
...
First, we walk through the steps that occur when a user opens Dagit and clicks _Launch Run_. Next, we describe the different components.

### Walk Through Steps

After a user clicks _Launch Run_, 
...
starts the gRPC server. For more information on argument options, [go here](https://docs.dagster.io/overview/repositories-workspaces/workspaces#grpc-server). 
