...
### Celery-less K8s
You can also configure Dagster to not use Celery. In this case, the <PyObject
module="dagster_k8s" object="K8sRunLauncher" /> will be used, which runs pipelines as a
...
Then, in the run config, an <PyObject module="dagster" object="in_process_executor" /> or <PyObject
module="dagster" object="multiprocess_executor" /> can be configured to run the execution
