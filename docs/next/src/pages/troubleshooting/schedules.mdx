...
These steps assume that you are using the default scheduler on your instance, which executes your
...
The left-hand navigation bar in dagit shows all of your schedules, with a green dot next
to each schedule that is running. Make sure that your schedule appears in this list with a green dot.
To ensure that dagit has loaded the latest version of your schedule code, you can press the reload
button next to your repository name to reload all the code in your repository.

- If dagit is unable to load the repository containing your schedule (for example,
  due to a syntax error or a problem with one of your definitions), there should be an error message in the left nav with a link that will give you more information about the error.
- If the repository is loading but the schedule doesn't appear in the list of schedules, make sure that your
  schedule function is included in the list of schedules returned by your repository function.
...
First, check that the daemon is running. Click on "Instance details" in the left nav in Dagit, and
examine the "Scheduler" row under the "Daemon statuses" section. The daemon process periodically sends
out a heartbeat from the scehduler, so if the scheduler daemon is listed as "Not running", that
indicates that there's a problem with your daemon deployment. If the daemon ran into an error that
caused it to throw an exception, that error will often appear in this UI as well.
