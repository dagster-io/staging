...
hourly. Suppose that we need to run our simple cereal pipeline every morning before breakfast, at
6:45 AM.
...
Now we'll write a <PyObject module="dagster" object="ScheduleDefinition" /> to define our schedule.
...
The scheduler implementation included in dagster by default requires you to launch a long-running
`dagster-daemon` process in addition to dagit.
