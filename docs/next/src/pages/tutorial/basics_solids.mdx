...
file:/docs_snippets/docs_snippets/intro_tutorial/basics/e02_solids/inputs.py
startAfter:start_inputs_main
endBefore:end_inputs_main
dedent:4
```
...
```python literalinclude showLines startLine=8 emphasize-lines=9-16,26  caption=config_schema.py
file:/docs_snippets/docs_snippets/intro_tutorial/basics/e02_solids/config_schema.py
lines:8-37
```

First, we pass the `config` argument to the <PyObject module="dagster" object="solid" displayText="@solid" /> decorator.
This tells Dagster to give our solid a config field structured as a dictionary. The values of the dictionary are
instances of <PyObject module="dagster" object="Field" />.
...
object="SystemComputeExecutionContext" displayText="context" /> object.

We can also pass a `config` argument to next <PyObject module="dagster" object="solid" 
displayText="@solid" /> `sort_by_calories`. 

```python literalinclude showLines startLine=43 emphasize-lines=5  caption=config_schema.py
file:/docs_snippets/docs_snippets/intro_tutorial/basics/e02_solids/config_schema.py
lines:43-67
```

Let's run config with `"calories": 130` like following and see how all of these look in Dagit:

```python literalinclude showLines startLine=76  caption=config_schema.py
file:/docs_snippets/docs_snippets/intro_tutorial/basics/e02_solids/config_schema.py
lines:76-81
```
...
The human-readable descriptions of `delimiter` and `escapechar` we provided on our config fields
appear in the config section on the right when we click into the `read_csv` solid definition.

![config_figure_two.png](/assets/images/tutorial/config_figure_two.png)

Our `inputs_pipeline` run successfully with the config schemas we defined for our solids.
