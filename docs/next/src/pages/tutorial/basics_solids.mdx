...
```python literalinclude showLines startLine=16 emphasize-lines=2-10,17,20 caption=config.py
...
by calories in a reversed order, we will have to set the `reverse` value to `True` and provide this key value pair to the `config`.

We can also pass the run config in a YAML file to the Dagster CLI tool like below. Since we want to find the most caloric cereal,
we set the reverse value to `True` in the `config` section.
