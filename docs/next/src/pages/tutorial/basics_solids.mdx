...
```python literalinclude showLines startLine=16 emphasize-lines=2-11,17 caption=config.py
...
Then, we define one of these fields, `reverse`, to be a boolean, setting a default value `False`, making
...
```python literalinclude showLines startLine=60 caption=config.py
file:/docs_snippets/docs_snippets/intro_tutorial/basics/e02_solids/config.py
...
to <PyObject module="dagster" object="solid" displayText="@solid" />. In this case, we have only defined one field, `reverse`.

Because the `reverse` field is not required and it is `False` by default, this line is optional. Only when we want to sort cereals
by calories in a reversed order, we will have to set the `reverse` value to `True` and provide this key value pair to `config`.

We can also pass the run config in a YAML file to the Dagster CLI tool like below. Since we don't provide the `config` section,
the config will set reverse value to `False` by default.
...
Let's see how all of these looks in Dagit. As usual, run:
