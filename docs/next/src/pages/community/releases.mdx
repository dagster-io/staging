...
We’re in the 0.y.z “anything goes” phase of [SemVer](https://semver.org/), so we do sometimes make backwards-incompatible changes without bumping our major version.  Here’s what to expect when upgrading:

* Upgrading to a new dot version within a minor release, e.g. 0.8.1 → 0.8.2, should almost never result in broken code.
...
If you notice an API breaking within a minor version, then it’s likely a bug, and we encourage you to let us know in Slack or by filing an issue on [github](https://github.com/dagster-io/dagster/issues).
