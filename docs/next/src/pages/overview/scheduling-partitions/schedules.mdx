...
For example, imagine we have a pipeline that computes metrics data for a given date, and want to
...
`scheduled_execution_time` property on the passed-in `ScheduleExecutionContext`.
...
of the `dagster-daemon` process (TODO link to docs). This scheduler uses checkpointing to ensure
that each schedule interval runs exactly once, even if the scheduler goes down. See TODO LINK HERE for more
information on how to deploy your scheduler.

Dagster also supports `SystemCronScheduler` and `K8sScheduler` as deprecated schedulers.
...
occur if different components in your deployment are using different system timezones.
...
3:00AM when the time zone changes) - when this happens, Dagster will run your schedule at the next time that exists (in this case,
3:00AM). It's also possible to specify a time that exists twice once every year (for example,
