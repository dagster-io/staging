...
<PyObject module="dagster.core.launcher" object="DefaultRunLauncher" />. This
run launcher spawns a new process on the same node as the pipeline's repository
location. It also provides the ability to terminate launched runs, and monitors
launched processes to detect unexpected process crashes.
