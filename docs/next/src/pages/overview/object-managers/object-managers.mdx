...
When running the full pipeline, `solid2`'s input will be loaded using the object manager on the output of `solid1`.  When running the pipeline subset, `solid2`'s input has no upstream output, so the input manager corresponding to its `root_manager_key` is used.
