...
Setting the `manager_key` on an `InputDefinition` controls how that input is loaded in pipelines where it has no upstream output.
...
To accomplish this, you can set up the `manager_key` on `solid2`'s `InputDefinition` to point to an input manager with the desired loading behavior.  As before, setting the `manager_key` on an `InputDefinition` controls how that input is loaded when it has no upstream output.
...
When running the full pipeline, `solid2`'s input will be loaded using the object manager on the output of `solid1`.  When running the pipeline subset, `solid2`'s input has no upstream output, so the input manager corresponding to its `manager_key` is used.
