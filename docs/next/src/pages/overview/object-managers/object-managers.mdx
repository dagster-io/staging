...
The default ObjectManager, <PyObject module="dagster" object="mem_object_manager" />, stores outputs in memory, but this only works for the single process executor.  Dagster provides out-of-the-box ObjectManagers that pickle objects and save them to a [local filesystem](/_apidocs/object-managers#dagster.fs_object_manager), [Amazon S3](/_apidocs/libraries/dagster_aws#dagster_aws.s3.s3_object_manager), [Azure ADLS2](/_apidocs/libraries/dagster_azure#dagster_azure.adls2.adls2_object_manager), or [GCS](/_apidocs/libraries/dagster_gcp#dagster_gcp.gcs.gcs_object_manager).
