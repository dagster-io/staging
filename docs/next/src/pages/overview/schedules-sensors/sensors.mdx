...
By default, sensors are configured to run roughly every 30 seconds, which is a setting specified
on the sensor definition. The minimum interval specified on the sensor definition
guarantees that the sensor will not be evaluated at more frequently than that interval.
