...
Partition-based schedules are useful when it's important for a successful run to exist for every scheduler tick.
For example, if each run fills in a single partition of a data warehouse table, basing your schedule
on a partition set allows you to track which partitions have corresponding runs and fill in gaps
when runs fail.
...
recent scheduler tick. For example, if each run drops and recreates a data warehouse table, then
there's usually no need to kick off runs corresponding to historical ticks - those runs would just
have the same effect as a run for the most recent tick.
