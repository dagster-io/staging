...
loaded independently, and ensures that errors in user code can't impact Dagster system code. A
repository location can include more than one repository.
...
has a unique name that is displayed in Dagit. The location name is also used to disambiguate
definitions with the same name across multiple locations. Dagster will supply a default name
for each location based on its workspace entry, but you can also supply a custom name.
