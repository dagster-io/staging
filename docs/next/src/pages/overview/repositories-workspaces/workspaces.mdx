...
Tools in Dagster ecosystem such as Dagit and the Dagster CLI need to be able to know where
...
Each entry in the workspace is referred to as a _repository location_. Each repository location
is loaded in its own server process, and Dagster tools use an RPC protocol to communicate wtih the
repository location. This allows multiple repository locations in different environments to be
...
If you use `python_file`, it must specify a path to a file containing the repository definition,
relative to the location of the workspace file. For example, the repository here:
...
could be loaded using the following workspace file:
...
package. A deprecated `python_module` key with similar semantics also exists, but in new code you
...
You can also specify an attribute to identify a single repository within a module or installed
package, as with `python_file`:
...
has a unqiue name that is displayed in dagit and is used to disambiguate definitions with the same name
across multiple locations. Dagster will supply a default name for each location based on its
workspace entry, but you can also supply a custom name as well.
...
example, you may need to restart any schedules in that repository location that were previously
started, since those schedules were expressed using the previous repository location name. You can
avoid this situation by giving each of your locations a unique name from the beginning.
...
might be a syntax error or some other unrecoverable error in one of its definitions. When this occurs,
...
each of your repository locations. However, it is also possible to run your own gRPC server on
...
Similarly to when specifying code in your `workspace.yaml`, the target can be either a
...
and load all repositories within the specified target.
...
If you are using the <PyObject module="dagster.core.launcher" object="DefaultRunLauncher"/>, which
is configured by default on your <PyObject module="dagster" object="DagsterInstance"/>, the run
launcher will launch runs against your hosted gRPC server. The gRPC server needs to be able to
access your run storage in order to be able to execute launched runs.
