...
Dagster solids have inputs and outputs.  When a solid produces an output, where is it stored?  AssetStores let the developer decide.

AssetStores are user-provided objects that know how to store step outputs and retrieve step inputs.  Each step output can have its own AssetStore, or multiple step outputs can share an AssetStore.  The AssetStore that’s used for storing a particular step output is automatically used for retrieving it in subsequent steps.
...
The default AssetStore stores outputs in memory, but this only works for the single process executor.  Dagster provides out-of-the-box AssetStores that pickle objects and save them to a local filesystem, Amazon S3, Azure ADLS, or GCS.
...
By default, all the steps in a pipeline use the same AssetStore, determined by the ResourceDefinition provided by the “asset_store” resource key.  Here’s how to specify that all step outputs are stored using the <PyObject module="dagster" object="fs_asset_store" />, which pickles outputs and stores them on the local filesystem.  It stores files in a directory with the run ID in the path, so that outputs from prior runs will never be overwritten.
...
TODO: link to other default asset stores


## Providing a custom asset store

If you have specific requirements for where and how your outputs should be stored, you can define your own AssetStore.  For example, if your solids output Pandas DataFrames to populate tables in a data warehouse, you might write the following:
...
The AssetStore can then access this metadata when storing or retrieving data.
