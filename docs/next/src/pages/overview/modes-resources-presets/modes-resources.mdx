...
Resources can depend upon other resources. If a resource key is included in the `required_resource_keys` set provided to the resource definition, then the body of the resource can access a required resource via the "resources" attribute of its context object. We use a required key when defining the database resources to represent the domain name of where to point our connection.
