...
configured solid to take on a different name. Since each solid definition within a repository must
have a unique name, make sure you give configured solids different names if you want to use them
together with the original solid or if you are configuring one solid in multiple ways within the
same repository. Below is one case that would require this pattern:
