...
Default configuration is provided for both of these pipelines using [Presets](/docs/apidocs/pipeline#presets), and the corresponding YAML files can be found under the `environments/` folder.
...
That's because these are actually [Composite Solids](/docs/apidocs/pipeline#presets): solids that contain other solids. Dagster supports arbitrarily nested, composed solid hierarchies, which can be really useful for encapsulating functionality as we've done here. Click Expand and you'll see the solids contained within `s3_to_df`:
...
Dagster decouples the instantiation of external resources like these from the business logic of your data pipelines. In Dagster, a set of resources configured for a particular environment is called a [Mode](/sections/api/apidocs/pipeline.html#modes).
