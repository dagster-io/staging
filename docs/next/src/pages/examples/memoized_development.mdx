...
```python literalinclude caption=repo.py showLines startLine=0
...
for outputs, and only run steps that have not been memoized. We also use
`dagster.core.storage.versioned_filesystem_io_manager` as our `io_manager`, which is a
<PyObject module="dagster.core.storage.memoizable_io_manager" object="MemoizableIOManager" />.
...
Here is the code to hash a file in python:
