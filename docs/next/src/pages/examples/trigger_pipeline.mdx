...
Then, you provide the inputs for the `repositoryLocationName`, `repositoryName`, `pipelineName`,
`runConfigData`, and `mode`. Lastly, execute the query on the client to get the result.
...
Before you run this example script, remember to bring up Dagit first.
You can either load the repository from a file or load from a module.
If you load from a Python file, run `dagit -f repo.py`.
If you load from a module, run `dagit -m repo`. Don't forget to change your input to `repo` as well.
Then, open another tab in your terminal, run `python trigger.py` to execut the script.
