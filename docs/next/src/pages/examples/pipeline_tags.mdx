...
optional, JSON-encodable metadata to be attached to the solid. Some frameworks may expect or
require certain metadata to be attached to a solid.

In this example, we have a solid `get_tag` that contains `tags={"metadata": "some_metadata"}`.
Tags for solids can also be accessed during the execution of a pipeline (see `get tag`).
