...
with the [`configured`](/overview/configuration/configured) Dagster API. The solid executes `dbt run`
via the dbt CLI. As you may have noticed in the log above, the solid also logs the output from the dbt
CLI execution in a few <PyObject module="dagster" object="AssetMaterilization" /> events, which allow
you to track the external entities in the ["Asset Catalog"](/overview/asset-materializations).
