...
However, sometimes it doesn't make sense to use Dagster's inputs/outputs to model the dependency. For example, if the body of one solid creates a particular table in a database and the body of another solid consumes that table, then Dagster is not aware of the data flow.
