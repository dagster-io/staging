...
In this situation, we can use "nothing" inputs and outputs.  Nothing inputs and outputs are a special kind of input and output that expect no data to be returned or consumed.  By hooking up a "nothing" output of the first solid to a "nothing" input of the second solid, Dagster understands that the second should execute after the first.  Unlike with typical inputs and outputs...
* The first solid does not return or yield a value corresponding to the output.
* The second solids does not include an argument for the input in its decorated function.
