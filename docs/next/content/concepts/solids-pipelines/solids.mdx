...
Solids are the functional unit of work in Dagster. A solid's responsibility is to read its inputs, perform an action, and optionally emit outputs. Multiple solids can be connected to create a [Pipeline](/concepts/solids-pipelines/pipelines).
...
Solids are used to organize related computations. Solids can later be assembled into Pipelines [Pipelines](/concepts/solids-pipelines/pipelines). They generally perform one specific action and are used for batch computations. For example, you can use a solid to:
...
# - The dagster type is checked at runtime. If the type check fails, solid execution will fail.
