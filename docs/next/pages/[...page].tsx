...
async function getVersionedContent(version: string, asPath: string) {
  ...
  try {
    ...
    // if unable to get remote content, fall back to get local content
    console.log(
      "Warning: versioned content is not available because we are unable to get it from remote S3 bucket. ",
      "Error: \n",
      err
    );
    return getLocalContent(asPath);
  }
}

async function getLocalContent(asPath: string) {
...
async function getContent(version: string, asPath: string) {
  if (version == "master") {
    // render files from the local content folder
    const contentString = getLocalContent(asPath);
    return contentString;
  } else {
