...
The [dagster/user-code-example](https://hub.docker.com/r/dagster/user-code-example) uses an [S3 IO Manger](/deployment/guides/aws#using-s3-for-io-management).

Therefore, you'll need an AWS S3 bucket available, and access to a pair of `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` values, since the IO Manager uses [boto](https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/s3.html).

This tutorial also has the option of using [`minio`](https://min.io/) to mock an S3 endpoint locally in K8s. Note that this option has only been tested on MacOS, so may need different configuration for other platforms.
...
_Skip this step if you're using minio_
...
Note the snippet below has been tested on MacOS only - we customize the S3 resource to use the minio endpoint instead.
