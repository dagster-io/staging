...
In practice, you may want to volume mount your pipeline code into your containers to enable deployment patterns such as git-sync sidecars that avoid the need to rebuild images and when pipeline code changes.
...
This launcher will start the container using the image that you set in the `DAGSTER_CURRENT_IMAGE` environment variable in your pipeline container (which will usually be the same image as the pipeline container itself)

Your container that is launching runs (usually the `dagster-daemon` container) must have permissions to create Docker containers in order to use this run launcher (mounting `/var/run/docker.sock` as a volume is one way to give it these permissions).
