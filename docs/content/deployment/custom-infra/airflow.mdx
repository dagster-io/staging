...
We don't recommend deploying Dagster pipelines to Airflow in greenfield installations. (We recommend the built-in [scheduler and partition system](/tutorial/advanced-tutorial/scheduling) for scheduling pipelines, and the [Celery-based executor](/deployment/custom-infra/celery) for large-scale deployments.) But if you have a large existing Airflow install, this integration will allow you to follow an incremental adoption path.
