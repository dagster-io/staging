...
To define a solid, use the <PyObject object="solid" decorator /> decorator. The decorated function is called the `compute_fn`. Users can optionally provide `context` as a first argument, which Dagster will supply during execution. The `context` provides access to important properties and objects, such as [solid configuration](/concepts/solids-pipelines/solids#solid-configuration) and [resources](/concepts/modes-resources).
