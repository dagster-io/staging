...
In most cases, the structure of a pipeline is pre-determined before execution. Dagster now has experimental support for creating pipelines where the final structure is not determined until run-time. This is useful for map/reduce pipeline pipeline structures, where you want to excute a set of solids for each result of another solid.
