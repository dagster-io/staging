...
To have multiple outputs from a composite solid, you will need to define the outputs on the composite solid and return a dictionary, where the keys are the output names and the values are the output values.
