...
Dagster provides a Python GraphQL Client to interface with <Link href="/concepts/dagit/graphql">Dagster's GraphQL API</Link> from Python.
...
When is this useful? Dagster exposes a powerful GraphQL API which allows for flexible manipulation of requests to the Dagster server.

This includes collecting data on pipeline runs, submitting new runs, and programmatically reloading Dagit.

However, this level of flexibility is not always necessary - for example, when submitting a new pipeline run,
you may only want to think about the pipeline name and configuration and to think less about maintaining a long GraphQL query.
...
`DagsterGraphQLClient` is available with the following methods:

- <PyObject module="dagster_graphql" object="DagsterGraphQLClient" method="submit_pipeline_execution"/>
- <PyObject module="dagster_graphql" object="DagsterGraphQLClient" method="get_run_status"/>
- <PyObject module="dagster_graphql" object="DagsterGraphQLClient" method="reload_repository_location"/>

### Backwards Compatability

The GraphQL API is still evolving and is subject to breaking changes, and thus so is the Python Client. For any of the methods listed above on the client, we will be very clear about breaking changes in release notes.
