...
The type system truly shines once the type check expresses richer behavior, such as column-level schema on a dataframe. For example, check out the [Validating Pandas DataFrames with Dagster Types](/guides/integrations/pandas) guide.
