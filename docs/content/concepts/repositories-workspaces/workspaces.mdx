...
A _workspace_ is a collection of user-defined [repositories](/concepts/repositories-workspaces/repositories) and information about where to find them.
Dagster tools, like Dagit and the Dagster CLI, use workspaces to load user code.
...
sourced from a variety of different locations, such as Python modules or Python virtualenvs.
...
- You can use multiple repositories to organize pipelines, but still work on all of your repositories using a single running Dagit.
...
- Each repository can be sourced from a separate Python environment, so teams can manage their dependencies
  (or even their Python versions) separately.
...
include more than one repository. Each repository location is loaded in its own process that
...
containing at least one repository definition. For example, the repository defined in
`repository.py` here:
...
Dagit will look for the `workspace.yaml` file in the current directory by default, allowing you to launch Dagit
from that directory with no arguments and see the repositories defined in the workspace:

    dagit

<img src="/images/concepts/workspace-dagit.png" />

Using a `workspace.yaml` keeps you from having to type the same
`-f` or `-m` flag repeatedly. You can also load the workspace yaml file from a different location with `-w`. See detailed references
...
You might have more than one repository in scope and want to specify a specific
one. Our schema supports this with the `attribute` key, which must be a repository
...
Renaming a repository location or changing its configuration in your `workspace.yaml` file
may cause parts of the Dagster system that rely on that name to need to be re-configured. For
example, you may need to restart any schedules in that repository location, since those schedules
were marked as started using the previous repository location.
...
By default, Dagster tools will automatically create a process on your local machine for
