...
Dagster provides a number of decorators (<PyObject object="hourly_schedule" />, <PyObject object="daily_schedule" />, <PyObject object="weekly_schedule" />, <PyObject object="monthly_schedule" />) that produce [partitioned schedules](https://docs.dagster.io/concepts/partitions-schedules-sensors/schedules#partition-based-schedules). These conveniently produce both the partition set that defines the configuration space of a specific pipeline and the schedule that it runs against.

In many cases, you may instead want to define the partition set and then manually define a schedule for it. In order to do that, you must first define a partition selector which returns a <PyObject object="Partition" /> object or a list of <PyObject object="Partition" /> objects. Every time the schedule fires, a pipeline run will be submitted for each partition returned by this partition selector.

For partition sets consisting of `datetime` objects, the <PyObject object="identity_partition_selector" /> partition selector and the <PyObject object="create_offset_partition_selector" /> factory function might be useful.
