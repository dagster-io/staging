...
- Partitioned [Schedules](/concepts/partitions-schedules-sensors/schedules): You can construct a schedule
  that runs a targets a partition with each run it launches. For example, a pipeline might run each day and
  process the data that arrived during the previous day.
- [Backfills](/concepts/partitions-schedules-sensors/backfill): You can launch a set of pipeline runs
  all at once, each run targeting one of the partitions in the set. For example, after making a code change,
  you might want to re-run your pipeline on every date that is has run on in the past.
