...
Schedules have several important properties. Each schedule:
...
Dagster includes a scheduler, which runs as a part of the dagster-daemon process. Once you have
...
launching a [backfill](/overview/partitions-backfill/backfill).
...
When you want to run the schedule on a fixed interval and don't need partitions, you
...
parameter on your schedule to any valid IAMA timezone. Schedules with no timezone set run in UTC.
...
begin executing immediately. See the Troubleshooting section below if your schedule has been
started but isn't submitting runs.
