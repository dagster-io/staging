...
For use-cases where you want to alert on a pipeline's failure, you can build a sensor listening to
the Dagster runs table, and let it instigate an “alert” pipeline run when certain condition
meets.
...
Then, you can build a sensor which fetches the failed runs from the runs table via `context.instance`,
and instigates a `failure_alert_pipeline` run for every failed run. Note that we use the failed run's
id as the `run_key` to avoid duplicate alerts.
