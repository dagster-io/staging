...
In order to unit test sensors, you can use the <PyObject object="build_sensor_context" /> function to construct a <PyObject object="SensorExecutionContext"/>. You can then invoke the sensor directly, and pass the context as an argument, which will return all run requests yielded from the sensor. The config obtained from these can be validated using the <PyObject object="validate_run_config" /> function.
