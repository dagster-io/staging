...
| <PyObject object="SensorContext"  />          | The context object passed to a sensor evaluation function.                                                                                                                                                 |
| <PyObject object="build_sensor_context"    /> | A function that constructs an instance of <PyObject object="SensorContext" />, This is intended to be used to test a sensor.                                                                               |
...
To define a sensor, use the <PyObject object="sensor" decorator /> decorator. The decorated function is called the `execution_fn` and must have `context` as the first argument. The context is a <PyObject object="SensorContext" />
...
- `cursor`: A cursor field on <PyObject object="SensorContext"/> that returns the last persisted cursor value from a previous evaluation.
- `update_cursor`: A method on <PyObject object="SensorContext"/> that takes a string to persist and make available to future evaluations.
