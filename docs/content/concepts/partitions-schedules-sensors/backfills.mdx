...
After defining a [Partition Set](/concepts/partitions-schedules-sensors/partitions#defining-a-partition), you can use backfills to instigate pipeline runs for each partition in the set.
...
You can launch and monitor backfills of a pipeline page in Dagit from the [Partitions tab](/concepts/partitions-schedules-sensors/partitions#launching-partitions).
