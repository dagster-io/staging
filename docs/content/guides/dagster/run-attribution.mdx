...
In this example, we'll perform Run Attribution by using a Custom Run Coordinator to read HTTP headers and attach a user's email as a tag to submitted pipeline runs.

### Custom Run Coordinator

We can subclass the Queued Run Coordinator as follows:
...
Then we can parse through the header (in this case, called `jwt_claims_header`) with any custom hook. In the following example, we're decoding a JWT header which contains the user's email.
