...
The <Link href="/deployment/run-coordinator">Run Coordinator</Link> is used to manage the policy that Dagster uses to manage the set of active runs on your deployment.
...
In this use case, we'd like to add a hook to customize submitted runs while still using a queue to submit runs to the [Dagster Daemon](/deployment/dagster-daemon).
To accomplish this, we can use the <Link href="/deployment/run-coordinator#queuedruncoordinator">Queued Run Coordinator</Link> as follows:
...
Then we can parse through the header (in this case, called the `jwt_claims_header`) with any custom hook. In the following example, we're decoding a JWT header which contains the user's email.
