...
With the pipeline-centric set of APIs, you would declare a partitioned schedule by using one of the partitioned schedule decorators, like <PyObject object="daily_schedule"/>, <PyObject object="hourly_schedule"/> etc. The created schedule would target a specific pipeline via `pipeline_name`.

Both the schedule and pipeline need to be provided to the repository in order to use the schedule.

```python file=/guides/dagster/graph_job_op/pipeline_with_partition_schedule.py
from dagster import daily_schedule, solid, pipeline, repository
import datetime
...
With the graph APIs, you specify the schedule as a set of partitions on which a job can run. The resulting schedule can be independently passed to a repository, and the underlying job will be inferred.

```python file=/guides/dagster/graph_job_op/graph_with_partition_schedule.py
from dagster import graph, daily_partitioned_config, schedule_from_partitions, op, repository
