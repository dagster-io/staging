...
For simple pipelines, the new APIs have an identical shape to the old APIs, but with different names. Here's a pipeline containing a single solid:
...
And here's a graph containing a single op:
...
With the old APIs, supplying resources to a pipeline involves defining a mode:

```python file=/guides/dagster/graph_job_op/pipeline_with_resources.py
from dagster import pipeline, solid, ModeDefinition, resource
...
With the old APIs, if you wanted to execute a pipeline with resources for a test, you would need to include a mode for those resources on the pipeline definition:

```python file=/guides/dagster/graph_job_op/pipeline_mode_test.py
from dagster import pipeline, solid, ModeDefinition, resource, execute_pipeline, ResourceDefinition
...
from dagster import graph, op, resource, ResourceDefinition
from unittest.mock import MagicMock
...
With the old APIs, if you wanted to specify the config for a pipeline at the time you were defining it (as opposed to at the time you were running it), you would use a <PyObject module="dagster" object="PresetDefinition" />.
...
With the old APIs, a schedule targets a pipeline by referencing the pipeline name in the schedule definition.

```python file=/guides/dagster/graph_job_op/pipeline_with_schedule.py
from dagster import pipeline, solid, ScheduleDefinition
...
With the old APIs, if you want to have the same config available for manual runs of a pipeline and used in a schedule, you need to do something like this:

```python file=/guides/dagster/graph_job_op/pipeline_with_preset_and_schedule.py
from dagster import pipeline, solid, PresetDefinition, schedule
