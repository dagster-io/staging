...
@solid(
    required_resources_keys={"tempdir"},
    output_defs=[OutputDefinition(metadata={"persistent_path_suffix": "/a/b/c"})],
)
def create_file(context) -> Path:
    local_path = Path(context.resources.tempdir) / "my_local_file"
    subprocess.run(["touch", local_path])
    return local_path


@solid
def stat_file(local_path: Path):
    print(Path(local_path).stat().st_size)
```
...
class S3FileIOManager(IOManager):
    def handle_output(self, context, obj):
        s3_client().upload_file(obj, "my_bucket", context.metadata["persistent_path_suffix"])

    def load_input(self, context):
        tempfile_path = Path(context.resources.tempdir) / str(uuid.uuid4())
    ...
    def get_output_asset_key(self, context):
        return AssetKey(["my_bucket", context.upstream_output.metadata["persistent_path_suffix"]])


@io_manager(required_resources={"tempdir"})
def s3_file_io_manager():
...
class LocalFileIOManager(IOManager):
    def handle_output(self, context, obj):
        ...
        persistent_path.parent.makedir(parent=True, exist_ok=True)
        shutil.copy(obj, persistent_path)

    def load_input(self, context):
        persistent_path = Path("base_dir") / context.metadata["persistent_path_suffix"]
        tempfile_path = Path(context.resources.tempdir) / str(uuid.uuid4())
        shutil.copy(persistent_path, tempfile_path)


@io_manager(required_resources={"tempdir"})
def local_file_io_manager():
