...
import {
  SensorTimelineQuery,
  SensorTimelineQuery_sensorOrError_Sensor_futureTick,
...
type FutureTick = SensorTimelineQuery_sensorOrError_Sensor_futureTick;
...
const TickTimelineGraph: React.FC<{
  ...
  onSelectRunIds: (runIds: string[]) => void;
}> = ({ticks, futureTick, onSelectRunIds}) => {
  ...
  React.useEffect(() => {
    if (!isPaused && (!futureTick || now < 1000 * futureTick.timestamp)) {
      setGraphNow(now);
    }
  }, [isPaused, futureTick, now]);

  const tickData = ticks.map((tick) => ({x: 1000 * tick.timestamp, y: 0}));
  if (futureTick) {
    tickData.push({x: 1000 * futureTick.timestamp, y: 0});
  }

  const isAtFutureTick = futureTick && 1000 * futureTick.timestamp <= now;
  const PULSE_DURATION = 2000;
  const futureTickRadius = isAtFutureTick
  ...
  const graphData: ChartComponentProps['data'] = {
    ...
    datasets: [
      ...
      },
      {
        ...
        pointRadius: [...Array(ticks.length).fill(3), futureTickRadius],
