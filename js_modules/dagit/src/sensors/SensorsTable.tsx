...
export const SensorsTable: React.FC<{
  ...
  const lastRequested =
    'The status of the runs requested in the last tick that resulted in a `Requested` state';
  const partitionStatus = (
    <div>
      The status of each partition in the partition set associated with this schedule. <br />
      Partitions have a `Success` status if the last run for that partition completed <br />
      successfully. Partititons have a `Missing` status if no run has been executed for that <br />
      partition.
    </div>
  );
  return (
        ...
        <thead>
          <tr>
            ...
            <th>
              <Box flex={{direction: 'row', alignItems: 'center'}}>
                Last Run
                <Tooltip position={'right'} content={lastRequested} wrapperTagName="div">
                  <Icon icon={IconNames.INFO_SIGN} style={{marginLeft: 10}} iconSize={15} />
                </Tooltip>
              </Box>
            </th>
