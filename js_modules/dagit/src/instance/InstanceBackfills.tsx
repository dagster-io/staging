...
const BackfillTable = ({backfills, refetch}: {backfills: Backfill[]; refetch: () => void}) => {
  ...
  const cancelableRuns = cancelRunBackfill?.runs
    .filter((run) => !doneStatuses.has(run?.status))
    .filter((run) => run.canTerminate);
  const unfinishedMap =
...
const BackfillRow = ({
  ...
  const partitionSetBackfillUrl =
    backfill.partitionSet && backfill.partitionSetOrigin
      ? workspacePathFromAddress(
          backfill.partitionSetOrigin.repoAddress,
  ...
  return (
    <tr>
      ...
      <td>
        {backfill.partitionSet && backfill.partitionSetOrigin ? (
          <Group direction={'column'} spacing={8}>
            ...
            <span style={{color: Colors.DARK_GRAY3, fontSize: 12}}>
              {backfill.partitionSetOrigin.repoAddress.name}@
              {backfill.partitionSetOrigin.repoAddress.location}
            </span>
              ...
              <span style={{fontSize: '13px'}}>
                <PipelineAndMode
                  ...
                  pipelineHref={workspacePathFromAddress(
                    backfill.partitionSetOrigin.repoAddress,
                    `/pipelines/${backfill.partitionSet.pipelineName}/`,
                  )}
        ...
        ) : backfill.partitionSetOrigin ? (
          backfill.partitionSetOrigin.partitionSetName
        ) : (
          '-'
        )}
      </td>
