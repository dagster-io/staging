...
const LaunchBackfillButton: React.FunctionComponent<{
  ...
  fromFailure?: boolean;
}> = ({ partitionSetName, partitionNames, reexecutionSteps, fromFailure }) => {
  const repositorySelector = useRepositorySelector();
  const [launchBackfill] = useMutation(LAUNCH_PARTITION_BACKFILL_MUTATION);
  const onLaunch = async () => {
    const result = await launchBackfill({
      variables: {
        backfillParams: {
          selector: {
            partitionSetName,
  ...
  return (
    <>
      <CollapsibleMenuItem isOpen={isOpen} onClick={onToggle}>
        <Icon icon={isOpen ? "chevron-right" : "chevron-down"} />
...
const FullPartitionsSelect: React.FunctionComponent<{
  ...
  partitionSetName: string;
  pipelineName: string;
}> = ({ partitionNames, partitionSetName, pipelineName }) => {
  ...
  return (
    <div>
      ...
      <LaunchBackfillButton partitionNames={selected} partitionSetName={partitionSetName} />
    </div>
  );
};
...
const FailedPartitionsSelect: React.FunctionComponent<{
  ...
  pipelineName: string;
  partitionSetName: string;
}> = ({ partitionNames, pipelineName, partitionSetName }) => {
  const [selected, setSelected] = React.useState<string[]>([]);
  const repositorySelector = useRepositorySelector();
