...
interface RepositoryLocationStateObserverProps {
  client: ApolloClient<any>;
  onReload: () => void;
}

export const RepositoryLocationStateObserver = ({client}: RepositoryLocationStateObserverProps) => {
  ...
  const errorLocationsFromQuery = locations
  ...
  const filteredErroredLocations = erroredLocations.filter(
    (locationName) => !errorLocationsFromQuery.includes(locationName),
  );

  const totalMessages = updatedLocations.length + filteredErroredLocations.length;
      ...
      {updatedLocations.length > 0 ? (
        <Group padding={{vertical: 8, horizontal: 12}} direction="horizontal" spacing={8}>
          ...
          <Caption color={Colors.DARK_GRAY3}>
            ...
            <SmallButtonLink onClick={refetchData}>Update data</SmallButtonLink>
          </Caption>
        </Group>
      ) : null}
