...
const ExecutionSessionContainer: React.FC<IExecutionSessionContainerProps> = (props) => {
  ...
  const initialDataForMode = React.useMemo(() => {
    const presetsForMode = pipeline.presets.filter((preset) => preset.mode === pipelineMode);
    const prepopulateConfig = flagPipelineModeTuples && presetsForMode.length === 1;
    return prepopulateConfig ? {runConfigYaml: presetsForMode[0].runConfigYaml} : {};
  }, [flagPipelineModeTuples, pipelineMode, pipeline.presets]);
