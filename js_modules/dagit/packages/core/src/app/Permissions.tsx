...
export type PermissionSet = ReturnType<typeof extractPermissions>;

export const DISABLED_MESSAGE = 'Disabled by your administrator';

export const PermissionsContext = React.createContext<PermissionSet>({} as PermissionSet);

export const PermissionsProvider: React.FC = (props) => {
  const {data} = useQuery<PermissionsQuery>(PERMISSIONS_QUERY);
  const value = React.useMemo(() => extractPermissions(data?.permissions || []), [data]);
