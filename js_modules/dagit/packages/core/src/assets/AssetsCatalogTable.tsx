...
export const AssetsCatalogTable: React.FunctionComponent<{
  ...
  const setIsFlattened = (flat: boolean) => setView(flat ? 'flat' : 'directory');
...
const AssetEntryRow: React.FunctionComponent<{
  ...
  ({
    ...
    const linkUrl = isAssetEntry
      ? `/instance/assets/${fullPath.map(encodeURIComponent).join('/')}`
      : `/instance/assets/${fullPath.map(encodeURIComponent).join('/')}?view=${view}`;
    return (
      <tr>
