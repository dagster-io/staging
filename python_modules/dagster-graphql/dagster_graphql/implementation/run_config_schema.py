...
def resolve_run_config_schema_or_error(graphene_info, selector, mode):
    from ..schema.errors import ModeNotFoundError
    from ..schema.run_config import RunConfigSchema
    ...
    if not external_pipeline.has_mode(mode):
        raise UserFacingGraphQLError(ModeNotFoundError(mode=mode, selector=selector))

    return RunConfigSchema(represented_pipeline=external_pipeline, mode=mode,)
...
def resolve_is_run_config_valid(graphene_info, represented_pipeline, mode, run_config):
    ...
    if not mode_def_snap.root_config_key:
        ...
        return PipelineConfigValidationValid(represented_pipeline.name)
