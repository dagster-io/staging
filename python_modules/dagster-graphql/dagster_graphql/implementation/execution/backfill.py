...
def create_and_launch_partition_backfill(graphene_info, backfill_params):
    ...
    backfill = PartitionBackfill(
        ...
        status=BackfillJobStatus.REQUESTED,
    ...
    if not graphene_info.context.instance.has_backfill_table() or backfill_params.get(
        "forceSynchronousSubmission"
    ):
        submitted_run_ids = submit_backfill_runs(graphene_info.context.instance, backfill)
...
def get_backfill(graphene_info, backfill_id):
    ...
    if graphene_info.context.instance.has_backfill_table():
