...
def create_and_launch_partition_backfill(graphene_info, backfill_params):
    from ...schema.backfill import PartitionBackfillSuccess
    from ...schema.errors import PartitionSetNotFoundError, PythonError
    ...
    if not matches:
        return PartitionSetNotFoundError(partition_set_name)
    ...
    if isinstance(result, ExternalPartitionExecutionErrorData):
        return PythonError(result.error)
    ...
    return PartitionBackfillSuccess(backfill_id=backfill_id, launched_run_ids=launched_run_ids)
