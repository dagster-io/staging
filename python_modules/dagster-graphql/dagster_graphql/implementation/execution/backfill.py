...
def create_and_launch_partition_backfill(graphene_info, backfill_params):
    ...
    for run_data in result.run_data:
        execution_params = _execution_params_for_backfill(
    ...
    return graphene_info.schema.type_named('PartitionBackfillSuccess')(
        backfill_id=result.backfill_id, launched_run_ids=launched_run_ids
    )


def _execution_params_for_backfill(
    instance, run_data, backfill_params, pipeline_selector, external_partition_set
):
    check.inst_param(instance, 'instance', DagsterInstance)
    check.inst_param(run_data, 'run_data', ExternalPartitionBackfillRunData)
    ...
    last_run = _fetch_last_run(instance, external_partition_set, run_data.name)
    ...
    if backfill_params.get('fromFailure'):
        ...
        return ExecutionParams(
            ...
            execution_metadata=ExecutionMetadata(
                run_id=None,
                tags=merge_dicts(run_data.tags, {'dagster/is_resume_retry': 'true'}),
