...
def _force_mark_as_failed(graphene_info, run):
    ...
    return graphene_info.schema.type_named("TerminatePipelineExecutionSuccess")(
        graphene_info.schema.type_named("PipelineRun")(failed_run)
    )


@capture_dauphin_error
def terminate_pipeline_execution(graphene_info, run_id, force_mark_as_failed):
    ...
    if run.status != PipelineRunStatus.STARTED and run.status != PipelineRunStatus.QUEUED:
        if not run.is_finished and force_mark_as_failed:
            return _force_mark_as_failed(graphene_info, run)
    ...
    if (
        ...
        and graphene_info.context.instance.run_coordinator.cancel(run_id)
    ):
