...
def get_pipeline_snapshot_or_error_from_pipeline_selector(graphene_info, pipeline_selector):
    from ..schema.pipelines.pipeline import PipelineSnapshot

    check.inst_param(pipeline_selector, "pipeline_selector", PipelineSelector)
    return PipelineSnapshot(get_full_external_pipeline_or_raise(graphene_info, pipeline_selector))
...
def _get_pipeline_snapshot_from_instance(instance, snapshot_id):
    from ..schema.pipelines.pipeline import PipelineSnapshot
    from dagster_graphql.schema.errors import PipelineSnapshotNotFoundError

    if not instance.has_pipeline_snapshot(snapshot_id):
        raise UserFacingGraphQLError(PipelineSnapshotNotFoundError(snapshot_id))
    ...
    return PipelineSnapshot(historical_pipeline)
...
def get_pipeline_reference_or_raise(graphene_info, pipeline_run):
    ...
    InvalidSubsetError."""
    from ..schema.pipelines.pipeline_ref import UnknownPipeline
    ...
    if pipeline_run.pipeline_snapshot_id is None:
        return UnknownPipeline(pipeline_run.pipeline_name, solid_selection)
...
def get_pipeline_from_selector(graphene_info, selector):
    ...
    return Pipeline(get_external_pipeline_or_raise(graphene_info, selector))
