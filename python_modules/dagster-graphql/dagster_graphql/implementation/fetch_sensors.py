...
def get_sensors_or_error(graphene_info, repository_selector):
    ...
    return GrapheneSensors(
        results=[GrapheneSensor(graphene_info, sensor) for sensor in repository.get_external_sensors()]
    )
...
def get_unloadable_sensor_states_or_error(graphene_info):
    ...
    return GrapheneJobStates(results=[GrapheneJobState(job_state=job_state) for job_state in unloadable_states])
