...
def get_sensors_or_error(graphene_info, repository_selector):
    from ..schema.sensors import Sensor, Sensors
    ...
    return Sensors(
        results=[Sensor(graphene_info, sensor) for sensor in repository.get_external_sensors()]
    )


@capture_error
def get_sensor_or_error(graphene_info, selector):
    from ..schema.errors import SensorNotFoundError
    from ..schema.sensors import Sensor
    ...
    if not external_job or not isinstance(external_job, ExternalSensor):
        raise UserFacingGraphQLError(SensorNotFoundError(selector.sensor_name))

    return Sensor(graphene_info, external_job)
...
def start_sensor(graphene_info, sensor_selector):
    ...
    if not isinstance(external_sensor, ExternalSensor):
        raise UserFacingGraphQLError(SensorNotFoundError(sensor_selector.sensor_name))
    graphene_info.context.instance.start_sensor(external_sensor)
    return Sensor(graphene_info, external_sensor)


@capture_error
def stop_sensor(graphene_info, job_origin_id):
    from ..schema.sensors import StopSensorMutationResult
    ...
    if not job_state:
        return StopSensorMutationResult(job_state=None)

    instance.stop_sensor(job_origin_id)
    return StopSensorMutationResult(job_state=job_state.with_status(JobStatus.STOPPED))
...
def get_unloadable_sensor_states_or_error(graphene_info):
    ...
    return JobStates(results=[JobState(job_state=job_state) for job_state in unloadable_states])
...
def get_sensors_for_pipeline(graphene_info, pipeline_selector):
    ...
    return [
        Sensor(graphene_info, external_sensor)
