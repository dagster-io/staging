...
class DagsterGraphQLContext:
    ...
    def reload_repository_location(self, name):
        # If this is called in the context of the Dagit webserver, the current GraphQL context is
        # a copy of the context made for the current request. To reload the repository location,
        # it is necessary to reload the root context and workspace.

        root_context = self if self.is_root_context else self._root_context
