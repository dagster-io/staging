...
def get_full_external_pipeline_or_raise(graphene_info, selector):
    from ..schema.errors import PipelineNotFoundError

    check.inst_param(graphene_info, "graphene_info", ResolveInfo)
    check.inst_param(selector, "selector", PipelineSelector)

    if not graphene_info.context.has_external_pipeline(selector):
        raise UserFacingGraphQLError(PipelineNotFoundError(selector=selector))
...
def ensure_valid_step_keys(full_external_execution_plan, step_keys):
    from ..schema.errors import InvalidStepError
    ...
    for step_key in step_keys:
        if not full_external_execution_plan.has_step(step_key):
            raise UserFacingGraphQLError(InvalidStepError(invalid_step_key=step_key))
...
def fetch_repositories(graphene_info):
    from ..schema.external import Repository, RepositoryConnection

    check.inst_param(graphene_info, "graphene_info", ResolveInfo)
    return RepositoryConnection(
        nodes=[
            Repository(repository=repository, repository_location=location)
...
def fetch_repository(graphene_info, repository_selector):
    from ..schema.errors import RepositoryNotFoundError
    from ..schema.external import Repository
...
def fetch_repository_locations(graphene_info):
    ...
    for location_name in graphene_info.context.repository_location_names:
        node = (
            RepositoryLocation(graphene_info.context.get_repository_location(location_name))
            if graphene_info.context.has_repository_location(location_name)
            else RepositoryLocationLoadFailure(
                location_name, graphene_info.context.get_repository_location_error(location_name),
            )
        )
        nodes.append(node)

    return RepositoryLocationConnection(nodes=nodes)
