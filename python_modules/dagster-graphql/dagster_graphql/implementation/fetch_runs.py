...
def get_validated_config(pipeline_def, run_config, mode):
    from ..schema.pipelines.config import PipelineConfigValidationInvalid
    ...
    if not validated_config.success:
        raise UserFacingGraphQLError(
            PipelineConfigValidationInvalid.for_validation_errors(
...
def get_run_by_id(graphene_info, run_id):
    from ..schema.errors import PipelineRunNotFoundError
    from ..schema.pipelines.pipeline import PipelineRun

    instance = graphene_info.context.instance
    run = instance.get_run_by_id(run_id)
    if not run:
        return PipelineRunNotFoundError(run_id)
    else:
        return PipelineRun(run)


def get_run_tags(graphene_info):
    from ..schema.tags import PipelineTagAndValues

    instance = graphene_info.context.instance
    return [
        PipelineTagAndValues(key=key, values=values)
...
def get_run_group(graphene_info, run_id):
    from ..schema.errors import RunGroupNotFoundError
    from ..schema.pipelines.pipeline import PipelineRun
    from ..schema.runs import RunGroup

    instance = graphene_info.context.instance
    result = instance.get_run_group(run_id)

    if result is None:
        return RunGroupNotFoundError(run_id)
    else:
        root_run_id, run_group = result
        return RunGroup(root_run_id=root_run_id, runs=[PipelineRun(run) for run in run_group])


def get_runs(graphene_info, filters, cursor=None, limit=None):
    from ..schema.pipelines.pipeline import PipelineRun

    check.opt_inst_param(filters, "filters", DagsterPipelineRunsFilter)
...
def get_run_groups(graphene_info, filters=None, cursor=None, limit=None):
    from ..schema.inputs import PipelineRunsFilter
    from ..schema.pipelines.pipeline import PipelineRun
    from ..schema.runs import RunGroup
    ...
    for root_run_id in run_groups:
        run_groups[root_run_id]["runs"] = [
            PipelineRun(run) for run in run_groups[root_run_id]["runs"]
        ]

    return [
        RunGroup(root_run_id=root_run_id, runs=run_group["runs"])
...
def validate_pipeline_config(graphene_info, selector, run_config, mode):
    from ..schema.pipelines.config import PipelineConfigValidationValid
    ...
    return PipelineConfigValidationValid(pipeline_name=external_pipeline.name)


@capture_error
def get_execution_plan(graphene_info, selector, run_config, mode):
    from ..schema.execution import ExecutionPlan
...
def get_stats(graphene_info, run_id):
    from ..schema.pipelines.pipeline_run_stats import PipelineRunStatsSnapshot

    stats = graphene_info.context.instance.get_run_stats(run_id)
    stats.id = "stats-{run_id}"
    return PipelineRunStatsSnapshot(stats)


def get_step_stats(graphene_info, run_id, step_keys=None):
    from ..schema.logs.events import PipelineRunStepStats

    step_stats = graphene_info.context.instance.get_run_step_stats(run_id, step_keys)
    return [PipelineRunStepStats(stats) for stats in step_stats]
