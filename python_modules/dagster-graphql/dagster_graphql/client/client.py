...
class DagsterGraphQLClient:
    ...
    def _get_repo_locations_and_names_with_pipeline(
        ...
        if repo_connection_status == "RepositoryConnection":
            valid_nodes: List[RepositoryNodeTuple] = [
                RepositoryNodeTuple.from_node(node) for node in query_res["nodes"]
            ]
            return list(
                filter(
                    lambda repo_node_tup: repo_node_tup.pipeline_name == pipeline_name, valid_nodes
                )
            )
        else:
    ...
    def submit_pipeline_execution(
        ...
        if not repository_location_name or not repository_name:
            repo_node_lst = self._get_repo_locations_and_names_with_pipeline(pipeline_name)
            if len(repo_node_lst) == 1:
                repo_node = repo_node_lst[0]
                repository_location_name = repo_node.repository_location_name
                repository_name = repo_node.repository_name
            else:
                raise DagsterGraphQLClientError(
                    "Must specify repository_location and repository_name"
                    f" since there are multiple pipelines with the name {pipeline_name}."
                    f"\n\tchoose one of: {repo_node_lst}"
                )
