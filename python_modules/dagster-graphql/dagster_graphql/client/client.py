...
class DagsterGraphQLClient:
    ...
    Args:
        hostname (Optional[str]): Hostname for the Dagster GraphQL API, like `localhost` or
            `dagit.dagster.YOUR_ORG_HERE`. Defaults to None.
        port_number (Optional[int]): Optional port number to connect to on the host.
            Defaults to None.
        transport (Optional[Transport]): A custom transport to use to connect to the GraphQL API
            with (e.g. for custom auth). Defaults to None.
