...
from ..errors import DagsterTypeNotFoundError, PipelineRunNotFoundError, PythonError
...
class Asset(graphene.ObjectType):
    ...
    runs = graphene.Field(
        non_null_list(lambda: PipelineRun), cursor=graphene.String(), limit=graphene.Int(),
    )
    ...
    def resolve_runs(self, graphene_info, **kwargs):
        ...
        return [
            ...
            for r in graphene_info.context.instance.get_runs(
                filters=DagsterPipelineRunsFilter(run_ids=run_ids)
            )
        ]
...
class PipelineRun(graphene.ObjectType):
    ...
    def __init__(self, pipeline_run):
        ...
        self._pipeline_run = check.inst_param(pipeline_run, "pipeline_run", DagsterPipelineRun)
...
class IPipelineSnapshotMixin:
    ...
    def resolve_runs(self, graphene_info, **kwargs):
        runs_filter = DagsterPipelineRunsFilter(pipeline_name=self.get_represented_pipeline().name)
