...
class DauphinDaemonType(dauphin.Enum):
    ...
    DAGSTER_DAEMON_SCHEDULER = "DAGSTER_DAEMON_SCHEDULER"
    SENSOR_DAEMON = "SENSOR_DAEMON"
    QUEUED_RUN_COORDINATOR_DAEMON = "QUEUED_RUN_COORDINATOR_DAEMON"
...
class DauphinDaemonStatus(dauphin.ObjectType):
    ...
    def __init__(self, daemon_status):
        ...
        daemon_type = {
            "DagsterDaemonScheduler": DauphinDaemonType.DAGSTER_DAEMON_SCHEDULER,
            "SensorDaemon": DauphinDaemonType.SENSOR_DAEMON,
            "QueuedRunCoordinatorDaemon": DauphinDaemonType.QUEUED_RUN_COORDINATOR_DAEMON,
        }[daemon_status.daemon_type]
...
class DauphinDaemonHealth(dauphin.ObjectType):
    ...
    def __init__(self, instance):
        ...
        self._daemon_statuses = {
            DauphinDaemonType.DAGSTER_DAEMON_SCHEDULER.value: DauphinDaemonStatus(  # pylint: disable=no-member
                get_daemon_status(instance, "DagsterDaemonScheduler")
            ),
            DauphinDaemonType.SENSOR_DAEMON.value: DauphinDaemonStatus(  # pylint: disable=no-member
                get_daemon_status(instance, "SensorDaemon")
            ),
            DauphinDaemonType.QUEUED_RUN_COORDINATOR_DAEMON.value: DauphinDaemonStatus(  # pylint: disable=no-member
                get_daemon_status(instance, "QueuedRunCoordinatorDaemon")
            ),
        }

    def resolve_daemonStatus(self, _graphene_info, daemon_type):
        check.str_param(daemon_type, "daemon_type")  # DauphinDaemonType
        return self._daemon_statuses[daemon_type]

    def resolve_allDaemonStatuses(self, _graphene_info):
        return list(self._daemon_statuses.values())
