...
class DauphinExecutionMetadata(dauphin.InputObjectType):
    ...
    rootRunId = dauphin.String(
        description="The ID of the run at the root of the run group. All partial / full re-executions should use the first run as the rootRunID so they are grouped together."
    )
    parentRunId = dauphin.String(
        description="The ID of the run serving as the parent within the run group. For the first re-execution, this will be the same as the `rootRunId`. For subsequent runs, the root or a previous re-execution could be the parent run."
    )
...
class DauphinExecutionParams(dauphin.InputObjectType):
    ...
    selector = dauphin.NonNull(
        "PipelineSelector",
        description="Defines the pipeline and solid subset that should be executed. All subsequent executions in the same run group (for example, a single-step re-execution) are scoped to the original run's pipeline selector and solid subset.",
    )
    ...
    stepKeys = dauphin.Field(
        ...
        description="Defines step keys to execute within the execution plan defined by the pipeline `selector`. To execute the entire execution plan, you can omit this parameter, provide an empty array, or provide every step name.",
    )
