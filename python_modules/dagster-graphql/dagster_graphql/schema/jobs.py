...
GrapheneJobType = graphene.Enum.from_enum(JobType)
GrapheneJobStatus = graphene.Enum.from_enum(JobStatus)
GrapheneJobTickStatus = graphene.Enum.from_enum(JobTickStatus)
...
class GrapheneRunRequest(graphene.ObjectType):
    ...
    def __init__(self, run_request):
        ...
        self._run_request = check.inst_param(run_request, "run_request", GrapheneRunRequest)
...
class GrapheneJobState(graphene.ObjectType):
    ...
    ticks = graphene.Field(non_null_list(GrapheneJobTick), limit=graphene.Int())
    ...
    def resolve_ticks(self, graphene_info, limit=None):
        ...
        return [GrapheneJobTick(graphene_info, tick) for tick in ticks]
