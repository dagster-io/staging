...
class DauphinFutureJobTick(dauphin.ObjectType):
    ...
    def _evaluate(self, graphene_info):
        ...
        self._schedule_data = repository_location.get_external_schedule_execution_data(
            instance=graphene_info.context.instance,
    ...
    def resolve_skipReason(self, graphene_info):
        if not self._evaluated:
            self._evaluate(graphene_info)

        if self._schedule_data is None:
            return None

        return self._schedule_data.skip_message
    ...
    def resolve_runRequests(self, graphene_info):
        ...
        return [
            ...
            for run_request in self._schedule_data.run_requests
        ]
