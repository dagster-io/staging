...
class GrapheneBackfillJobStatus(graphene.Enum):
    REQUESTED = "REQUESTED"
    COMPLETED = "COMPLETED"
    FAILED = "FAILED"
...
class GraphenePartitionBackfill(graphene.ObjectType):
    ...
    status = graphene.NonNull(GrapheneBackfillJobStatus)
    isPersisted = graphene.NonNull(graphene.Boolean)
    requested = graphene.Int()
    total = graphene.Int()
    ...
    def __init__(self, backfill_id, backfill_job=None):
        ...
        if backfill_job:
            ...
            if backfill_job.status == BackfillJobStatus.COMPLETED:
                requested = total
            elif backfill_job.checkpoint:
                requested = backfill_job.partition_names.index(backfill_job.checkpoint) + 1
            else:
                requested = 0
