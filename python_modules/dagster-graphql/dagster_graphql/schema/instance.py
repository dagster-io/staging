...
from dagster.daemon.types import DaemonStatus as DagsterDaemonStatus
from dagster.daemon.types import DaemonType as DagsterDaemonType
...
DaemonType = graphene.Enum.from_enum(DagsterDaemonType)
...
class DaemonStatus(graphene.ObjectType):
    ...
    def __init__(self, daemon_status):
        check.inst_param(daemon_status, "daemon_status", DagsterDaemonStatus)
...
class DaemonHealth(graphene.ObjectType):
    ...
    def __init__(self, instance):
        ...
        self._daemon_statuses = {
            DagsterDaemonType.SCHEDULER.value: get_daemon_status(  # pylint: disable=no-member
                instance, DagsterDaemonType.SCHEDULER
            ),
            DagsterDaemonType.SENSOR.value: get_daemon_status(  # pylint: disable=no-member
                instance, DagsterDaemonType.SENSOR
            ),
            DagsterDaemonType.QUEUED_RUN_COORDINATOR.value: get_daemon_status(  # pylint: disable=no-member
                instance, DagsterDaemonType.QUEUED_RUN_COORDINATOR
            ),
        }
