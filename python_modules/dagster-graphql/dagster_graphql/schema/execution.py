...
from .pipelines.errors import PipelineNotFoundError
from .util import non_null_list


class ExecutionStepOutput(graphene.ObjectType):
    name = graphene.NonNull(graphene.String)
    type = graphene.Field(graphene.NonNull(DagsterType))
...
class ExecutionStepInput(graphene.ObjectType):
    name = graphene.NonNull(graphene.String)
    type = graphene.Field(graphene.NonNull(DagsterType))
    dependsOn = non_null_list(lambda: ExecutionStep)
...
class StepKind(graphene.Enum):
    COMPUTE = "COMPUTE"
    UNRESOLVED = "UNRESOLVED"

    @property
    def description(self):
        if self == StepKind.COMPUTE:
            return "This is a user-defined computation step"
        if self == StepKind.UNRESOLVED:
...
class ExecutionStep(graphene.ObjectType):
    ...
    inputs = non_null_list(ExecutionStepInput)
    outputs = non_null_list(ExecutionStepOutput)
    solidHandleID = graphene.NonNull(graphene.String)
    kind = graphene.NonNull(StepKind)
    ...
    def resolve_metadata(self, graphene_info):
        return [
            MetadataItemDefinition(key=mdi.key, value=mdi.value)
...
class ExecutionPlan(graphene.ObjectType):
    steps = non_null_list(ExecutionStep)
