import graphene
from dagster.core.execution.retries import Retries as DagsterRetries
...
class PipelineRunsFilter(graphene.InputObjectType):
    ...
    def to_selector(self):
        ...
        if self.statuses:
            statuses = [
                DagsterPipelineRunStatus[status]
                for status in self.statuses  # pylint: disable=not-an-iterable
            ]
        else:
            statuses = None

        return DagsterPipelineRunsFilter(
            run_ids=self.runIds,
...
def create_retries_params(retries_config):
    return DagsterRetries.from_graphql_input(retries_config)
...
types = [
    ...
    MarshalledInput,
    MarshalledOutput,
    PartitionBackfillParams,
    PartitionSetSelector,
    PipelineRunsFilter,
    PipelineSelector,
    RepositorySelector,
    Retries,
    RetriesPreviousAttempts,
    RetriesPreviousAttempts,
    ScheduleSelector,
    SensorSelector,
    StepExecution,
