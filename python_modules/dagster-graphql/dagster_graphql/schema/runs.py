...
from .errors import InvalidPipelineRunsFilterError, PythonError, RunGroupNotFoundError
...
class LaunchPipelineRunSuccess(graphene.ObjectType):
    run = graphene.Field(graphene.NonNull("dagster_graphql.schema.pipelines.pipeline.PipelineRun"))


class RunGroup(graphene.ObjectType):
    rootRunId = graphene.NonNull(graphene.String)
    runs = graphene.List("dagster_graphql.schema.pipelines.pipeline.PipelineRun")
...
class PipelineRuns(graphene.ObjectType):
    ...
    results = non_null_list("dagster_graphql.schema.pipelines.pipeline.PipelineRun")
    count = graphene.Int()
...
types = [
    LaunchPipelineExecutionResult,
    LaunchPipelineReexecutionResult,
