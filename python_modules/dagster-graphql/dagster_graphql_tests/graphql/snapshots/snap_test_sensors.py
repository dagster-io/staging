# -*- coding: utf-8 -*-
# snapshottest: v1 - https://goo.gl/zC4yUc
from __future__ import unicode_literals

from snapshottest import Snapshot

snapshots = Snapshot()

snapshots['TestSensors.test_get_sensor[sqlite_with_sync_run_launcher_in_process_env] 1'] = {
    '__typename': 'Sensor',
    'id': 'always_no_config_sensor:no_config_pipeline',
    'mode': 'default',
    'name': 'always_no_config_sensor',
    'pipelineName': 'no_config_pipeline',
    'runs': [
    ],
    'runsCount': 0,
    'solidSelection': None,
    'status': 'STOPPED',
    'ticks': [
    ]
}

snapshots['TestSensors.test_get_sensors[readonly_in_memory_instance_in_process_env] 1'] = [
    {
        'id': 'always_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'always_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'multi_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'multi_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'never_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'never_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'once_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'once_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    }
]

snapshots['TestSensors.test_get_sensors[readonly_in_memory_instance_managed_grpc_env] 1'] = [
    {
        'id': 'always_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'always_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'multi_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'multi_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'never_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'never_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'once_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'once_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    }
]

snapshots['TestSensors.test_get_sensors[readonly_in_memory_instance_multi_location] 1'] = [
    {
        'id': 'always_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'always_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'multi_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'multi_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'never_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'never_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'once_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'once_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    }
]

snapshots['TestSensors.test_get_sensors[readonly_postgres_instance_in_process_env] 1'] = [
    {
        'id': 'always_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'always_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'multi_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'multi_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'never_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'never_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'once_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'once_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    }
]

snapshots['TestSensors.test_get_sensors[readonly_postgres_instance_managed_grpc_env] 1'] = [
    {
        'id': 'always_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'always_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'multi_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'multi_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'never_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'never_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'once_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'once_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    }
]

snapshots['TestSensors.test_get_sensors[readonly_postgres_instance_multi_location] 1'] = [
    {
        'id': 'always_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'always_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'multi_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'multi_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'never_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'never_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'once_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'once_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    }
]

snapshots['TestSensors.test_get_sensors[readonly_sqlite_instance_deployed_grpc_env] 1'] = [
    {
        'id': 'always_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'always_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'multi_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'multi_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'never_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'never_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'once_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'once_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    }
]

snapshots['TestSensors.test_get_sensors[readonly_sqlite_instance_in_process_env] 1'] = [
    {
        'id': 'always_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'always_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'multi_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'multi_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'never_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'never_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'once_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'once_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    }
]

snapshots['TestSensors.test_get_sensors[readonly_sqlite_instance_managed_grpc_env] 1'] = [
    {
        'id': 'always_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'always_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'multi_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'multi_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'never_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'never_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'once_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'once_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    }
]

snapshots['TestSensors.test_get_sensors[readonly_sqlite_instance_multi_location] 1'] = [
    {
        'id': 'always_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'always_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'multi_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'multi_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'never_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'never_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'once_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'once_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    }
]

snapshots['TestSensors.test_get_sensors[sqlite_with_default_run_launcher_in_process_env] 1'] = [
    {
        'id': 'always_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'always_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'multi_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'multi_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'never_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'never_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'once_no_config:no_config_pipeline',
        'mode': 'default',
        'name': 'once_no_config',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    }
]

snapshots['TestSensors.test_get_sensors[sqlite_with_sync_run_launcher_in_process_env] 1'] = [
    {
        'id': 'always_no_config_sensor:no_config_pipeline',
        'mode': 'default',
        'name': 'always_no_config_sensor',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'multi_no_config_sensor:no_config_pipeline',
        'mode': 'default',
        'name': 'multi_no_config_sensor',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'never_no_config_sensor:no_config_pipeline',
        'mode': 'default',
        'name': 'never_no_config_sensor',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    },
    {
        'id': 'once_no_config_sensor:no_config_pipeline',
        'mode': 'default',
        'name': 'once_no_config_sensor',
        'pipelineName': 'no_config_pipeline',
        'runs': [
        ],
        'runsCount': 0,
        'solidSelection': None,
        'status': 'STOPPED',
        'ticks': [
        ]
    }
]
