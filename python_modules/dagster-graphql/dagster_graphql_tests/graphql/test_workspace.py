...
class TestLoadWorkspace(
    ...
    def test_load_workspace(self, graphql_context):
        ...
        with mock.patch(
            ...
            assert result.data["workspaceOrError"]["__typename"] == "WorkspaceConnection", str(
                result.data
            )
            ...
            success_nodes = [node for node in nodes if node.get("location")]
            assert len(success_nodes) == 2

            for success_node in success_nodes:
                assert not success_node.get("loadError")
            ...
            assert "No such file or directory" in failure_node["loadError"]["message"]
