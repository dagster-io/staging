...
class TestRunVariantTermination(
    ...
    def test_terminate_failed(self, graphql_context):
        ...
        with safe_tempfile_path() as path:
            ...
            result = execute_dagster_graphql(
                graphql_context,
                RUN_CANCELLATION_QUERY,
                variables={"runId": run_id, "terminatePolicy": "MARK_AS_FAILED_IMMEDIATELY"},
            )
    ...
    def test_run_finished(self, graphql_context):
        ...
        assert (
            "has status SUCCESS and has already finished."
        ...
        result = execute_dagster_graphql(
            ...
            variables={
                ...
                "terminatePolicy": "MARK_AS_FAILED_IMMEDIATELY",
            },
        )
