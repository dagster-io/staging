...
class PickledObjectS3AssetStore(AssetStore):
    def __init__(
        self, s3_bucket, s3_session=None, s3_prefix="dagster",
    ):
    ...
    def set_asset(self, context, obj):
        ...
        s3_file_handle = S3FileHandle(self.bucket, key)
        return AssetMaterialization(
            asset_key=AssetKey([context.pipeline_name, context.step_key, context.output_name]),
            metadata_entries=[
                EventMetadataEntry.path(s3_file_handle.s3_path, label=self._last_key(key))
            ],
        )
...
def s3_asset_store(init_context):
    ...
    .. code-block:: python
        ...
        pipeline_def = PipelineDefinition(
            mode_defs=[
                ModeDefinition(
                    resource_defs={'s3': s3_asset_store, ...},
                ), ...
            ], ...
        )
