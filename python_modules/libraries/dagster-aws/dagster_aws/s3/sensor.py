...
@experimental
def s3_sensor(
    pipeline_name, bucket, prefix=None, solid_selection=None, mode=None, default_interval=300
):
    ...
    def inner(fn):
        ...
        def _wrapped_fn(context):
            ...
            since = (
                utc_datetime_from_timestamp(context.last_evaluation_time)
                if context.last_evaluation_time
                else utc_datetime_from_timestamp(datetime.utcnow().timestamp() - default_interval)
            )
