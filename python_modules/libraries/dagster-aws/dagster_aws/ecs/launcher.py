...
class EcsRunLauncher(RunLauncher, ConfigurableClass):
    ...
    def launch_run(self, run, external_pipeline):
        ...
        # The current process process might not be running in a container that
        # has the pipeline's code installed. Inheret most of the processes's
        # container definition (things like environment, dependencies, etc.)
        # but replace the image with the pipeline's origin's image and give it
        # a new name.
