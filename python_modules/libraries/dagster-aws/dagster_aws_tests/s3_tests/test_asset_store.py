...
def define_inty_pipeline():
    ...
    @pipeline(mode_defs=[ModeDefinition(resource_defs={"asset_store": s3_asset_store},)])
...
def test_s3_asset_store_execution(bucket):
    pipeline_def = define_inty_pipeline()

    run_config = {"resources": {"asset_store": {"config": {"s3_bucket": bucket.name}}}}
    ...
    asset_store = PickledObjectS3AssetStore(bucket.name)
