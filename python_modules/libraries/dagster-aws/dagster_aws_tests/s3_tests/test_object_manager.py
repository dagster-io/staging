...
def test_s3_object_manager_execution(mock_s3_bucket):
    ...
    asset_store = PickledObjectS3ObjectManager(mock_s3_bucket.name, s3_prefix="dagster")
    ...
    assert asset_store.load_input(context) == 1
    ...
    assert asset_store.load_input(context) == 2
