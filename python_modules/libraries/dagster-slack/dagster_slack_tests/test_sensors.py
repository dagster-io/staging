...
def test_slack_pipeline_failure_sensor_auth_error():
    with environ({"SLACK_TOKEN": None}):

        with pytest.raises(
            DagsterInvalidDefinitionError,
            match='Env variable \\"SLACK_TOKEN\\" is unset.',
        ):
            make_slack_on_pipeline_failure_sensor(channel="#foo")
