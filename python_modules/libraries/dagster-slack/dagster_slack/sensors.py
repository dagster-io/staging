...
def make_slack_on_pipeline_failure_sensor(
    ...
    Args:
        ...
        extra_message_fn (Optional(Callable[[PipelineFailureSensorContext], str])): Function which
            takes in the ``PipelineFailureSensorContext`` outputs the message you want to send.
            Defaults to the slack block that contains error message, pipeline name, and run ID.
