...
class DagsterKubernetesClient:
    ...
    def wait_for_job_success(
        ...
        Raises:
            DagsterK8sError: Raised when wait_timeout is exceeded
            DagsterK8sStepFailureException: Raised when K8s Job has status failed
            DagsterK8sPipelineStatusException: Raised when pipeline does not have STARTED status
        """
        check.str_param(job_name, "job_name")
        check.str_param(namespace, "namespace")
        ...
        while True:
            ...
            if status.failed and status.failed > 0:
                raise DagsterK8sStepFailureException(
                    "Encountered failed job pods with status {}".format(status)
                )
            ...
            if instance and run_id:
                ...
                if pipeline_run_status != PipelineRunStatus.STARTED:
                    raise DagsterK8sPipelineStatusException(
                        "Status is {}, not STARTED".format(pipeline_run_status)
                    )
    ...
    # Users usually should call retrieve_job_logs
