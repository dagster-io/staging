...
class K8sStepHandler(StepHandler):
    ...
    def __init__(
        ...
        self._load_incluster_config = load_incluster_config
    ...
    def _load_kube_config(self):
        if self._load_incluster_config:
            check.invariant(
                self._kubeconfig_file is None,
                "`kubeconfig_file` is set but `load_incluster_config` is True.",
            )
            kubernetes.config.load_incluster_config()
        else:
            check.opt_str_param(self._kubeconfig_file, "kubeconfig_file")
            kubernetes.config.load_kube_config(self._kubeconfig_file)
    ...
    def launch_step(self, step_handler_context: StepHandlerContext):
        ...
        step_handler_context.instance.handle_new_event(e)
        events.append(e)
    ...
    def check_step_health(self, step_handler_context: StepHandlerContext):
        ...
        if job.status.failed:
            ...
            step_handler_context.instance.handle_new_event(step_failure_event)
