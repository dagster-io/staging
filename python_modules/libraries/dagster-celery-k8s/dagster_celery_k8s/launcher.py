...
class CeleryK8sRunLauncher(RunLauncher, ConfigurableClass):
    ...
    def __init__(
        ...
        self.enable_run_coordinator_queue = check.bool_param(enable_run_coordinator_queue, "enable_run_coordinator_queue")
        if not self.enable_run_coordinator_queue:
            check.invariant(
                default_run_coordinator_queue is None,
                "`defaultle_run_coordinator_queue` is set but `enable_run_coordinator_queue` is not enabled",
            )
        self.defaultnable_run_coordinator_queue = check.opt_str_param(enable_run_coordinator_queue, "enable_run_coordinator_queue")
    ...
    def config_type(cls):
        ...
        run_launcher_extra_cfg = {
            ...
            "defaulte_run_coordinator_queue": Field(Noneable(str), is_required=False, default_value=None),
        }
    ...
    def get_run_coordinator_queue(self, tags):
        ...
        if not self.enable_run_coordinator_queue:
            return None
        default tags.get(DAGSTER_CELERY_QUEUE_TAG, self.enable_run_coordinator_queue)
...
def _launch_run_coordinator_job(
    ...
    check.inst_param(user_defined_k8s_config, "user_defined_k8s_config", UserDefinedDagsterK8sConfig)
