...
class CeleryK8sRunLauncher(RunLauncher, ConfigurableClass):
    ...
    Args:
        ...
        queue_runs: (Optional[bool]): (Experimental) Set this value to launch runs via a Celery
            queue, enabling limits on concurrent runs
        default_run_queue: (Optional[str]): (Experimental) The Celery queue for run coordinator
    ...
    def __init__(
        ...
        self.queue_runs = check.bool_param(queue_runs, "queue_runs")
        if not self.queue_runs:
            check.invariant(
                default_run_queue is None,
                "`default_run_queue` is set but `queue_runs` is not enabled",
            )
        self.default_run_queue = check.opt_str_param(default_run_queue, "default_run_queue")
    ...
    def config_type(cls):
        ...
        run_launcher_extra_cfg = {
            ...
            "queue_runs": Field(bool, is_required=False, default_value=False),
            "default_run_queue": Field(Noneable(str), is_required=False, default_value=None),
        }
    ...
    def get_run_coordinator_queue(self, tags):
        ...
        if not self.queue_runs:
            return None
        return tags.get(DAGSTER_CELERY_QUEUE_TAG, self.default_run_queue)
