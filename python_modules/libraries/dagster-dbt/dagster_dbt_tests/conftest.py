...
def dbt_seed(prepare_dbt_cli):  # pylint: disable=unused-argument, redefined-outer-name
    ...
    # print("env: {}".format(yaml.dump(dict(os.environ))))
    xx = subprocess.run(
        [DBT_EXECUTABLE, 'seed', '--profiles-dir', DBT_CONFIG_DIR], check=True, capture_output=True
    )
    print("subprocess happened! {} {}".format(kfd(xx.stdout), kfd(xx.stderr)))
...
def kill_all_subprocs():
    os.system(f'terminal-notifier -message "killing {len(all_subprocs)} procs!"')
