...
class DbtResult(namedtuple("_DbtResult", "results raw_run_results generated_at elapsed_time")):
    ...
    Attributes:
        ...
        raw_run_results (Dict[str, Any]): JSON data parsed from run_results.json.
    """

    def __new__(
        cls,
        results: List[NodeResult],
        raw_run_results: Dict[str, Any],
        generated_at: Optional[str] = None,
        elapsed_time: Optional[float] = None,
    ):
        return super().__new__(
            cls,
            results=check.list_param(results, "results", of_type=NodeResult),
            raw_run_results=check.opt_dict_param(raw_run_results, "raw_run_results", key_type=str),
            generated_at=check.opt_str_param(generated_at, "generated_at"),
            elapsed_time=check.opt_float_param(elapsed_time, "elapsed_time"),
        )

    @classmethod
    def from_run_results(cls, run_results: Dict[str, Any]) -> "DbtResult":
        ...
        check.dict_param(run_results, "run_results", key_type=str)

        generated_at = check.opt_str_elem(run_results, "generated_at")
        elapsed_time = check.float_elem(run_results, "elapsed_time")

        results = [
            NodeResult.from_dict(d) for d in check.is_list(run_results["results"], of_type=dict)
        ]

        return cls(
            results=results,
            raw_run_results=run_results,
