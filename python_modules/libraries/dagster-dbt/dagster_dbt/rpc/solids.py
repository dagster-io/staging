...
from .types import PolledRunResult
from .utils import log_rpc, raise_for_rpc_error


def _generate_materializations(prr: PolledRunResult) -> Iterator[AssetMaterialization]:
    """Yields ``AssetMaterializations`` for metadata in the dbt RPC ``PolledRunResult``."""
...
def _poll_rpc(
    ...
    polled_run_results = PolledRunResult.from_dict(resp.json().get("result"))
...
def unwrap_result(poll_rpc_generator) -> PolledRunResult:
    """A helper function that extracts the `PolledRunResult` value from a generator.
    ...
    if output is None:
        raise DagsterDbtUnexpectedRpcPollOutputError(
            description="poll_rpc yielded None as its last value. Expected value of type Output containing PolledRunResult.",
        )

    if not isinstance(output, Output):
        raise DagsterDbtUnexpectedRpcPollOutputError(
            description=f"poll_rpc yielded value of type {type(output)} as its last value. Expected value of type Output containing PolledRunResult.",
        )

    if not isinstance(output.value, PolledRunResult):
        raise DagsterDbtUnexpectedRpcPollOutputError(
            description=f"poll_rpc yielded Output containing {type(output.value)}. Expected PolledRunResult.",
        )
...
@solid(
    ...
    input_defs=[InputDefinition(name="start_after", dagster_type=Nothing)],
    output_defs=[OutputDefinition(name="result", dagster_type=PolledRunResult)],
    config_schema={
        "models": Field(
            config=Noneable(Array(String)),
            default_value=None,
...
def dbt_rpc_run_and_wait(context: SolidExecutionContext) -> PolledRunResult:
...
def dbt_rpc_test_and_wait(context: SolidExecutionContext) -> PolledRunResult:
...
def dbt_rpc_run_operation_and_wait(context: SolidExecutionContext) -> PolledRunResult:
...
def dbt_rpc_snapshot_and_wait(context: SolidExecutionContext) -> PolledRunResult:
...
def dbt_rpc_snapshot_freshness_and_wait(context: SolidExecutionContext) -> PolledRunResult:
