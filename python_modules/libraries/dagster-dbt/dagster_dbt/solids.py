...
def generate_materializations(
    # context, # TODO currently unused
    rpc_poll_result: DbtRpcPollResult,
...
def dbt_rpc_poll(
    ...
    if should_yield_materializations:
        for materialization in generate_materializations(
            # context=context, # TODO currently unused
...
@solid(
    description="A solid to invoke dbt run.",
...
@solid(
    description="A solid to invoke dbt run and poll the resulting rpc process until it's complete.",
    input_defs=[InputDefinition(name="start_after", dagster_type=Nothing)],
...
@solid(
    description="A solid to invoke dbt test.",
    input_defs=[InputDefinition(name="start_after", dagster_type=Nothing)],
    output_defs=[
        OutputDefinition(
            name="request_token",
...
@solid(
    description="A solid to invoke dbt test and poll the resulting rpc process until it's complete.",
    input_defs=[InputDefinition(name="start_after", dagster_type=Nothing)],
    output_defs=[OutputDefinition(name="result", dagster_type=DbtRpcPollResult)],
    config_schema={
        "models": Field(
...
@solid(
    description="A solid to invoke a dbt run operation.",
    input_defs=[InputDefinition(name="start_after", dagster_type=Nothing)],
    output_defs=[
        OutputDefinition(
            name="request_token",
...
@solid(
    description="A solid to invoke a dbt run operation and poll the resulting rpc process until it's complete.",
    input_defs=[InputDefinition(name="start_after", dagster_type=Nothing)],
    output_defs=[OutputDefinition(name="result", dagster_type=DbtRpcPollResult)],
    config_schema={
        "macro": Field(
...
@solid(
    description="A solid to invoke a dbt snapshot.",
    input_defs=[InputDefinition(name="start_after", dagster_type=Nothing)],
    output_defs=[
        OutputDefinition(
            name="request_token",
...
@solid(
    description="A solid to invoke a dbt snapshot and poll the resulting rpc process until it's complete.",
    input_defs=[InputDefinition(name="start_after", dagster_type=Nothing)],
    output_defs=[OutputDefinition(name="result", dagster_type=DbtRpcPollResult)],
    config_schema={
        "select": Field(
...
@solid(
    description="A solid to invoke dbt source snapshot-freshness and poll the resulting rpc process until it's complete.",
    input_defs=[InputDefinition(name="start_after", dagster_type=Nothing)],
    output_defs=[OutputDefinition(name="result", dagster_type=DbtRpcPollResult)],
    config_schema={
        "select": Field(
...
@solid(
    description="A solid to compile a SQL query in context of a dbt project.",
    input_defs=[
...
def dbt_rpc_compile_sql(context, sql: String) -> String:
    ...
    )  # pylint: disable=no-member # TODO no idea what's going on here
...
def create_dbt_rpc_run_sql_solid(
    ...
    @solid(
        ...
        description=kwargs.pop(
            "description",
            "A solid to run a SQL query in context of a dbt project and return the results in a pandas DataFrame.",
        ),
        input_defs=[
            InputDefinition(name="start_after", dagster_type=Nothing),
            InputDefinition(
    ...
    def _dbt_rpc_run_sql(context, sql: String) -> DataFrame:
        ...
        table = result.results[  # pylint: disable=no-member
            0
        ].table  # TODO no idea what's going on here
