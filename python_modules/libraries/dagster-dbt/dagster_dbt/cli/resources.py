...
class DbtCliResource:
    ...
    def execute_command(self, command: str, extra_flags: Dict[str, Any] = None) -> DbtCliOutput:
        """
        Executes a dbt CLI command.

        Args:
            command (str): The command you wish to run (e.g. 'run', 'test', 'docs generate', etc.)
            extra_flags (Dict[str, Any]): A dictionary containing additional flags to be passed into
                this CLI invocation. These will be merged with the default flags that were configured
                on resource initialization (if any), overriding the default values if necessary.
    ...
    def run(self, extra_flags: Dict[str, Any] = None) -> DbtCliOutput:
        """
        Executes the `dbt run` CLI command.
    ...
    def seed(self, extra_flags: Dict[str, Any] = None):
        """
        Executes the `dbt seed` CLI command.
    ...
    def test(self, extra_flags: Dict[str, Any] = None):
        """
        Executes the `dbt test` CLI command.
