...
def test_basic_event_store(conn_string):
    ...
    event_log_storage = MySQLEventLogStorage.create_clean_storage(conn_string)
...
def test_wipe_mysql_event_log(conn_string):
    @solid
    def return_one(_):
        return 1
...
def test_delete_mysql_event_log(conn_string):
    @solid
    def return_one(_):
        return 1
...
def test_load_from_config(hostname):
    url_cfg = """
      event_log_storage:
        ...
        config:
            mysql_url: mysql+mysqlconnector://test:test@{hostname}:5432/test
    """.format(
        hostname=hostname
    )
