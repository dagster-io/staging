...
class MySQLRunStorage(SqlRunStorage, ConfigurableClass):
    ...
    def __init__(self, mysql_url, inst_data=None):
        ...
        table_names = retry_mysql_connection_fn(lambda: db.inspect(self._engine).get_table_names())

        if "runs" not in table_names:
            with self.connect() as conn:
                alembic_config = get_alembic_config(__file__)  # pylint: disable=unused-variable
