...
def get_conn(host: str, user: str, passwd: str):
    conn = mysql.connect(
        host=check.str_param(host, "host"),
        user=check.str_param(user, "user"),
        passwd=check.str_param(passwd, "passwd"),
    )

    # conn.set_isolation_level(psycopg2.extensions.ISOLATION_LEVEL_AUTOCOMMIT)
    # TODO - no global isolation level setting for MySQL
    return conn
...
#             if retry_limit == 0:
#                 six.raise_from(DagsterMySQLException("too many retries for DB creation"), exc)
...
def retry_mysql_connection_fn(fn, retry_limit=5, retry_wait=0.2):
    ...
    while True:
        ...
        # TODO
            ...
            if retry_limit == 0:
                six.raise_from(DagsterMySQLException("too many retries for DB connection"), exc)
...
def mysql_statement_timeout(millis):
