...
from lakehouse.queryable_asset_set import QueryableAssetSet, generate_dep_graph
...
def test_generate_dep_graph():
    ...
    assert dep_graph == {
        'upstream': {'a': {}, 'b': {'a'}, 'c': {'a'}, 'd': {'b', 'c'}, 'e': {}, 'f': {'e'}},
        'downstream': {'a': {'b', 'c'}, 'b': {'d'}, 'c': {'d'}, 'd': {}, 'e': {'f'}, 'f': {}},
    }
...
def test_parse_solid_selection_single(query, expected_asset_names):
    ...
    assert {asset.path for asset in result_assets} == expected_asset_names
