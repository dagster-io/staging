    ...
    **kwargs,
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if "resources" not in run_config:
            ...
            if "io_manager" not in run_config["resources"]:
...
@pytest.mark.notebook_test
def test_sum_nums_pipeline():
    with exec_for_test("sum_nums_pipeline") as result:
        assert result.success
        assert result.result_for_solid("sum_nums").output_value() == 18
