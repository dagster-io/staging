...
from dagster.utils import PICKLE_PROTOCOL, EventGenerationManager, ensure_gen

from .context import (
    DagstermillExecutionContext,
    DagstermillRuntimeExecutionContext,
    LegacyDagstermillRuntimeExecutionContext,
)
from .errors import DagstermillError
from .io import write_notebook_output
...
class Manager:
    ...
    def reconstitute_pipeline_context(
        ...
        with scoped_pipeline_context(
            ...
            if legacy:
                context_class = LegacyDagstermillRuntimeExecutionContext
            else:
                context_class = DagstermillRuntimeExecutionContext
    ...
    def load_input(self, input_name):
        """Loads an input to the Dagstermill solid at runtime.
        ...
        # cf. dagster.core.exectution.plan.execute_step.core_dagster_event_sequence_for_step
        ...
        for event_or_input_value in ensure_gen(step_input.source.load_input_object(step_context)):
            if isinstance(event_or_input_value, DagsterEvent):
                # We don't want to yield these events twice
                continue
            else:
    ...
    def yield_result(self, value, output_name: str = "result"):
        ...
        assert isinstance(solid_context, DagstermillRuntimeExecutionContext), (
            f"Solid context is of wrong type for in-pipeline execution: got {type(solid_context)} "
            "but expected DagstermillRuntimeExecutionContext"
        )
        if isinstance(solid_context, LegacyDagstermillRuntimeExecutionContext):
            from .deprecated.serialize import write_value
            ...
            write_notebook_output(solid_context=solid_context, output_name=output_name, value=value)
    ...
    def load_parameter(self, input_name, input_value):
        ...
        from .deprecated.serialize import read_value
