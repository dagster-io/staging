...
def define_dagstermill_solid(
    ...
    if tags is not None:
        ...
        def check_reserved_tag_key_not_overridden(tag_key_name: str):
            check.invariant(
                tag_key_name not in tags,
                f"user-defined solid tags contains the `{tag_key_name}` key, but the `{tag_key_name}` key is reserved for use by Dagster",
            )

        check_reserved_tag_key_not_overridden("notebook_path")
        check_reserved_tag_key_not_overridden("kind")
