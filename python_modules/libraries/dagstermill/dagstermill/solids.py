...
def define_dagstermill_solid(
    ...
    if tags is not None:
        tag_contains_reserved_key_msg = (
            lambda key_name: "user-defined solid `tags` contains `{key_name}` key, but the `{key_name}` key is reserved for use by Dagster"
        )
        check.invariant("notebook_path" not in tags, tag_contains_reserved_key_msg("notebook_path"))
        check.invariant("kind" not in tags, tag_contains_reserved_key_msg("kind"))
