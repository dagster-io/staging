...
@contextmanager
def create_pg_connection(engine, dunder_file, storage_name=None):
    check.inst_param(engine, "engine", sqlalchemy.engine.Engine)
    check.str_param(dunder_file, "dunder_file")
    check.opt_str_param(storage_name, "storage_type_desc", "")
    ...
    try:
        ...
        with handle_schema_errors(
            ...
            msg="Postgres {}storage requires migration".format(storage_name),
        ):
            yield conn
    finally:
        if conn:
