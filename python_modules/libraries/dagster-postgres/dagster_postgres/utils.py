    ...
    while True:
        try:
            ...
            if (
                ...
                origin = (
                    str(exc.origin)
                    + ((" : CODE: " + str(exc.origin.pgcode)) if exc.origin else " : NO CODE")
                    if isinstance(sqlalchemy.exc.ProgrammingError)
                    else ("PG CODE: " + str(exc.pgcode))
                )

                six.raise_from(DagsterPostgresException("dgibson ORIGIN: " + origin), exc)
