...
class PostgresRunStorage(SqlRunStorage, ConfigurableClass):
    ...
    def has_migrated(self, migration_name):
        if migration_name not in self._migration_cache:
            self._migration_cache[migration_name] = super(PostgresRunStorage, self).has_migrated(
                migration_name
            )
        return self._migration_cache[migration_name]

    def mark_migrated(self, migration_name):
        super(PostgresRunStorage, self).mark_migrated(migration_name)
        if migration_name in self._migration_cache:
            del self._migration_cache[migration_name]
