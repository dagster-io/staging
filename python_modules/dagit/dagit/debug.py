...
def create_debug_cli(input_file):
    ...
    with GzipFile(input_file, "rb") as file:
        ...
        if not isinstance(debug_payload, DebugRunPayload):
            click.UsageError("File contents were not expected type.")
...
def main():
    create_debug_cli()  # pylint: disable=no-value-for-parameter
