...
def define_bar_sensors():
    ...
    def foo_sensor(_):
        return SensorTickData(run_params=[SensorRunParams(run_config={})])
...
def managed_grpc_scheduler_instance(overrides=None):
    return scheduler_instance(
        overrides=merge_dicts({"opt_in": {"local_servers": True}}, overrides if overrides else {})
    )
...
def sensor_command_contexts():
    command_line_scheduler_overrides = {
        "scheduler": {"module": "dagster.core.scheduler", "class": "DagsterCommandLineScheduler",}
    }

    return [
        args_with_instance(
            scheduler_instance(command_line_scheduler_overrides),
            ["-w", file_relative_path(__file__, "workspace.yaml")],
        ),
        pytest.param(
            args_with_instance(
                managed_grpc_scheduler_instance(command_line_scheduler_overrides),
                ["-w", file_relative_path(__file__, "workspace.yaml")],
            ),
            marks=pytest.mark.managed_grpc,
        ),
        pytest.param(
            grpc_server_scheduler_cli_args(command_line_scheduler_overrides),
            marks=pytest.mark.deployed_grpc,
        ),
    ]
