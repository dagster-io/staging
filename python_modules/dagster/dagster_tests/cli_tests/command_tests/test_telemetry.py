...
@pytest.mark.parametrize("env", [{"BUILDKITE": None}, {"PYTHONLEGACYWINDOWSSTDIO": None}])
@responses.activate
...
@pytest.mark.parametrize("env", [{"BUILDKITE": "True"}, {"PYTHONLEGACYWINDOWSSTDIO": "1"}])
@responses.activate
def test_dagster_telemetry_no_buildkite_upload(env):
    with environ(env):
        with instance_for_test(enable_telemetry=True):
            runner = CliRunner()
            with pushd(path_to_file("")):
