...
@pytest.mark.parametrize("gen_sensor_args", sensor_command_contexts())
def test_sensors_up(gen_sensor_args):
    with gen_sensor_args as (cli_args, instance):
        runner = CliRunner()
        with mock.patch("dagster.core.instance.DagsterInstance.get") as _instance:
            _instance.return_value = instance
            result = runner.invoke(sensor_up_command, cli_args,)
            assert result.exit_code == 0
            assert "Changes:\n" in result.output
            assert "  + foo_sensor (add)" in result.output


@pytest.mark.parametrize("gen_sensor_args", sensor_command_contexts())
def test_sensors_up_and_list(gen_sensor_args):
    with gen_sensor_args as (cli_args, instance):
        runner = CliRunner()
        with mock.patch("dagster.core.instance.DagsterInstance.get") as _instance:
            _instance.return_value = instance
...
def test_check_repo_and_scheduler_instance_scheduler_not_set():
    ...
    repository.get_external_sensors.return_value = [mock.MagicMock()]
    instance = mock.MagicMock(spec=DagsterInstance)
    type(instance).sensorr = mock.PropertyMock(return_value=None)
