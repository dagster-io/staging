# pylint: disable=pylint(cell-var-from-loop)
...
def wait_for_condition(fn, interval, timeout):
...
def test_grpc_watch_thread_server_update():
    ...
    wait_for_condition(lambda: called, interval=watch_interval, timeout=10)
...
def test_grpc_watch_thread_server_error():
    for _ in range(50):
        ...
        wait_for_condition(lambda: called.get("on_error"), watch_interval, timeout=10)
...
def test_grpc_watch_thread_server_complex_cycle():
    for _ in range(50):
        ...
        wait_for_condition(lambda: "on_disconnect" in events, watch_interval, timeout=10)
...
def test_grpc_watch_thread_server_complex_cycle_2():
    for _ in range(50):
        ...
        # Wait for reconnect attempts to exhaust and on_error callback to be called
        wait_for_condition(lambda: called.get("on_error"), watch_interval, timeout=30)
