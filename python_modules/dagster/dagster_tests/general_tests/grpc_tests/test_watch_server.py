...
def test_grpc_watch_thread_server_update():
    ...
    try:
        ...
        shutdown_event, watch_thread = create_grpc_watch_thread(
            client, on_updated=on_updated, watch_interval=watch_interval, max_reconnect_attempts=20,
        )
        watch_thread.start()
...
def test_grpc_watch_thread_server_reconnect():
    ...
    interrupt_ipc_subprocess_pid(server_process.pid)
    wait_for_condition(lambda: called.get("on_disconnect"), watch_interval, timeout=10)

    server_process = open_server_process(port=port, socket=None, fixed_server_id=fixed_server_id)
    wait_for_condition(lambda: called.get("on_reconnected"), watch_interval, timeout=10)
...
def test_grpc_watch_thread_server_complex_cycle():
    ...
    for x in range(1, cycles + 1):
        ...
        wait_for_condition(lambda: events.count("on_disconnect") == x, watch_interval, timeout=10)
        open_server_process(port=port, socket=None, fixed_server_id=fixed_server_id)
        wait_for_condition(lambda: events.count("on_reconnected") == x, watch_interval, timeout=10)

    # SImulate server update
    client.shutdown_server()
    wait_for_condition(
        lambda: events.count("on_disconnect") == cycles + 1, watch_interval, timeout=10
    )
    open_server_process(port=port, socket=None)
    wait_for_condition(lambda: "on_updated" in events, watch_interval, timeout=10)
