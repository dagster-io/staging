...
def test_stream_detect_server_restart():
    ...
    # For some reason, checking the error using pytest.raises does not work because
    # pytest thinks grpc._channel is not a valid property, so we use a boolean flag
    # to track the error instead.
    #
    # Should investigate why pytest has a problem with this:
    #
