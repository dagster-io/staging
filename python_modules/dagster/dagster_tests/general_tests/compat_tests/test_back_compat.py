...
def test_event_log_secondary_index_table():
    ...
    with restore_directory(test_dir):
        ...
        assert "event_log_secondary_indexes" not in get_sqlite3_tables(db_path)

        # Make sure the schema is migrated
        instance = DagsterInstance.from_ref(InstanceRef.from_dir(test_dir))
        instance.upgrade()

        assert "event_log_secondary_indexes" in get_sqlite3_tables(db_path)
