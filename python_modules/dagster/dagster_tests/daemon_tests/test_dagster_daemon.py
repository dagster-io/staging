...
def test_healthy():
    ...
    with instance_for_test(
        ...
        controller = DagsterDaemonController(instance)
        assert not controller.healthy(instance, curr_time=init_time)

        controller.run_iteration(init_time)
        assert controller.healthy(instance, curr_time=init_time)

        assert not controller.healthy(instance, curr_time=beyond_tolerated_time)
...
def test_healthy_with_different_daemons():
    ...
    with instance_for_test(
        ...
        assert not DagsterDaemonController.healthy(instance, curr_time=init_time)
