...
@asset_sensor(asset_key=AssetKey("foo"), job=the_pipeline)
def asset_pipeline_job_sensor(context, _event):
    return RunRequest(run_key=context.cursor, run_config={})
...
@pytest.mark.parametrize("external_repo_context", repos())
def test_asset_pipeline_job_sensor(external_repo_context):
    freeze_datetime = to_timezone(
        create_pendulum_time(year=2019, month=2, day=27, tz="UTC"),
        "US/Central",
    )
    with instance_with_sensors(external_repo_context) as (
        ...
        with pendulum.test(freeze_datetime):
            pipeline_job_sensor = external_repo.get_external_sensor("asset_pipeline_job_sensor")
            instance.start_sensor(pipeline_job_sensor)

            evaluate_sensors(instance, workspace)

            ticks = instance.get_job_ticks(pipeline_job_sensor.get_external_origin_id())
            assert len(ticks) == 1
            validate_tick(
                ticks[0],
                pipeline_job_sensor,
        ...
        with pendulum.test(freeze_datetime):
            ...
            assert run.run_config == {}
            assert run.tags
            assert run.tags.get("dagster/sensor_name") == "asset_pipeline_job_sensor"
