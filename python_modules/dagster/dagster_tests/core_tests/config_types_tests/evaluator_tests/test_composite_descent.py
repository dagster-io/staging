...
def test_configuring_solids_without_specifying_name():
    ...
    @composite_solid(config_schema={'num': int}, config_fn=lambda cfg: {"return_int": {"config": cfg['num']}})
    def return_int_composite():
        return_int()

    with pytest.raises(
        CheckError, match="Param \"name\" is not a str",
    ):
...
def test_configuring_composite_solid_with_no_config_mapping():
    ...
    with pytest.raises(
        CheckError, match="Only composite solids utilizing config mapping can be pre-configured. The solid \"composite_without_config_fn\"",
    ):
