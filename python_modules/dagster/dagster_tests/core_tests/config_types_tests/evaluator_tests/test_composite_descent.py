...
def test_configuring_composite_solid_with_no_config_mapping():
    ...
    @composite_solid
    def compsite_without_config_fn():
        return return_run_id()

    with pytest.raises(
        CheckError, match="Only composite solids utilizing config mapping can be pre-configured",
    ):
        configured(compsite_without_config_fn, name="configured_composite")({})
