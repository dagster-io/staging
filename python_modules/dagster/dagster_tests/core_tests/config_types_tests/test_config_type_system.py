...
def test_multiple_required_fields_failing():
    ...
    with pytest.raises(
        AssertionError,
        match=f"Missing required config entries \['field_one', 'field_two'\] at the root. .* {expected_suggested_config}",
    ):
    ...
    with pytest.raises(
        AssertionError,
        match=f'Missing required config entry "field_two" at the root. .* {expected_suggested_config}',
    ):
