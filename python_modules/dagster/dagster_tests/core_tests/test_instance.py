...
def test_dagster_home_dagster_yaml_not_set(tmpdir):
    with environ({"DAGSTER_HOME": str(tmpdir)}):
        with pytest.warns(
            UserWarning,
            match=re.escape(
                "The dagster instance configuration file (dagster.yaml) is not present"
            ),
        ):
            _dagster_home()
