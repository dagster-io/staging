...
def test_resolve_step_versions_external_dependencies():
    config_value = {"solids": {"versioned_solid_ext_input": {"inputs": {"custom_type": "a"}}}}
    speculative_execution_plan = create_execution_plan(
        versioned_pipeline_ext_input, run_config=config_value,
    )

    versions = resolve_step_versions(speculative_execution_plan, config_value=config_value,)
...
def fake_solid(context):
    return "solidified_" + context.resources.test_resource
