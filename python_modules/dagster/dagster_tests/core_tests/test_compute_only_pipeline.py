from dagster import InputDefinition, OutputDefinition, execute_pipeline, pipeline, solid
...
def test_execute_solid_with_dep_only_inputs_no_api():
    ...
    @solid(input_defs=[InputDefinition("step_one_solid")], output_defs=[OutputDefinition()])
    def step_two_solid(_):
...
def test_execute_solid_with_dep_only_inputs_with_api():
    ...
    @solid(input_defs=[InputDefinition(step_one_solid.name)], output_defs=[OutputDefinition()])
    def step_two_solid(_):
        _set_key_value(did_run_dict, "step_two", True)
