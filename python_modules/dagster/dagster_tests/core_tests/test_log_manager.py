...
from dagster.core.execution.plan.objects import ErrorSetting, StepFailureData
...
def make_log_string(error, error_setting=None):
    step_failure_event = DagsterEvent(
        ...
        event_specific_data=StepFailureData(
            error=error, user_failure_data=None, error_setting=error_setting
        ),
...
def test_construct_log_string_with_user_code_error():
    ...
    log_string = make_log_string(error, error_setting=ErrorSetting.USER_ERROR)
    expected_start = textwrap.dedent(
        """
        my_pipeline - f79a8a93-27f1-41b5-b465-b35d0809b26d - 54348 - STEP_FAILURE - Execution of step "solid2" failed.
