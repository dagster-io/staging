...
def test_system_storage_config_requires_run_config():
    """Test creation of StorageConfig object when system storage definition requires config."""
    ...
    run_config = {'test_system_requires_config': {'config': {'field': 'value'}}}

    storage_config = StorageConfig.from_dict(
        run_config, system_storage_defs=[system_storage_requires_config]
    )
    assert storage_config.system_storage_name == 'test_system_requires_config'
    assert storage_config.system_storage_config == {'field': 'value'}

    with pytest.raises(
        DagsterInvariantViolationError,
        match='Run config required for system storage, but none provided.',
    ):
        StorageConfig.from_dict(None, system_storage_defs=[system_storage_requires_config])


def test_intermediate_storage_config_requires_run_config():
    """IntermediateStorageConfig creation when IntermediateStorageDefinition requires config."""
    ...
    storage_config = IntermediateStorageConfig.from_dict(
        run_config, intermediate_storage_defs=[intermediate_storage_requires_config]
    )
    assert storage_config.intermediate_storage_name == 'test_system_requires_config'
    assert storage_config.intermediate_storage_config == {'field': 'value'}
    ...
    with pytest.raises(
        ...
        IntermediateStorageConfig.from_dict(
            None, intermediate_storage_defs=[intermediate_storage_requires_config]
        )
