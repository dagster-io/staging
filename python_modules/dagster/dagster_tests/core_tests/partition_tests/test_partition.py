...
@pytest.mark.parametrize(
    ...
    argvalues=[
        (lambda current_time: [Partition("a_partition")],),
        (lambda current_time: [Partition(x) for x in range(10)],),
    ],
)
def test_dynamic_partitions(partition_fn: Callable[[Optional[datetime]], List[Partition]]):
    ...
    assert partition_params.get_partitions(current_time=None) == partition_fn(current_time=None)
