...
def test_ins():
    ...
    @op(ins=[In("a", metadata={"x": 1}), In("b", metadata={"y": 2})])
...
def test_multi_out():
    @op(out=MultiOut([Out(name="a", metadata={"x": 1}), Out(name="b", metadata={"y": 2})]))
...
def test_multi_out_optional():
    @op(
        out=MultiOut(
            [Out(name="a", metadata={"x": 1}, is_required=False), Out(name="b", metadata={"y": 2})]
        )
    )
    def my_op():
        yield Output(output_name="b", value=2)
...
def test_ins_dict():
    ...
    assert my_op.input_defs[0].dagster_type.typing_type == Any
    assert my_op.input_defs[1].dagster_type.typing_type == Any
...
def test_multi_out_dict():
    ...
    assert my_op.output_defs[0].metadata == {"x": 1}
    assert my_op.output_defs[0].name == "a"
    assert my_op.output_defs[0].dagster_type.typing_type == Any
    assert my_op.output_defs[1].metadata == {"y": 2}
    assert my_op.output_defs[1].name == "b"
    assert my_op.output_defs[1].dagster_type.typing_type == Any
