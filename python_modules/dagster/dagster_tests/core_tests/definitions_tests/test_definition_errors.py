...
def test_invalid_item_in_solid_list():
    with pytest.raises(
        ...
        PipelineDefinition(name="test", solid_defs=["not_a_solid"])
...
def test_one_layer_off_dependencies():
    with pytest.raises(
        ...
        PipelineDefinition(
            name="test", solid_defs=solid_a_b_list(), dependencies={"B": DependencyDefinition("A")}
        )
...
def test_list_dependencies():
    with pytest.raises(
        ...
        PipelineDefinition(name="test", solid_defs=solid_a_b_list(), dependencies=[])
