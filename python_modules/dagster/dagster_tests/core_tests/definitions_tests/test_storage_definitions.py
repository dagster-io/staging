...
def test_intermediate_storage_deprecation_warning():
    ...
    with pytest.warns(
        UserWarning,
        match=re.escape(
            "IntermediateStorageDefinition and @intermediate_storage is deprecated in 0.10.0 and will be removed in 0.11.0."
        ),
    ):
    ...
    with pytest.warns(
        UserWarning,
        match=re.escape('"intermediate_storage" entry in the run config is deprecated'),
    ):
