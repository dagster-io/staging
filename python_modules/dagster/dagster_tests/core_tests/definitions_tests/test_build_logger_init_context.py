...
import pytest
from dagster import build_logger_init_context, logger
...
def test_basic_logger_init():
    ...
    ret_logger = foo_logger.logger_fn(build_logger_init_context(foo_logger))
...
def test_logger_with_config():
    ...
    with pytest.raises(DagsterInvalidConfigError, match="Error in config for logger"):
        int_logger.logger_fn(build_logger_init_context(int_logger))
    ...
    with pytest.raises(DagsterInvalidConfigError, match="Error in config mapping for logger"):
        ...
        conf_logger.logger_fn(build_logger_init_context(conf_logger))

    ret_logger = int_logger.logger_fn(build_logger_init_context(int_logger, logger_config=3))
    assert ret_logger.level == 3

    conf_logger = int_logger.configured(4)
    ret_logger = conf_logger.logger_fn(build_logger_init_context(conf_logger))
