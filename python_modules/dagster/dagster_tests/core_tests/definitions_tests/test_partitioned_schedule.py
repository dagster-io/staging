...
def job_for_partitions(
    partitioned_config, minute_of_hour=None, time_of_day=None, day_of_week=None, day_of_month=None
):
    @solid
    def my_solid():
        pass
...
def test_daily_schedule_from_partitions():
    ...
    my_schedule = job_for_partitions(my_partitioned_config, time_of_day=time(hour=9, minute=30))
...
def test_hourly_schedule_from_partitions():
    ...
    my_schedule = job_for_partitions(my_partitioned_config, minute_of_hour=time(minute=30))
...
def test_weekly_schedule_from_partitions():
    ...
    my_schedule = job_for_partitions(
        my_partitioned_config, time_of_day=time(hour=9, minute=30), day_of_week=2
    )
...
def test_monthly_schedule_from_partitions():
    ...
    my_schedule = job_for_partitions(
        my_partitioned_config, time_of_day=time(hour=9, minute=30), day_of_month=2
    )
    assert my_schedule.cron_schedule == "30 9 2 * *"
