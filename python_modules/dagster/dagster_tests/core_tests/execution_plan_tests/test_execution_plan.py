...
def test_active_execution_plan():
    ...
    with plan.start(retries=(RetryMode.DISABLED)) as active_execution:
...
def test_retries_active_execution():
    ...
    with plan.start(retries=(RetryMode.ENABLED)) as active_execution:
...
def test_retries_deferred_active_execution():
    ...
    with plan.start(retries=(RetryMode.DEFERRED)) as active_execution:
