...
def test_topological_sort():
    plan = create_execution_plan(define_diamond_pipeline(), default_executors)
...
def test_priorities():
    ...
    plan = create_execution_plan(priorities, default_executors)
...
def test_fan_out_should_skip_step():
    ...
    execute_plan(
        create_execution_plan(
            optional_outputs, instance.default_executor_defs, step_keys_to_execute=["foo"]
        ),
    ...
    assert not should_skip_step(
        create_execution_plan(
            optional_outputs, instance.default_executor_defs, step_keys_to_execute=["bar_1"]
        ),
        instance,
        pipeline_run.run_id,
    )
    assert should_skip_step(
        create_execution_plan(
            optional_outputs, instance.default_executor_defs, step_keys_to_execute=["bar_2"]
        ),
        instance,
        pipeline_run.run_id,
    )
    assert should_skip_step(
        create_execution_plan(
            optional_outputs, instance.default_executor_defs, step_keys_to_execute=["bar_3"]
        ),
        instance,
        pipeline_run.run_id,
    )
