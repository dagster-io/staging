...
def test_topological_sort():
    ...
    assert [str(step.key) for step in levels[0]] == ["return_two.compute"]
    assert [str(step.key) for step in levels[1]] == ["add_three.compute", "mult_three.compute"]
    assert [str(step.key) for step in levels[2]] == ["adder.compute"]
...
def test_active_execution_plan():
    ...
    with plan.start(retries=Retries(RetryMode.DISABLED)) as active_execution:
        ...
        assert len(steps) == 1
        step_1 = steps[0]
        assert str(step_1.key) == "return_two.compute"
        ...
        step_2 = steps[0]
        step_3 = steps[1]
        assert str(step_2.key) == "add_three.compute"
        assert str(step_3.key) == "mult_three.compute"
        ...
        assert str(step_4.key) == "adder.compute"
...
def test_retries_active_execution():
    ...
    with plan.start(retries=Retries(RetryMode.ENABLED)) as active_execution:
        ...
        steps = active_execution.get_steps_to_execute()
        assert len(steps) == 1
        assert str(steps[0].key) == "return_two.compute"
...
def test_priorities():
    ...
    with plan.start(Retries(RetryMode.DISABLED), sort_key_fn) as active_execution:
        steps = active_execution.get_steps_to_execute()
        assert str(steps[0].key) == "pri_5.compute"
        assert str(steps[1].key) == "pri_4.compute"
        assert str(steps[2].key) == "pri_3.compute"
        assert str(steps[3].key) == "pri_2.compute"
        assert str(steps[4].key) == "pri_none.compute"
        assert str(steps[5].key) == "pri_neg_1.compute"
