...
def test_failure_hook_error_info():
    called = {}

    @failure_hook
    def a_hook(context):
        called[context.solid.name] = context.error_info
    ...
    assert "SomeUserException" in called.get("failed_solid").cause.cls_name


def test_invalid_error_info_access():
    called = {}
