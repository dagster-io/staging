...
def test_failure_hook_error():
    ...
    @failure_hook
    def a_hook(context: HookContext, error: SerializableErrorInfo):
        called[context.solid.name] = error
    ...
    assert isinstance(called.get("failed_solid"), SerializableErrorInfo)
