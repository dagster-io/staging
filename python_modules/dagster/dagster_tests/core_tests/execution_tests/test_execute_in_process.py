...
import pytest
from dagster import DagsterInvalidDefinitionError, Field, InputDefinition, check, resource, solid
...
def test_execute_solid_requires_config():
    ...
    result = execute_in_process(solid_requires_config, solid_config={"config": {"foo": "bar"}})
    assert result.success
    assert result.output_values["result"] == "bar"
...
def test_execute_graph_solids_require_config():
    ...
    result = execute_in_process(
        ...
        solid_config={"solids": {"solid_requires_config": {"config": {"foo": "bar"}}}},
    )
...
def test_execute_solid_with_io_config_io_manager():
    ...
    result = execute_in_process(
        ...
        solid_config={
            "outputs": {"result": {"test_output": "foo"}},
            "inputs": {"_x": {"test_input": "bar"}},
        },
    )
    assert result.success
...
def test_execute_in_process_input_config():
    _, add = get_solids()
    result = execute_in_process(
        add, solid_config={"inputs": {"x": {"value": 1}, "y": {"value": 2}}}
    )
    assert result.output_values["result"] == 3


def test_execute_in_process_input_config_and_input_values_passed():
    _, add = get_solids()
    with pytest.raises(check.CheckError):
        execute_in_process(add, input_values={"x": 1}, solid_config={"inputs": {"y": {"value": 2}}})
