...
@solid(
    input_defs=[InputDefinition("string_input", String)],
    version="take_string_version",
    config_schema={"input_str": Field(String), "base_dir": Field(String)},
)
def take_string_1(context, string_input):
    yield Output(
        context.solid_config["input_str"],
        address=os.path.join(
...
def take_string_2(context, string_input):
    yield Output(
        context.solid_config["input_str"],
        address=os.path.join(
...
@solid(
    input_defs=[
        InputDefinition("string_input_1", String),
        InputDefinition("string_input_2", String),
    ],
    version="take_string_two_inputs_version",
    config_schema={"input_str": Field(String), "base_dir": Field(String)},
)
def take_string_two_inputs(context, string_input_1, string_input_2):
    yield Output(
        context.solid_config["input_str"],
        address=os.path.join(
...
def basic_pipeline():
    take_string_two_inputs(
        string_input_1=take_string_1(create_string_1()),
        string_input_2=take_string_2(create_string_2()),
    )
