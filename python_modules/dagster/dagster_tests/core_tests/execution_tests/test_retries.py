...
def test_policy_delay_calc():
    ...
    # should these error instead?
    assert RetryPolicy(jitter=Jitter.PLUS_MINUS).calculate_delay(5) == 0
    assert RetryPolicy(jitter=Jitter.FULL).calculate_delay(5) == 0
    assert (
        RetryPolicy(backoff=Backoff.EXPONENTIAL, jitter=Jitter.PLUS_MINUS).calculate_delay(5) == 0
    )
    assert RetryPolicy(backoff=Backoff.LINEAR, jitter=Jitter.PLUS_MINUS).calculate_delay(5) == 0
