...
def test_policy_delay_calc():
    ...
    RetryPolicy(jitter=Jitter.PLUS_MINUS).calculate_delay(5) == 0
    RetryPolicy(jitter=Jitter.FULL).calculate_delay(5) == 0
    RetryPolicy(backoff=Backoff.EXPONENTIAL, jitter=Jitter.PLUS_MINUS).calculate_delay(5) == 0
    RetryPolicy(backoff=Backoff.LINEAR, jitter=Jitter.PLUS_MINUS).calculate_delay(5) == 0
