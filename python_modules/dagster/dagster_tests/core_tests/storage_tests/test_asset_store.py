...
def test_default_asset_store():
    with seven.TemporaryDirectory() as tmpdir_path:
        ...
        filepath_a = os.path.join(tmpdir_path, "solid_a.compute", "result")
        ...
        filepath_b = os.path.join(tmpdir_path, "solid_b.compute", "result")
        assert os.path.isfile(filepath_b)
        with open(filepath_b, "rb") as read_obj:
            assert pickle.load(read_obj) == 1


def test_default_asset_store_with_path():
    with seven.TemporaryDirectory() as tmpdir_path:
