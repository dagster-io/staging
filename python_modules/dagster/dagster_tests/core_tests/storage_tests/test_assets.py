...
@asset_test
def test_asset_tags(asset_aware_context):
    with asset_aware_context() as ctx:
        instance, event_log_storage = ctx
        execute_pipeline(pipeline_one, instance=instance)
        execute_pipeline(pipeline_two, instance=instance)
        ...
        # set tags
        event_log_storage.set_asset_tags(AssetKey("asset_1"), tags=dict(foo="FOO", bar="BAR"))
        event_log_storage.set_asset_tags(AssetKey("asset_2"), tags=dict(foo="FOO"))

        # get tags for asset
        tags = event_log_storage.get_asset_tags(AssetKey("asset_1"))
        assert len(tags) == 2
        assert tags["foo"] == "FOO"
        assert tags["bar"] == "BAR"

        # get asset keys with tag filters
        foo_keys = event_log_storage.get_asset_keys(tags=dict(foo="FOO"))
        assert len(foo_keys) == 2
        assert set([asset_key.to_string() for asset_key in foo_keys]) == set(
            ['["asset_1"]', '["asset_2"]']
        )
        bar_keys = event_log_storage.get_asset_keys(tags=dict(bar="BAR"))
        assert len(bar_keys) == 1
        assert set([asset_key.to_string() for asset_key in bar_keys]) == set(['["asset_1"]'])

        # overwrite tags
        event_log_storage.set_asset_tags(AssetKey("asset_1"), tags=dict(foo="FEZ"))
        tags = event_log_storage.get_asset_tags(AssetKey("asset_1"))
        assert len(tags) == 1
        assert tags["foo"] == "FEZ"
        foo_keys = event_log_storage.get_asset_keys(tags=dict(foo="FOO"))
        assert len(foo_keys) == 1
        assert set([asset_key.to_string() for asset_key in foo_keys]) == set(['["asset_2"]'])
        bar_keys = event_log_storage.get_asset_keys(tags=dict(bar="BAR"))
        assert not bar_keys
