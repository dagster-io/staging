...
def stats_records(run_id):
    now = time.time()
    return [
        event_record(run_id, "A", now - 325, DagsterEventType.STEP_START),
        event_record(
            run_id,
            "A",
            now - 225,
        ...
        event_record(run_id, "B", now - 225, DagsterEventType.STEP_START),
        event_record(
            run_id,
            "B",
            now - 175,
        ...
        event_record(run_id, "C", now - 175, DagsterEventType.STEP_START),
        event_record(run_id, "C", now - 150, DagsterEventType.STEP_SKIPPED),
        event_record(run_id, "D", now - 150, DagsterEventType.STEP_START),
        event_record(
            run_id,
            "D",
            now - 125,
...
def event_record(run_id, solid_name, timestamp, event_type, event_specific_data=None):
...
def fetch_all_events(configured_storage, run_id=None):
...
RUN_SEGMENTED_STORAGES = (InMemoryEventLogStorage, SqliteEventLogStorage)
...
class TestEventLogStorage:
    ...
    def test_event_log_storage_store_events_and_wipe(self, storage):
        ...
        assert storage.get_stats_for_run(RUN_ID)
        storage.wipe()
        assert len(storage.get_logs_for_run(RUN_ID)) == 0
    ...
    def test_event_log_storage_watch(self, storage):
        ...
        storage.end_watch(RUN_ID, watcher_1)
        ...
        while len(watched_2) < 2 and attempts > 0:
            time.sleep(0.25)
            attempts -= 1
        storage.end_watch(RUN_ID, watcher_2)
    ...
    def test_event_log_storage_pagination(self, storage):
        storage.store_event(create_event(0))
        storage.store_event(create_event(1))
        storage.store_event(create_event(2))

        assert len(storage.get_logs_for_run(RUN_ID)) == 3
        assert len(storage.get_logs_for_run(RUN_ID, -1)) == 3
        assert len(storage.get_logs_for_run(RUN_ID, 0)) == 2
        assert len(storage.get_logs_for_run(RUN_ID, 1)) == 1
        assert len(storage.get_logs_for_run(RUN_ID, 2)) == 0
    ...
    def test_event_log_get_stats_for_run(self, storage):
        ...
        assert math.isclose(storage.get_stats_for_run(RUN_ID).enqueued_time, enqueued_time)
        assert math.isclose(storage.get_stats_for_run(RUN_ID).launch_time, launched_time)
        assert math.isclose(storage.get_stats_for_run(RUN_ID).start_time, start_time)

    def test_event_log_step_stats(self, storage):
        # When an event log doesn't have a PIPELINE_START or PIPELINE_SUCCESS | PIPELINE_FAILURE event,
        # we want to ensure storage.get_stats_for_run(...) doesn't throw an error.

        for record in stats_records(run_id=RUN_ID):
    ...
    def test_basic_event_store(self, storage):
        ...
        events, _result = synthesize_events(_solids, run_id=RUN_ID)

        for event in events:
            storage.store_event(event)

        rows = fetch_all_events(storage, run_id=RUN_ID)
    ...
    def test_basic_get_logs_for_run_cursor(self, storage):
        ...
        assert event_types(storage.get_logs_for_run(result.run_id, cursor=0)) == event_types(events)

        assert event_types(storage.get_logs_for_run(result.run_id, cursor=1)) == event_types(events)
    ...
    def test_event_watcher_single_run_event(self, storage):
        ...
        storage.event_watcher.watch_run(run_id, 0, event_list.append)
    ...
    def test_event_watcher_filter_two_runs_event(self, storage):
        ...
        storage.event_watcher.watch_run(run_id_one, 0, event_list_one.append)
        storage.event_watcher.watch_run(run_id_two, 0, event_list_two.append)
