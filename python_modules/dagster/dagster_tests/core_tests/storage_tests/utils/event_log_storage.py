...
class TestEventLogStorage:
    ...
    def test_get_event_rows(self, storage):
        ...
        # before cursor
        assert not list(filter(lambda r: r[0] >= 5, storage.get_event_rows(before_cursor=5)))
        assert [
            id for id, _ in storage.get_event_rows(before_cursor=5, ascending=True, limit=3)
        ] == [min_record_num, min_record_num + 1, min_record_num + 2]
        assert [
            id for id, _ in storage.get_event_rows(before_cursor=5, ascending=False, limit=3)
        ] == [4, 3, 2]

        # after cursor
        assert not list(filter(lambda r: r[0] <= 2, storage.get_event_rows(after_cursor=2)))
        assert [
            id for id, _ in storage.get_event_rows(after_cursor=2, ascending=True, limit=3)
        ] == [3, 4, 5]
        assert [
            id for id, _ in storage.get_event_rows(after_cursor=2, ascending=False, limit=3)
    ...
    def test_get_event_rows_sqlite(self, storage):
        ...
        with instance_for_test() as instance:
            ...
            run_rows = instance.get_run_rows()
            ...
            filtered_records = storage.get_event_rows(
                run_updated_after=run_rows[-1]["update_timestamp"],  # events after first run
