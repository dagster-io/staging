    ...
    def test_get_event_rows(self, storage):
        ...
        assert [
            ...
            for id, _ in storage.get_event_rows(
                before_cursor=min_record_num + 3, ascending=True, limit=2
            )
        ] == [min_record_num, min_record_num + 1]
        assert [
            id
            for id, _ in storage.get_event_rows(
                before_cursor=min_record_num + 3, ascending=False, limit=2
            )
        ] == [min_record_num + 2, min_record_num + 1]
        ...
        assert [
            id
            for id, _ in storage.get_event_rows(
                after_cursor=min_record_num + 2, ascending=True, limit=2
            )
        ] == [min_record_num + 3, min_record_num + 4]
        assert [
            id for id, _ in storage.get_event_rows(after_cursor=2, ascending=False, limit=2)
