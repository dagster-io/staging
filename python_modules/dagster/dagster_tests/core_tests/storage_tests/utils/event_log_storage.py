...
class TestEventLogStorage:
    ...
    def test_get_logs_for_run_type(self, storage):
        @solid
        def return_one(_):
            return 1
        ...
        assert _event_types(
            storage.get_logs_for_run(
                result.run_id, dagster_event_type=DagsterEventType.PIPELINE_SUCCESS
            )
        ) == [DagsterEventType.PIPELINE_SUCCESS]

        assert _event_types(
            storage.get_logs_for_run(
                result.run_id, dagster_event_type=DagsterEventType.STEP_SUCCESS
            )
        ) == [DagsterEventType.STEP_SUCCESS]
