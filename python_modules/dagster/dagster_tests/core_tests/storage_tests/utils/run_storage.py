...
class TestRunStorage:
    ...
    def test_backfill(self, storage):
        ...
        assert storage.has_backfill_table()
        ...
        assert len(storage.get_backfills(status=BackfillJobStatus.REQUESTED)) == 1
        backfill = storage.get_backfill(one.backfill_id)
        assert backfill == one

        storage.update_backfill(one.with_status(status=BackfillJobStatus.COMPLETED))
        assert len(storage.get_backfills()) == 1
        assert len(storage.get_backfills(status=BackfillJobStatus.REQUESTED)) == 0
