...
def test_io_manager_with_config():
    ...
    @pipeline(
        mode_defs=[ModeDefinition(resource_defs={"io_manager": configurable_io_manager})]
    )
    def my_pipeline():
        my_solid()
...
def test_configured():
    ...
    assert (
        configured_io_manager.required_resource_keys == an_io_manager.required_resource_keys
    )
...
def test_io_manager_resources_on_context():
    ...
    @solid(
        ...
        output_defs=[
            OutputDefinition(dagster_type=str, manager_key="io_manager_reqs_resources")
        ],
    )
