...
from dagster.core.runs_coordinator.local_runs_coordinator import LocalRunsCoordinator
...
def coodinator(instance):  # pylint: disable=redefined-outer-name
    q = LocalRunsCoordinator()
    q.initialize(instance)
    yield q


def call_enqueue_run(instance, coodinator, run):  # pylint: disable=redefined-outer-name
    with external_pipeline_from_run(instance, run) as external:
        return coodinator.enqueue_run(instance, run, external)
...
def test_enqueue_run(instance, coodinator):  # pylint: disable=redefined-outer-name
    run = call_enqueue_run(instance, coodinator, create_run(instance, run_id="foo-1"))
...
def test_enqueue_run_checks_status(instance, coodinator):  # pylint: disable=redefined-outer-name
