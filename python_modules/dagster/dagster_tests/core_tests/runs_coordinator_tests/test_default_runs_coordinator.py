...
from dagster.core.runs_coordinator.default_runs_coordinator import DefaultRunsCoordinator
...
def test_submit_run(instance, coordinator):  # pylint: disable=redefined-outer-name
    run = coordinator.submit_run(
        instance, create_run(instance, run_id="foo-1", status=PipelineRunStatus.NOT_STARTED), None
    )
...
def test_submit_run_checks_status(instance, coordinator):  # pylint: disable=redefined-outer-name
    with pytest.raises(CheckError):
        coordinator.submit_run(
            instance, create_run(instance, run_id="foo-1", status=PipelineRunStatus.QUEUED), None,
        )
