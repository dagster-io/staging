...
def test_wrong_output():
    ...
    with pytest.raises(
        DagsterSolidInvocationError,
        match='Error occurred while invoking solid "solid_wrong_output"',
    ):
        solid_wrong_output()
...
def test_output_not_sent():
    ...
    with pytest.raises(
        DagsterSolidInvocationError,
        match='Error occurred while invoking solid "solid_multiple_outputs_not_sent"',
    ):
...
def test_output_sent_multiple_times():
    ...
    with pytest.raises(
        DagsterSolidInvocationError,
        match='Error occurred while invoking solid "solid_yields_twice"',
    ):
        solid_yields_twice()
...
def test_solid_retry_requested():
    ...
    with pytest.raises(
        DagsterInvariantViolationError,
        match="Retry request denied. Retries are not supported when directly invoking solids.",
    ):
        solid_retries()
