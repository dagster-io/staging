...
from dagster.core.errors import DagsterInvariantViolationError, DagsterSolidInvocationError
...
def test_attempted_invocation_in_composition():
    ...
    error_msg = (
        'Attempted to execute solid "basic_solid" within the context of a '
        "`composite_solid` or `pipeline`. When constructing a pipeline, solids should be invoked "
        "with no arguments."
    )
    with pytest.raises(
