...
def instance_with_schedules(external_repo_context, overrides=None):
    with schedule_instance(overrides) as instance:
        with GrpcServerRegistry(wait_for_processes_on_exit=True) as grpc_server_registry:
...
def test_bad_load_repository_location(capfd):
    with schedule_instance() as instance, GrpcServerRegistry() as grpc_server_registry:
