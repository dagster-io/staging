...
@pytest.mark.parametrize("external_repo_context", repos())
def test_schedule_without_timezone(external_repo_context):
    with mock_system_timezone("Europe/London"):
        with instance_with_schedules(external_repo_context) as (
            ...
            initial_datetime = pendulum.create(
                year=2019, month=2, day=27, hour=0, minute=0, second=0, tz="Europe/London"
            )
            ...
            with pendulum.test(initial_datetime):
                ...
                #            assert (
                #                "Using the system timezone, Europe/London, for daily_schedule_without_timezone as it did not specify an execution_timezone in its definition. "
                ...
                expected_datetime = pendulum.create(
                    year=2019, month=2, day=27, tz="Europe/London"
                ).in_tz("UTC")
                ...
                validate_run_started(
                    ...
                    partition_time=pendulum.create(2019, 2, 26, tz="Europe/London"),
                )
