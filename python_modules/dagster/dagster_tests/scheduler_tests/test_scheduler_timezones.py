...
def test_non_utc_timezone_run(external_repo_context, capfd):
    ...
    with instance_with_schedules(external_repo_context) as (
        ...
        freeze_datetime = create_pendulum_time(2019, 2, 27, 23, 59, 59, tz="US/Central").in_tz(
            "US/Pacific"
        )
...
def test_differing_timezones(external_repo_context):
    ...
    with instance_with_schedules(external_repo_context) as (
        ...
        freeze_datetime = create_pendulum_time(2019, 2, 27, 23, 59, 59, tz="US/Eastern").in_tz(
            "US/Pacific"
        )
        with pendulum.test(freeze_datetime):
            external_schedule = external_repo.get_external_schedule("daily_central_time_schedule")
...
def test_different_days_in_different_timezones(external_repo_context):
    with instance_with_schedules(external_repo_context) as (
        ...
        freeze_datetime = create_pendulum_time(2019, 2, 27, 22, 59, 59, tz="US/Central").in_tz(
            "US/Pacific"
        )
        with pendulum.test(freeze_datetime):
...
def test_hourly_dst_spring_forward(external_repo_context):
    ...
    with instance_with_schedules(external_repo_context) as (
        ...
        # 1AM CST
        freeze_datetime = create_pendulum_time(2019, 3, 10, 1, 0, 0, tz="US/Central").in_tz(
            "US/Pacific"
        )
        ...
        with pendulum.test(freeze_datetime):
            ...
            expected_datetimes_utc = [
                create_pendulum_time(2019, 3, 10, 4, 0, 0, tz="US/Central").in_tz("UTC"),
                create_pendulum_time(2019, 3, 10, 3, 0, 0, tz="US/Central").in_tz("UTC"),
                create_pendulum_time(2019, 3, 10, 1, 0, 0, tz="US/Central").in_tz("UTC"),
            ]
...
def test_hourly_dst_fall_back(external_repo_context):
    ...
    with instance_with_schedules(external_repo_context) as (
        ...
        # 12:30 AM CST
        freeze_datetime = create_pendulum_time(2019, 11, 3, 0, 30, 0, tz="US/Central").in_tz(
            "US/Pacific"
        )
...
def test_daily_dst_spring_forward(external_repo_context):
    ...
    with instance_with_schedules(external_repo_context) as (
        ...
        # Night before DST
        freeze_datetime = create_pendulum_time(2019, 3, 10, 0, 0, 0, tz="US/Central").in_tz(
            "US/Pacific"
        )
...
def test_daily_dst_fall_back(external_repo_context):
    ...
    with instance_with_schedules(external_repo_context) as (
        ...
        # Night before DST
        freeze_datetime = create_pendulum_time(2019, 11, 3, 0, 0, 0, tz="US/Central").in_tz(
            "US/Pacific"
        )
...
def test_execute_during_dst_transition_spring_forward(external_repo_context):
    ...
    with instance_with_schedules(external_repo_context) as (
        ...
        # Day before DST
        freeze_datetime = create_pendulum_time(2019, 3, 9, 0, 0, 0, tz="US/Central").in_tz(
            "US/Pacific"
        )
        ...
        with pendulum.test(freeze_datetime):
            ...
            expected_datetimes_utc = [
                create_pendulum_time(2019, 3, 11, 2, 30, 0, tz="US/Central").in_tz("UTC"),
                create_pendulum_time(2019, 3, 10, 3, 00, 0, tz="US/Central").in_tz("UTC"),
                create_pendulum_time(2019, 3, 9, 2, 30, 0, tz="US/Central").in_tz("UTC"),
            ]
...
def test_execute_during_dst_transition_fall_back(external_repo_context):
    with instance_with_schedules(external_repo_context) as (
        ...
        freeze_datetime = create_pendulum_time(2019, 11, 2, 0, 0, 0, tz="US/Central").in_tz(
            "US/Pacific"
        )
