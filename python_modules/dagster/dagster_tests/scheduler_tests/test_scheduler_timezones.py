...
def test_different_timezone_run(external_repo_context, capfd):
    # Schedules run in US/Central time (on a server in Pacific time)
...
# A scheduler that runs late enough in the day that the day in UTC is different than it is in UTC
@pytest.mark.parametrize("external_repo_context", [default_repo, grpc_repo])
