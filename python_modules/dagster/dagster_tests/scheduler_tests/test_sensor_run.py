...
def test_step_selection(external_repo_context, capfd):
    ...
    with instance_with_sensors(external_repo_context) as (instance, external_repo):
        with pendulum.test(freeze_datetime):
            ...
            assert (
                captured.out
                == """2019-02-27 17:59:59 - SensorDaemon - INFO - Checking for new runs for the following sensors: step_selection_sensor
2019-02-27 17:59:59 - SensorDaemon - INFO - Sensor returned false for step_selection_sensor, skipping
"""
            )
