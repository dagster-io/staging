...
from dagster.core.definitions.sensor import SensorRunParams, SensorTickData
...
from dagster.scheduler.scheduler import get_default_scheduler_logger
from dagster.scheduler.sensor import launch_sensor_runs
...
def simple_sensor(context):
    if not context.last_evaluation_time or not int(context.last_evaluation_time) % 2:
        return SensorTickData()

    return SensorTickData(run_params=[SensorRunParams(run_config={}, tags={})])
...
def execution_key_sensor(_context):
    return SensorTickData(
        run_params=[SensorRunParams(run_config={}, tags={}, execution_key="only_once")]
    )
...
def sensor_instance(overrides=None):
...
def instance_with_sensors(external_repo_context, overrides=None):
    with sensor_instance(overrides) as instance:
...
def test_simple_sensor(external_repo_context, capfd):
    ...
    with instance_with_sensors(external_repo_context) as (instance, external_repo):
        with pendulum.test(freeze_datetime):
            ...
            launch_sensor_runs(instance, get_default_scheduler_logger())
            ...
            assert (
                captured.out
                == """2019-02-27 17:59:59 - dagster-scheduler - INFO - Checking for new runs for the following sensors: simple_sensor
2019-02-27 17:59:59 - dagster-scheduler - INFO - Sensor returned false for simple_sensor, skipping
"""
            )
        ...
        with pendulum.test(freeze_datetime):
            ...
            assert (
                captured.out
                == """2019-02-27 18:00:00 - dagster-scheduler - INFO - Checking for new runs for the following sensors: simple_sensor
2019-02-27 18:00:00 - dagster-scheduler - INFO - Launching run for simple_sensor
2019-02-27 18:00:00 - dagster-scheduler - INFO - Completed launch of run {run_id} for simple_sensor
""".format(
                    run_id=run.run_id
                )
            )
    ...
    with instance_with_sensors(external_repo_context) as (instance, external_repo):
        with pendulum.test(freeze_datetime):
            ...
            validate_tick(
                ...
                None,
                "Error occurred during the execution of sensor_tick_fn for sensor error_sensor",
            )
