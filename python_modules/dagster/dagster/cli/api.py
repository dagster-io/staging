    ...
    with (
        ...
        if args.should_verify_step:
            ...
            step_stats = instance.get_run_step_stats(
                pipeline_run.run_id, step_keys=[args.step_keys_to_execute]
            )
            for step_key in args.step_keys_to_execute:
                step_stat_for_key = next(
                    [step_stat for step_stat in step_stats if step_stat.step_key == step_key], None
                )
