...
def check_repo_and_scheduler(repository, instance):
    ...
    no_scheduler_configured_message = (
        "A DagsterCommandLineScheduler scheduler must be configured to run sensor commands. "
        "You must add the following lines in your dagster.yaml file, located in the $DAGSTER_HOME "
        "directory.:\n\n"
        "scheduler:\n"
    ...
    with DagsterInstance.get() as instance:
        with get_external_repository_from_kwargs(cli_args) as external_repo:
            ...
            if running_filter:
                ...
                jobs = instance.all_stored_job_state(
                    external_repo.get_external_origin_id(), JobType.SENSOR
                )
        ...
        with get_external_repository_from_kwargs(cli_args) as external_repo:
            ...
            try:
                for external_sensor in external_repo.get_external_sensors():
                    existing_job_state = instance.get_job_state(
                        external_sensor.get_external_origin_id()
                    )
                    if not existing_job_state:
                        job_state = JobState(
                            external_sensor.get_external_origin(), JobType.SENSOR, JobStatus.STOPPED
                        )
                        instance.add_job_state(job_state)

                external_sensor_origin_ids = {
                    s.get_external_origin_id() for s in external_repo.get_external_sensors()
                }
