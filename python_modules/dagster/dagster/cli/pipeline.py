...
def execute_backfill_command(cli_args, print_fn, instance=None):
    ...
    # partition sets can come from either the repository itself or the schedules on the repository
    # (in the latter case we can get the mode and solid selection from the schedule)
    pipeline_partition_set_names = merge_dicts(
        {
        ...
        {
            external_schedule.partition_set_name: external_schedule
            for external_schedule in external_repo.get_external_schedules()
            if external_schedule.pipeline_name == external_pipeline.name
            and external_schedule.partition_set_name
        },
    )
    ...
    partition_set_or_schedule = pipeline_partition_set_names.get(partition_set_name)

    if not partition_set_or_schedule:
        raise click.UsageError('No partition set found named `{}`'.format(partition_set_name))

    mode = partition_set_or_schedule.mode
    solid_selection = partition_set_or_schedule.solid_selection
