...
def create_pipeline_cli_group():
    ...
    group.add_command(pipeline_print_versions_command)
    return group
...
@click.command(
    ...
    help="Display the freshness of memoized results for the given pipeline.\n\n{instructions}".format(
        instructions=get_pipeline_in_same_python_env_instructions("print_versions")
    ),
)
@python_pipeline_target_argument
@click.option(
...
def pipeline_print_versions_command(**kwargs):
    with DagsterInstance.get() as instance:
        execute_print_versions_command(instance, kwargs)


def execute_print_versions_command(instance, kwargs):
    check.inst_param(instance, "instance", DagsterInstance)
    ...
    table = []
    for step_output_handle, version in step_output_versions.items():
        if (pipeline_name, step_output_handle) in step_output_addresses:
            address = step_output_addresses[(pipeline_name, step_output_handle)]
            table.append(
                [
                    "{key}.{output}".format(
