...
def execute_launch_command(instance, kwargs):
    ...
    if kwargs.get('config'):
        ...
        config = get_run_config_from_file_list(config_file_list)
    else:
...
def get_config_from_args(kwargs):
    if kwargs.get('config_yaml') is None:
        return {}
    try:
        return json.loads(kwargs.get('config_yaml'))
    except JSONDecodeError:
        raise click.UsageError(
            'Invalid JSON-string given for `--config_yaml`: {}\n\n{}'.format(
                kwargs.get('config_yaml'),
