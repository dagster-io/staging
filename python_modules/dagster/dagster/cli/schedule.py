...
from dagster.core.scheduler import ScheduleStatus, ScheduleTickStatus, get_schedule_change_set
...
def scheduler_command():
    ...
    for schedule_state in schedules:
        ...
        # TODO support crontabs set in a specific timezone (for now assume everything is in UTC)
        crontab = Crontab(schedule_state.cron_schedule)
        ...
        schedule_time_utc = cron_tab.next(
            crontab.previous(start_time_utc, delta=False), delta=False
        )
        ...
        while schedule_time_utc < end_time_utc:
            ...
            schedule_time_utc = cron_tab.next(schedule_time_utc, delta=False)
...
def _schedule_run_at_time(
    ...
    with _schedule_tick_state(
        ...
        try:
            launched_run = instance.launch_run(run_to_launch.run_id, external_pipeline)
