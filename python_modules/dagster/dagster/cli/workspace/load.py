...
def load_workspace_from_yaml_paths(yaml_paths, python_user_process_api):
    ...
    for workspace_config, yaml_path in zip(workspace_configs, yaml_paths):
        ...
        new_origins = {
            ...
            for origin in _repo_location_origins_from_config(
                workspace_config, yaml_path, python_user_process_api
            )
        }

        origins_by_name = merge_dicts(origins_by_name, new_origins)
...
def load_workspace_from_config(workspace_config, yaml_path, python_user_process_api):
    return Workspace(
        _repo_location_origins_from_config(workspace_config, yaml_path, python_user_process_api)
    )
...
def _repo_location_origins_from_config(workspace_config, yaml_path, python_user_process_api):
    ...
    for location_config in workspace_config["load_from"]:
        location_origins.append(
            _location_origin_from_location_config(
...
def _get_python_file_config_data(python_file_config, yaml_path):
    if isinstance(python_file_config, six.string_types):
        return (rebase_file(python_file_config, yaml_path), None, python_file_config, None)

    if python_file_config.get("location_name"):
        location_name = python_file_config.get("location_name")
    elif python_file_config.get("attribute"):
        location_name = "{relative_path}:{attribute}".format(
            relative_path=python_file_config.get("relative_path"),
            attribute=python_file_config.get("attribute"),
        )
    else:
        location_name = python_file_config.get("relative_path")
