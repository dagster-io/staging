...
def wait_for_grpc_server(ipc_output_file, timeout=3):
    event_list = list(ipc_read_event_stream(ipc_output_file, timeout=timeout))  # Timeout??
    check.invariant(len(event_list) == 1)
...
def open_server_process(
    ...
    output_file = os.path.join(
        get_system_temp_directory(), "grpc-server-{uuid}".format(uuid=uuid.uuid4().hex)
    )
