...
def watch_grpc_server_thread(
    ...
    The expectation is that on the on_updated and on_error events, the subscibers dispose of
    this thread, create a new thread, and subscribe to events on the new thread. These two events are
    called at most once, while `on_disconnected` and `on_reconnected` may be called multiple times
    in order to be properly handle intermittent network failures.
    """
    ...
    watch_interval = check.opt_int_param(watch_interval, "watch_interval", WATCH_INTERVAL)
