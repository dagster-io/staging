...
class _TagLimitsCounter:
    def __init__(self, tag_limits, in_progress_runs):
        self._tag_limits = tag_limits
    ...
    def _get_limited_tags(self, run):
        tag_values = []
        for tag_value in run.tags.items():
            if tag_value in self._tag_limits:
    ...
    def is_run_blocked(self, run):
        limited_tags = self._get_limited_tags(run)
        for tag_value in limited_tags:
            if self._in_progress_limit_counts[tag_value] >= self._tag_limits[tag_value]:
                return True
        return False
...
class QueuedRunCoordinatorDaemon(DagsterDaemon):
    ...
    @experimental
    def __init__(self, instance, interval_seconds, max_concurrent_runs, tag_limits=None):
        ...
        self._tag_limits = check.opt_dict_param(tag_limits, "tag_limits")
    ...
    def run_iteration(self):
        ...
        num_dequeued_runs = 0
        tag_limits_counter = _TagLimitsCounter(self._tag_limits, in_progress_runs)
        for run in sorted_runs:
            if num_dequeued_runs >= max_runs_to_launch:
                break

            if tag_limits_counter.is_run_blocked(run):
                continue

            self._dequeue_run(run)
            tag_limits_counter.update_counters_with_launched_run(run)
            num_dequeued_runs += 1
    ...
    def _get_in_progress_runs(self):
        # Note: should add a maximum fetch limit https://github.com/dagster-io/dagster/issues/3339
        return self._instance.get_runs(filters=PipelineRunsFilter(statuses=IN_PROGRESS_STATUSES))
