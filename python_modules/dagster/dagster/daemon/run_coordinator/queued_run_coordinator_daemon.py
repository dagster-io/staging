...
from dagster.utils.external import external_pipeline_from_run_with_repository_location_handle
...
class _RepositoryLocationHandleManager:
    ...
    def get_external_pipeline_from_run(self, pipeline_run):
        ...
        if origin_id not in self._locations:
            handle_ctx = RepositoryLocationHandle.create_from_repository_location_origin(
                repo_location_origin
            )
            self._locations[origin_id] = (handle_ctx, handle_ctx.__enter__())

        _, handle = self._locations[origin_id]
        return external_pipeline_from_run_with_repository_location_handle(pipeline_run, handle)

    def close(self):
        for handle_ctx, _ in self._locations.values():
...
class QueuedRunCoordinatorDaemon(DagsterDaemon):
    ...
    def __init__(
        ...
        self._repo_location_handle_manager = None
    ...
    def run_iteration(self):
        ...
        self._repo_location_handle_manager = _RepositoryLocationHandleManager()
        ...
        self._repo_location_handle_manager.close()
    ...
    def _dequeue_run(self, run):
        external_pipeline = self._repo_location_handle_manager.get_external_pipeline_from_run(run)
