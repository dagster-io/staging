    ...
    def run_iteration(self, curr_time):
        ...
        for daemon in self.daemons:
            ...
            if (not daemon.daemon_type() in self._last_heartbeat_times) or (
                ...
                self._last_iteration_times[daemon.daemon_type()] = curr_time
                daemon_generators.append((daemon, daemon.run_iteration()))
                # print(daemon.daemon_type())
                ...
                self._current_iteration_exceptions[daemon.daemon_type()] = []
        ...
        while len(daemon_generators) > 0:
            ...
            try:
                ...
                if error_info:
                    self._current_iteration_exceptions[daemon.daemon_type()].append(error_info)
        ...
        if (not daemon_type in self._last_heartbeat_times) or (
            ...
            self._instance.add_daemon_heartbeat(
                DaemonHeartbeat(
                    ...
                    daemon.daemon_type(),
                    self._daemon_uuid,
                    self._last_iteration_exceptions[daemon.daemon_type()],
                )
            )
