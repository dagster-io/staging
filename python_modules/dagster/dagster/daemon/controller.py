...
def get_daemon_status(instance, daemon_type, curr_time=None):
    ...
    if daemon_type not in _required_daemons(instance):
        return DaemonStatus(required=False, healthy=None, last_heartbeat=None)

    # daemon not present
    heartbeats = instance.get_daemon_heartbeats()
    if daemon_type not in heartbeats:
        return DaemonStatus(required=True, healthy=False, last_heartbeat=None)
    ...
    return DaemonStatus(required=True, healthy=healthy, last_heartbeat=heartbeats[daemon_type])
