...
class DagsterDaemon:
    ...
    def run_loop(self, daemon_uuid, daemon_shutdown_event, until=None):
        ...
        with DagsterInstance.get() as instance:
            loop_expired = False
            while not daemon_shutdown_event.is_set() and not loop_expired:
                curr_time = pendulum.now("UTC")
                if until and curr_time > until:
                    loop_expired = True
    ...
    def _run_iteration(self, instance, daemon_uuid, daemon_shutdown_event, until=None):
        ...
        loop_expired = False
        while not loop_expired:
            loop_expired = until and pendulum.now("UTC") > until
            try:
