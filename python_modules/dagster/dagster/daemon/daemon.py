...
ERROR_TIME_LIMIT = 300  # Show all errors that happened in the last 5 minutes
ERROR_LIMIT = 5  # Show at most 5 errors
...
class DagsterDaemon(AbstractContextManager):
        ...
        try:
            while (not daemon_shutdown_event.is_set()) and (
                ...
                try:
                    ...
                    try:
                        self._check_add_heartbeat(instance, daemon_uuid, heartbeat_interval_seconds)
                    except Exception:  # pylint: disable=broad-except
                        self._logger.error(
    ...
    def _check_add_heartbeat(self, instance, daemon_uuid, heartbeat_interval_seconds):
        error_time_limit = pendulum.now("UTC").subtract(seconds=ERROR_TIME_LIMIT)

        self._errors = self._errors[:ERROR_LIMIT]

        self._errors = [
            (error, timestamp)
            for (error, timestamp) in self._errors
            if timestamp >= error_time_limit
        ]

        first_time_logging_error = self._errors and not self._first_error_logged
        ...
        if last_stored_heartbeat and last_stored_heartbeat.daemon_id != daemon_uuid:
