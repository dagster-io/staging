...
def create_offset_partition_selector(execution_time_to_partition_fn):
    ...
    def offset_partition_selector(context, partition_set_def):
        ...
        if not partitions:
            return SkipReason(
                f"The partition {partition_time.isoformat()} precedes the earliest schedule tick "
                f"{context.scheduled_execution_time.isoformat()}. "
                "Verify your schedule's start_date is correct."
            )

        if partition_time > partitions[-1].value:
            return SkipReason(
                f"The partition {partition_time.isoformat()} is after the last schedule tick "
                f"{context.scheduled_execution_time.isoformat()}. "
