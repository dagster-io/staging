...
class TestScheduleStorage:
    ...
    @staticmethod
    def fake_external_repository_origin():
        return ExternalRepositoryOrigin(
            GrpcServerRepositoryLocationOrigin(host="fake", port=1234), "fake_repository"
        )
    ...
    def build_schedule(
        ...
        return JobState(
            cls.fake_external_repository_origin().get_job_origin(schedule_name),
            JobType.SCHEDULE,
            status,
    ...
    def build_sensor(cls, sensor_name, status=JobStatus.STOPPED):
        external_job_origin = cls.fake_external_repository_origin().get_job_origin(sensor_name)
    ...
    def test_basic_schedule_storage(self, storage):
        ...
        schedules = storage.all_stored_job_state(
            self.fake_external_repository_origin().get_id(), JobType.SCHEDULE,
        )
    ...
    def test_basic_job_storage(self, storage):
        ...
        jobs = storage.all_stored_job_state(self.fake_external_repository_origin().get_id())
        assert len(jobs) == 1
