...
class TestPostgresInstance:
    ...
    @staticmethod
    def clean_run_storage(conn_string, autocreate_tables=True):
        check.invariant(
            TestPostgresInstance.dagster_postgres_installed(),
            "dagster_postgres must be installed to test with postgres",
        )
        ...
        storage = PostgresRunStorage.create_clean_storage(
            conn_string, autocreate_tables=autocreate_tables
        )
        assert storage
        return storage

    @staticmethod
    def clean_event_log_storage(conn_string, autocreate_tables=True):
        check.invariant(
            TestPostgresInstance.dagster_postgres_installed(),
            "dagster_postgres must be installed to test with postgres",
        )
    ...
    @staticmethod
    def clean_schedule_storage(conn_string, autocreate_tables=True):
        check.invariant(
            TestPostgresInstance.dagster_postgres_installed(),
            "dagster_postgres must be installed to test with postgres",
        )
