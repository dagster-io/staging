...
def _pack_value(val: Any, whitelist_map: WhitelistMap) -> Any:
    ...
    if isinstance(val, set):
        return {"__set__": [_pack_value(item, whitelist_map) for item in sorted(list(val))]}
    if isinstance(val, frozenset):
        return {"__frozenset__": [_pack_value(item, whitelist_map) for item in sorted(list(val))]}
