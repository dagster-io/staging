...
def _validate_shape_config(context, config_value, check_for_extra_incoming_fields):
    ...
    for field_snap in context.config_type_snap.fields:
        ...
        if aliased_name and aliased_name in config_value and name in config_value:
            field_errors.append(
                create_field_substitution_collision_error(
                    context.for_field_snap(field_snap), name=name, aliased_name=aliased_name
                )
            )
        elif name in config_value or field_aliases.get(name, name) in config_value:
            config_value_at_name = (
                config_value[name] if name in config_value else config_value[field_aliases[name]]
            )
...
def _check_for_extra_incoming_fields(
    context, defined_field_names, incoming_field_names, field_aliases=None
):
    field_aliases = check.opt_dict_param(field_aliases, "field_aliases")
    extra_fields = list(incoming_field_names - defined_field_names)

    # Check for fields that are actually aliases of other fields.
    extra_fields = [field_name for field_name in extra_fields if field_name not in field_aliases.values()]
...
def _compute_missing_fields_error(context, field_snaps, incoming_fields, field_aliases=None):
    missing_fields = []
    ...
    for field_snap in field_snaps:
        if (
            ...
            and field_aliases.get(field_snap.name, field_snap.name) not in incoming_fields
        ):
