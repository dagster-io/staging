...
class DagsterEvaluationErrorReason(Enum):
    ...
    FIELD_SUBSTITUTION_COLLISION = "FIELD_SUBSTITUTION_COLLISION"
...
class FieldSubstitutionCollisionErrorData(NamedTuple):
    field_name: str
...
ERROR_DATA_TYPES = (
    ...
    FieldSubstitutionCollisionErrorData,
)
...
def create_field_substitution_collision_error(context, name, aliased_name):
    ...
    return EvaluationError(
        stack=context.stack,
        reason=DagsterEvaluationErrorReason.FIELD_SUBSTITUTION_COLLISION,
        message=f"Received both field '{name}' and field '{aliased_name}' in config. Please use one or the other.",
        error_data=FieldSubstitutionCollisionErrorData(
            field_name=name, aliased_field_name=aliased_name
        ),
    )
