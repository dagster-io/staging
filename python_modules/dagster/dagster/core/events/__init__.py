...
class DagsterEventType(Enum):
    ...
    PIPELINE_CANCELLING = "PIPELINE_CANCELLING"
...
FAILURE_EVENTS = {
    ...
    DagsterEventType.STEP_FAILURE,
    DagsterEventType.PIPELINE_CANCELLED,
}
...
PIPELINE_EVENTS = {
    ...
    DagsterEventType.PIPELINE_FAILURE,
    DagsterEventType.PIPELINE_CANCELLING,
    DagsterEventType.PIPELINE_CANCELLED,
}
...
class DagsterEvent(
    ...
    @staticmethod
    def pipeline_cancelled(pipeline_context, error_info=None):
        return DagsterEvent.from_pipeline(
            DagsterEventType.PIPELINE_CANCELLED,
            pipeline_context,
            message='Execution of pipeline "{pipeline_name}" cancelled.'.format(
                pipeline_name=pipeline_context.pipeline_def.name
            ),
            event_specific_data=PipelineCancelledData(
                check.opt_inst_param(error_info, "error_info", SerializableErrorInfo)
            ),
        )
...
@whitelist_for_serdes
class PipelineCancelledData(namedtuple("_PipelineCancelledData", "error")):
    def __new__(cls, error):
        return super(PipelineCancelledData, cls).__new__(
            cls, error=check.opt_inst_param(error, "error", SerializableErrorInfo)
        )
