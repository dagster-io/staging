...
class ObjectStoreIntermediateStorage(IntermediateStorage):
    def __init__(
        ...
        # TODO yuhan: reexecution dones't work for externally stored intermediates
        # -> cross-run intermediate records (instance scoped)
        # get from event logs OR persist it somewhere so a new run can pick it up
    ...
    def get_intermediate(self, context, dagster_type=None, step_output_handle=None, address=None):
        ...
        if step_output_handle in self.external_intermediates:
            ...
            _address = self.external_intermediates[step_output_handle]
