...
class ObjectStoreBackcompatObjectManager(PickledObjectFilesystemObjectManager):
    ...
    def __init__(self, object_store, base_dir=None):
    ...
    def handle_output(self, context, obj):
        ...
        if serialization_strategy:
            self.object_store.set_object(
                key=filepath, obj=obj, serialization_strategy=serialization_strategy
            )
            return
        # Ensure path exists
        mkdir_p(os.path.dirname(filepath))
    ...
    def load_input(self, context):
        ...
        serialization_strategy = context.dagster_type.serialization_strategy
        if serialization_strategy:
            obj, _ = self.object_store.get_object(
                key=filepath, serialization_strategy=serialization_strategy
            )
            return obj
