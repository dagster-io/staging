...
class SqlRunStorage(RunStorage):  # pylint: disable=no-init
    ...
    def handle_run_event(self, run_id, event):
        ...
        lookup = {
            ...
            DagsterEventType.PIPELINE_DEQUEUED: PipelineRunStatus.NOT_STARTED,
        }
    ...
    def add_daemon_heartbeat(self, daemon_heartbeat):
        with self.connect() as conn:
            subquery = (
                DaemonHeartbeatsTable.select()
                .where(DaemonHeartbeatsTable.c.daemon_type == daemon_heartbeat.daemon_type)
                .alias("foo")
            )
            count = self.fetchone(db.select([db.func.count()]).select_from(subquery))[0]

            if count == 0:
                # NOTE: possible race condition if another daemon comes alive and inserts first
                conn.execute(
                ...
                raise DagsterInvariantViolationError(
                    f"daemon_type column should be unique. Got {count} rows"
                )
