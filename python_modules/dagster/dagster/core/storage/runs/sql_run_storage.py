...
class SqlRunStorage(RunStorage):  # pylint: disable=no-init
    ...
    def handle_run_event(self, run_id, event):
        ...
        with self.connect() as conn:
            ...
            update_query = self.apply_additional_filters(update_query, RunsTable).values(
    ...
    def _runs_query(self, filters=None, cursor=None, limit=None, columns=None):
        ...
        query = self.apply_additional_filters(base_query, RunsTable)
    ...
    def get_run_by_id(self, run_id):
        ...
        query = self.apply_additional_filters(query, RunsTable)
        rows = self.fetchall(query)
    ...
    def get_run_tags(self):
        ...
        query = self.apply_additional_filters(query, RunTagsTable)
        rows = self.fetchall(query)
        for r in rows:
            result[r[0]].add(r[1])
