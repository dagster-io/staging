...
class VersionedAssetStore(AssetStore):
    @abstractmethod
    def set_asset_with_version(self, context, obj, version):
        """Persist the given data object to be associated with the given version."""

    def set_asset(self, context, obj):
        step_output_handle = StepOutputHandle(context.step_key, context.output_name)
        self.set_asset_with_version(
            context, obj, context.execution_plan.step_output_versions[step_output_handle],
        )

    @abstractmethod
    def get_asset_with_version(self, context, version):
        """Given a step output handle and version, retrieve the corresponding data object if it exists."""

    def get_asset(self, context):
        step_output_handle = StepOutputHandle(context.step_key, context.step_output)
        return self.get_asset_with_version(
            context, context.execution_plan.step_output_versions[step_output_handle],
        )

    @abstractmethod
    def has_asset_with_version(self, context, version):
        """Returns True if asset is stored for given version, False if not."""
...
class VersionedPickledObjectFilesystemAssetStore(VersionedAssetStore):
    ...
    def _get_path(self, step_key, step_output_name, version):
        # automatically construct filepath
        check.str_param(step_key, "step_output_handle")
        check.str_param(step_output_name, "step_output_name")

        return os.path.join(self.base_dir, step_key, step_output_name, version)
    ...
    def set_asset_with_version(self, context, obj, version):
        ...
        filepath = self._get_path(context.step_key, context.output_name, version)
