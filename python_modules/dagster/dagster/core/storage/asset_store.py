...
class VersionedAssetStore(AssetStore):
    @abstractmethod
    def set_asset_with_version(self, context, step_output_handle, obj, asset_metadata, version):
    ...
    @abstractmethod
    def get_asset_with_version(self, context, step_output_handle, asset_metadata, version):
    ...
    @abstractmethod
    def has_asset_with_version(self, step_output_handle, asset_metadata, version):
...
class VersionedPickledObjectFilesystemAssetStore(VersionedAssetStore):
    ...
    def _get_path(self, step_output_handle, version):
        # automatically construct filepath
        check.inst_param(step_output_handle, "step_output_handle", StepOutputHandle)

        return os.path.join(
            self.base_dir, step_output_handle.step_key, step_output_handle.output_name, version
        )

    def set_asset_with_version(self, context, step_output_handle, obj, _asset_metadata, version):
        ...
        filepath = self._get_path(step_output_handle, version)
    ...
    def get_asset_with_version(self, context, step_output_handle, _asset_metadata, version):
        """Unpickle the file and Load it to a data object."""
        check.inst_param(step_output_handle, "step_output_handle", StepOutputHandle)
        version = check.str_param(version, "version")
    ...
    def has_asset_with_version(self, step_output_handle, _asset_metadata, version):
