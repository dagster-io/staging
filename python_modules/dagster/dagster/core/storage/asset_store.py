...
class AssetAddress(six.with_metaclass(ABCMeta)):
    """
    ...
    def __init__(self, asset_id, asset_metadata=None):
...
class PickledObjectFileystemAssetAddress(
    ...
    """The pointer of the asset that is generated by ``PickledObjectFileystemAssetStore``.

    Args:
        asset_id ([str]): the asset ID.
        asset_metadata (Optional[Dict[str, Any]]): The metadata of the asset store.

    """

    def __new__(cls, asset_id, asset_metadata=None):
...
class PickledObjectFileystemAssetStore(AssetStore):
    ...
    def set_asset(self, context, step_output_handle, obj, _asset_metadata):
        """
        store data object to file and track it as ``PickledObjectFileystemAssetAddress``
        """
        check.inst_param(step_output_handle, "step_output_handle", StepOutputHandle)
        ...
        return PickledObjectFileystemAssetAddress(asset_id=self._get_new_id())
...
class CustomPathPickledObjectFileystemAssetAddress(
    namedtuple("_CustomPathPickledObjectFileystemAssetAddress", "asset_id asset_metadata"),
    AssetAddress,
):
    """The pointer of the asset that is generated by ``CustomPathPickledObjectFileystemAssetStore``.

    Args:
        asset_id ([str]): the asset ID.
        asset_metadata ([Dict[str, Any]]): The metadata of the asset store.
            * ``{'path': str}`` allows the user to specify the file path of the asset.

    """

    def __new__(cls, asset_id, asset_metadata):
        return super(CustomPathPickledObjectFileystemAssetAddress, cls).__new__(
            cls,
            asset_id=check.str_param(asset_id, "asset_id"),
            asset_metadata=check.dict_param(asset_metadata, "asset_metadata", key_type=str),
        )
...
class CustomPathPickledObjectFileystemAssetStore(AssetStore):
    ...
    def set_asset(self, _context, step_output_handle, obj, asset_metadata):
        """
        store data object to file and track it as ``CustomPathPickledObjectFileystemAssetAddress``
        """
        check.inst_param(step_output_handle, "step_output_handle", StepOutputHandle)
