...
class InMemoryEventLogStorage(EventLogStorage, ConfigurableClass):
    ...
    def get_stored_events(
        ...
        stored_event_records = [
            StoredEventRecord(storage_id=event_id, event_record=event)
            for event_id, event in enumerate(filtered_events)
            if after_id is None or event_id > after_id
        ]

        stored_event_records = sorted(
            stored_event_records, key=lambda x: x.storage_id, reverse=not ascending
        )

        if limit:
            stored_event_records = stored_event_records[:limit]
