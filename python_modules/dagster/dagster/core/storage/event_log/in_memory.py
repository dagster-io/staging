...
class InMemoryEventLogStorage(EventLogStorage, AssetAwareEventLogStorage, ConfigurableClass):
    ...
    def get_addresses_for_step_output_versions(self, step_output_versions):
        ...
        Returns:
            Dict[str, str]: For each step output, an address if there is one and
                None otherwise.
        """
        step_output_records = (
            record
        ...
        for record in step_output_records:
            ...
            if address and (
                ...
                latest_version_by_address[address] = (step_output, version, timestamp)

        address_by_output = {
            step_output: address
            for address, (step_output, version, _) in latest_version_by_address.items()
            if (step_output, version) in set(step_output_versions.items())
        }
