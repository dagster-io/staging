...
class SqlEventLogStorage(EventLogStorage):
    ...
    def get_logs_for_run_by_log_id(self, run_id, cursor=None, dagster_event_type=None):
    ...
    def get_logs_for_run(self, run_id, cursor=-1, dagster_event_type=None):
        ...
        Args:
            ...
            dagster_event_type (Optional[DagsterEventType]): the dagster event type to filter the logs.
        """
        check.str_param(run_id, "run_id")
        check.int_param(cursor, "cursor")
        check.invariant(
        ...
        events_by_id = self.get_logs_for_run_by_log_id(run_id, cursor, dagster_event_type)
