...
class SqlEventLogStorage(EventLogStorage):
    ...
    def _apply_filter_limit_to_query(
        self,
        query,
        events_filter=None,
        limit=None,
        ...
        if events_filter.event_type:
            query = query.where(
                SqlEventLogStorageTable.c.dagster_event_type == events_filter.event_type.value
            )

        if events_filter.asset_key:
            query = query.where(
                db.or_(
                    SqlEventLogStorageTable.c.asset_key == events_filter.asset_key.to_string(),
                    SqlEventLogStorageTable.c.asset_key
                    == events_filter.asset_key.to_string(legacy=True),
                )
            )

        if events_filter.asset_partitions:
            query = query.where(
                SqlEventLogStorageTable.c.partition.in_(events_filter.asset_partitions)
            )
        ...
        if apply_cursor_filters:
            ...
            # shards
            if events_filter.before_cursor:
                before_cursor_id = (
        ...
        if events_filter.before_timestamp:
            query = query.where(
                SqlEventLogStorageTable.c.timestamp
                < datetime.utcfromtimestamp(events_filter.before_timestamp)
            )

        if events_filter.after_timestamp:
            query = query.where(
                SqlEventLogStorageTable.c.timestamp
                > datetime.utcfromtimestamp(events_filter.after_timestamp)
            )
    ...
    def get_event_records(
        ...
        check.opt_inst_param(events_filter, "events_filter", EventLogFilter)
        check.opt_int_param(limit, "limit")
        check.bool_param(ascending, "ascending")

        query = db.select([SqlEventLogStorageTable.c.id, SqlEventLogStorageTable.c.event])
        if events_filter and events_filter.asset_key:
            asset_details = self._get_asset_details(events_filter.asset_key)
        else:
            asset_details = None

        query = self._apply_filter_limit_to_query(
            query=query,
