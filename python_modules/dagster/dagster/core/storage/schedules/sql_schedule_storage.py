...
class SqlScheduleStorage(ScheduleStorage):
    ...
    def get_latest_job_tick(self, job_origin_id):
        ...
        query = (
            ...
            .order_by(JobTickTable.c.tick_timestamp.desc())
            .limit(1)
        )
        ...
        with self.connect() as conn:
            try:
                tick_insert = JobTickTable.insert().values(  # pylint: disable=no-value-for-parameter
                    ...
                    tick_timestamp=utc_datetime_from_timestamp(job_tick_data.timestamp),
        ...
        with self.connect() as conn:
            conn.execute(
                ...
                .values(
                    ...
                    tick_timestamp=utc_datetime_from_timestamp(tick.timestamp),
