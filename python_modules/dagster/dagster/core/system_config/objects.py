...
class IntermediateStorageConfig(
    ...
    def from_dict(config=None):
        ...
        if config:
            warnings.warn(
                (
                    'the "storage" and "intermediate_storage" entry in the run config is deprecated, '
                    "and will removed in the Dagster 0.11.0. We have introduced a new feature Object "
                    "Managers, which serve the same purpose as Intermediate Storage and are "
                    'resources that can be configured via the "object_manager" resource key.'
                )
            )
