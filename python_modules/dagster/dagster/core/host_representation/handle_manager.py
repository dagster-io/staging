...
from .handle import SharedGrpcRepositoryLocationHandle
...
class RepositoryLocationHandleManager:
    ...
    def get_handle(self, repository_location_origin):
        ...
        # TODO: More handle memoization here using server ID checks, so we don't need to make
        # the same gRPC calls against the same server repeatedly
        ...
        if isinstance(repository_location_origin, ManagedGrpcPythonEnvRepositoryLocationOrigin):
            ...
            handle = (
                existing_handle
                if existing_handle
                else SharedGrpcRepositoryLocationHandle(
