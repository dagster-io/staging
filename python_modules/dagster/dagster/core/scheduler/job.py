...
class JobTickData(
    namedtuple(
        "_JobTickData",
        "job_origin_id job_name job_type status timestamp run_id error execution_key cursor",
    )
):
    def __new__(
        cls,
        ...
        Keyword Arguments:
            ...
            cursor (Optional[str]): A string that identifies a series of job ticks, associated with
                a single schedule tick or sensor evaluation
        """
    ...
    def with_status(
        self, status, run_id=None, error=None, timestamp=None, execution_key=None, cursor=None
    ):
        check.inst_param(status, "status", JobTickStatus)
        return JobTickData(
