...
class PartitionBackfill(
    namedtuple(
        "_PartitionBackfill",
        (
            "backfill_id origin status partition_names from_failure reexecution_steps tags "
            "timestamp checkpoint error"
        ),
    ),
):
    def __new__(
        ...
        return super(PartitionBackfill, cls).__new__(
            ...
            check.inst_param(origin, "origin", ExternalPartitionSetOrigin),
            check.inst_param(status, "status", BackfillJobStatus),
            ...
            check.opt_str_param(checkpoint, "checkpoint"),
    ...
    def with_checkpoint(self, checkpoint):
        check.str_param(checkpoint, "checkpoint")
        return PartitionBackfill(
            self.backfill_id,
            self.origin,
            self.status,
...
def submit_backfill_runs(instance, backfill_job, partition_names=None):
    check.inst_param(instance, "instance", DagsterInstance)
    check.inst_param(backfill_job, "backfill_job", PartitionBackfill)
    repository_origin = backfill_job.origin.external_repository_origin

    if not partition_names:
        partition_names = backfill_job.partition_names

    with repository_origin.repository_location_origin.create_handle() as repo_location_handle:
        ...
        partition_set_name = backfill_job.origin.partition_set_name
