...
def _get_all_manager_keys(node_def):
    io_manager_keys = {output_def.io_manager_key for output_def in node_def.output_defs}
    if isinstance(node_def, GraphDefinition):
        for inner_node_def in node_def.node_defs:
            io_manager_keys = io_manager_keys.union(_get_all_manager_keys(inner_node_def))
...
def execute_in_process(
    ...
    io_manager_keys = _get_all_manager_keys(node)
    ...
    for key in io_manager_keys:
        if key not in resource_defs:
            resource_defs[key] = mem_io_manager
    ...
    pipeline_def = PipelineDefinition(
        node_defs,
        name=f"pipeline_wraps_[{node.name}]",
        mode_defs=[mode_def],
        dependencies=dependencies,
        system_created_node=True,
    )
