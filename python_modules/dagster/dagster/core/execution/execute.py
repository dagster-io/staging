...
def _create_recorder_callback(recording_dict):
    def _callback(step_key, output):
        recording_dict[(step_key, output.output_name)] = output.value
    ...
    with ephemeral_instance_if_missing(instance) as execute_instance:
        ...
        _execute_run_iterable = ExecuteRunWithPlanIterable(
            ...
            execution_context_manager=PipelineExecutionContextManager(
                ...
                run_config=run_config,
                output_recorder=_create_recorder_callback(recorder)
                if output_recording_enabled
                else None,
            ),
        )
