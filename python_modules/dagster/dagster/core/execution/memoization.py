...
def copy_required_intermediates_for_execution(pipeline_context, execution_plan):
    ...
    output_handles_to_copy = [
        step_output_handle
        for step_output_handle in output_handles_for_current_run.intersection(
            output_handles_from_previous_run
        )
        if not pipeline_context.using_asset_store(step_output_handle)
    ]
