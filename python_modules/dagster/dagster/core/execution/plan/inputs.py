...
class FromStepOutput(
    ...
    def load_input_object(self, step_context):
        ...
        if self.input_def.manager_key:
            loader = getattr(step_context.resources, self.input_def.manager_key)
            yield loader.load(self.get_load_context(step_context))
            ...
            yield object_manager.load(self.get_load_context(step_context))
        else:
            yield step_context.intermediate_storage.get_intermediate(
    ...
    def load_input_object(self, step_context):
        ...
        for inner_source in self.sources:
            ...
            if inner_source.can_load_input_object(step_context):
                for event_or_input_value in inner_source.load_input_object(step_context):
