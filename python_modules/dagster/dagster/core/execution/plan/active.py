...
class ActiveExecution:
    ...
    def __init__(self, execution_plan, retries, sort_key_fn=None):
        ...
        self._dynamic_outputs = defaultdict(lambda: defaultdict(list))
    ...
    def _update(self):
        ...
        for idx, key in enumerate(list(self._pending_resolve)):
            new_step_deps = self._plan.resolve(key, self._dynamic_outputs[key])
    ...
    def mark_success(self, step_key):
        ...
        self._mark_complete(step_key)
        if step_key in self._dynamic_outputs:
    ...
    def handle_event(self, dagster_event):
        ...
        if dagster_event.is_step_failure:
            ...
            if dagster_event.step_output_data.step_output_handle.mapping_key:
                self._dynamic_outputs[dagster_event.step_key][
