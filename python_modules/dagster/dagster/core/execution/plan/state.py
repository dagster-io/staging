...
class KnownExecutionState(
    ...
    def __new__(cls, previous_retry_attempts, dynamic_mappings, all_executable_steps):
        return super(KnownExecutionState, cls).__new__(
            ...
            check.set_param(all_executable_steps, "all_executable_steps", of_type=str),
        )
    ...
    def derive_from_logs(logs: List[EventRecord]) -> "KnownExecutionState":
        ...
        all_executable_steps: Set[str] = set()
        ...
        for log in logs:
            ...
            if event.is_step_success:
                all_executable_steps.add(event.step_key)
        ...
        return KnownExecutionState(
            dict(previous_retry_attempts), successful_dynamic_mappings, all_executable_steps
        )
