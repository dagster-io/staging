...
class StepFailureData(
    NamedTuple(
        ...
        [
            ...
            ("error_setting", ErrorSetting),
        ],
    )
):
    def __new__(cls, error, user_failure_data, error_setting=None):
        return super(StepFailureData, cls).__new__(
            ...
            error_setting=check.opt_inst_param(
                error_setting, "error_setting", ErrorSetting, default=ErrorSetting.FRAMEWORK_ERROR
            ),
        )
    ...
    def error_display_string(self) -> str:
        ...
        """
        if self.error_setting == ErrorSetting.USER_ERROR:
            user_code_error = self.error.cause
            return self.error.message.strip() + ":\n\n" + user_code_error.to_string()
        elif self.error_setting == ErrorSetting.FRAMEWORK_ERROR:
            return self.error.to_string()
        else:
            check.invariant(f"Unexpected error setting: {self.error_setting}")
...
def step_failure_event_from_exc_info(
    ...
    error_setting: Optional[ErrorSetting] = None,
):
