...
# normalize step key?
def deconstruct_step_key(step_key, mappable_key=None):
    # returns step_key, mappable_key, unresolved_key
    m = re.match(r"(.*)\[(.*)\]\.compute", step_key)
    unresolved_suffix = "[?].compute"
    if m:
        if mappable_key:
            return (
                m.group(1) + "[" + mappable_key + "].compute",
                mappable_key,
                m.group(1) + unresolved_suffix,
            )
        else:
            return step_key, m.group(2), m.group(1) + unresolved_suffix
    return step_key, mappable_key, step_key
...
class StepOutputHandle(namedtuple("_StepOutputHandle", "step_key output_name mappable_key")):
    ...
    def __new__(cls, step_key, output_name="result", mappable_key=None):
        # the magic sauce
        _step_key, _mappable_key, _ = deconstruct_step_key(step_key, mappable_key)

        return super(StepOutputHandle, cls).__new__(
            cls,
            step_key=check.str_param(_step_key, "step_key"),
            output_name=check.str_param(output_name, "output_name"),
            mappable_key=check.opt_str_param(_mappable_key, "mappable_key"),
        )

    @property
    def unresolved_step_key(self):
        _step_key, _mappable_key, _unresolved_step_key = deconstruct_step_key(
            self.step_key, self.mappable_key
        )
        return _unresolved_step_key
...
class StepInput(
    ...
    def __new__(
        ...
        is_mappable = (
            source_handles and any([x.mappable_key for x in source_handles])
        ) or is_mappable
            ...
            logging_tags=merge_dicts(
                {
                    ...
                    if not mappable_key
                    else str(solid_handle) + "[" + mappable_key + "]." + key_suffix,
