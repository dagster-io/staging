            ...
            for name, output_def in solid.definition.output_dict.items():
                ...
                if isinstance(step, ExecutionStep):
                    ...
                    step_output_handle = UnresolvedStepOutputHandle(
                        ...
                        step.mapping_step_key,
                        step.mapping_output_name,
                    )
...
StepHandleUnion = (StepHandle, UnresolvedStepHandle, MappedStepHandle)
...
class ExecutionPlan(
    ...
    def resolve(self, mapping_step_key, mappings):
        """Resolve UnresolvedExecutionSteps that depend on mapping_step_key, with the mapped output results"""
        check.str_param(mapping_step_key, "mapping_step_key")
        ...
        for unresolved_step_handle in self.resolvable_map[mapping_step_key]:
            ...
            if unresolved_step_handle in self.step_handles_to_execute:
                ...
                resolved_steps += unresolved_step.resolve(mapping_step_key, mappings)
    ...
    def build_subset_plan(self, step_keys_to_execute):
        ...
        for handle in step_handles_to_execute:
            ...
            if handle in self.step_dict:
                ...
                self.resolve(
                    unresolved_step.mapping_step_key,
                    {unresolved_step.mapping_output_name: [handle.mapping_key]},
                )
                check.invariant(
