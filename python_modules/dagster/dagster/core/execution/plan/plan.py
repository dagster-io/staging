...
def should_skip_step(execution_plan, instance, run_id):
    ...
    run-level plan process, e.g. Airflow step execution. Note: this only takes one step at a time.
    ...
    check.invariant(
        len(execution_plan.step_keys_to_execute) == 1,
        "Saw {num_steps} execution steps for task-skipping check, expected only one.".format(
            num_steps=len(execution_plan.step_keys_to_execute)
        ),
    )
