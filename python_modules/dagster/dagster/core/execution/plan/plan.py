...
class _PlanBuilder:
    ...
    def build(self) -> "ExecutionPlan":
        ...
        if pipeline_def.is_using_memoization:
            if self.step_keys_to_execute is not None:
...
class ExecutionPlan(
    ...
    def get_version_for_step_output_handle(self, step_output_handle):
        if self.step_output_versions is None:
            return None
        else:
            return self.step_output_versions[step_output_handle]

    def build_memoized_plan(self, pipeline_def, resolved_run_config, instance):
        """
        Returns:
        ...
        for step in self.steps:
            for output_name in step.step_output_dict.keys():
                ...
                with build_resources(
                    ...
                    instance=instance,
                    resource_config=resolved_run_config.to_dict().get("resources", {}),
    ...
    def rebuild_from_snapshot(pipeline_name, execution_plan_snapshot):
        ...
        if execution_plan_snapshot.step_output_versions is not None:
            step_output_versions = {}
            for step_output_str, version in execution_plan_snapshot.step_output_versions.items():
                step_key, output_name, mapping_key = step_output_str.split("-")
                mapping_key = mapping_key if not mapping_key == "MAPPING_KEY_NO_VAL" else None
                step_output_versions[StepOutputHandle(step_key, output_name, mapping_key)] = version
        else:
            step_output_versions = None
