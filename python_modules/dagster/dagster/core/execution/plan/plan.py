...
def get_step_input_source(
    ...
    if dependency_structure.has_multi_deps(input_handle):
        sources = []
        for idx, handle_or_sigil in enumerate(dependency_structure.get_multi_deps(input_handle)):
            if handle_or_sigil:
                sources.append(FromStepOutput(plan_builder.get_output_handle(handle_or_sigil)))
            else:
