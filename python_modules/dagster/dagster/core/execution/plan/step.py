...
class StepKind(Enum):
    COMPUTE = "COMPUTE"
    # trying this on to appease snapshotting
...
class ExecutionStep(
    ...
    def get_mapping_key(self):
        if isinstance(self.handle, MappedStepHandle):
...
class UnresolvedExecutionStep(
    ...
    def get_execution_dependency_keys(self):
        deps = set()
        for inp in self.step_inputs:
            deps.update([handle.step_key for handle in inp.get_step_output_handle_dependencies()])
        return deps
    ...
    @property
    def mapping_output_name(self):
        keys = set()
        for inp in self.step_inputs:
            if isinstance(inp, UnresolvedStepInput):
                keys.add(inp.mapping_output_name)

        check.invariant(
            len(keys) == 1, "Unresolved step expects one and only one mapping output name"
        )
    ...
    def resolve(self, mapping_step_key, mappings):
        ...
        check.invariant(self.mapping_step_key == mapping_step_key, "mapping step key did not match")

        execution_steps = []
        solid = self.solid

        for output_name, mapped_keys in mappings.items():
            if self.mapping_output_name != output_name:
                continue
