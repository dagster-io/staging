...
from dagster.core.errors import (
    DagsterExecutionInputLoadingError,
    DagsterExecutionOutputHandlingError,
...
def _set_objects(step_context, step_output, step_output_handle, output):
    ...
    with user_code_error_boundary(
        ...
        msg_fn=lambda: """Error occurred during the the handling of step output:
        step key: "{key}"
        output name: "{output_name}"
        """.format(
            key=step_context.step.key, output_name=output_context.name,
        ),
        step_key=step_context.step.key,
...
def _load_input_values(step_context):
    ...
    for step_input in step.step_inputs:
        ...
        with user_code_error_boundary(
            DagsterExecutionInputLoadingError,
            control_flow_exceptions=[Failure, RetryRequested],
            msg_fn=lambda: """Error occurred during the loading of a step input:
                step key: "{key}"
                input name: "{input_name}"
            """.format(
                key=step_context.step.key,
                input_name=step_input.name,  # pylint: disable=cell-var-from-loop
            ),
            step_key=step_context.step.key,
