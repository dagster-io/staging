...
def _value_for_input_source(step_context, input_name, dagster_type, source, check_for_missing):
    ...
    if isinstance(source, FromStepOutput):
        ...
        if (
            dagster_type.kind == DagsterTypeKind.LIST
            or dagster_type.kind == DagsterTypeKind.NULLABLE
        ):
            inner_dagster_type = dagster_type.inner_type
        else:  # This is the case where the fan-in is typed Any
            inner_dagster_type = dagster_type

        values = []
        for inner_source in source.sources:
            value = _value_for_input_source(
                step_context, input_name, inner_dagster_type, inner_source, check_for_missing=True
            )
            if value is not _MISSING_SIGIL:
                values.append(value)
