...
def build_resources(
    ...
    A user can optionally provide a list to the `recorder` object. If provided, `DagsterEvent`s
    yielded during resource spinup and teardown will be appended to this list.
    ...
    Args:
        ...
        recorder (Optional[List[DagsterEvent]]): A list where `DagsterEvent`s yielded during
            resource initialization and teardown will be appended.
        ...
        Examples:
            ...
            .. code-block:: python
                ...
                with build_resources({"basic_resource": basic_resource}) as resource_context:
                    assert resource_context.resource_instance_dict["basic_resource"] == "foo"


                @resource(config_schema{"plant": str})
                def basic_resource_with_config(init_context):
                    plant = init_context.resource_config["plant"]
                    return f"{plant} is a plant"

                recorder = []
                with build_resources(
                    {"basic_resource": basic_resource_with_config},
                    run_config = {
                        "basic_resource": {
                            "config": {
                    ...
                    assert (
                        resource_context.resource_instance_dict["basic_resource"] ==
                        "Cardinal Flower is a plant"
                    )
