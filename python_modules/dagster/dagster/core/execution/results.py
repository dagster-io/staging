...
def _core_resource_generation(
    execution_plan, run_id, resource_defs, resource_managers, mode_def, log_manager
):
...
def generate_resource_from_key(execution_plan, run_id, log_manager):
...
def standalone_resource_init_manager(execution_plan, run_id, log_manager):
    generator = generate_resource_from_key(execution_plan, run_id, log_manager)
    return EventGenerationManager(generator, ScopedResourcesBuilder)


@contextmanager
def initialize_resources(execution_plan, run_id, log_manager):
    resources_manager = standalone_resource_init_manager(execution_plan, run_id, log_manager)
    try:
...
class ExecutionResult:
    ...
    def output_values(self):
        results = {}
        pipeline_def = self._execution_plan.pipeline_def
        log_manager = create_log_manager(
            pipeline_def,
            pipeline_def.get_mode_definition("created"),
            self._execution_plan,
            self._run_id,
        )
        with initialize_resources(
            self._execution_plan, self._run_id, log_manager
        ) as scoped_resources:
            for compute_step_event in self.compute_step_events:
                if compute_step_event.is_successful_output:
                    ...
                    value = self._get_value(
                        scoped_resources.resource_instance_dict,
                        self._execution_plan,
                        output,
                        log_manager,
                    )
    ...
    def _get_value(self, resources, execution_plan, step_output_data, log_manager):
