...
from typing import Any, Deque, Dict, Generator, List, Optional, Set, cast
...
def resource_initialization_manager(
    ...
    resource_instances_to_override: Optional[Dict[str, "InitializedResource"]] = None,
):
...
def _core_resource_initialization_event_generator(
    resource_defs: Dict[str, ResourceDefinition],
    run_config_for_resources: Dict[str, Any],
    ...
    try:
        ...
        for level in toposort(resource_dependencies):
            for resource_name in level:
                ...
                if resource_keys_to_init and not resource_name in resource_keys_to_init:
                    continue

                resource_context = InitResourceContext(
                    resource_def=resource_def,
                    resource_config=run_config_for_resources.get(resource_name, {}).get("config"),
...
def resource_initialization_event_generator(
    ...
    try:
        ...
        yield from _core_resource_initialization_event_generator(
            resource_defs=resource_defs,
            run_config_for_resources=run_config_for_resources,
