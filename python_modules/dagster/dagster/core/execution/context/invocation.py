...
class DirectSolidExecutionContext(SolidExecutionContext):
    ...
    def bind(self, solid_def: SolidDefinition) -> "BoundSolidExecutionContext":
        ...
        return BoundSolidExecutionContext(
            solid_def, solid_config, self.resources, self.instance, self._materializations, self.log
        )
...
def _resolve_bound_config(solid_config: Any, solid_def: SolidDefinition) -> Any:
    ...
    if solid_def.config_field:
        ...
        config_evr = process_config(solid_def.config_field.config_type, solid_config)
...
def _get_default_if_exists(solid_def: SolidDefinition):
    return (
        solid_def.config_field.default_value
        if solid_def.config_field and solid_def.config_field.default_provided
        else None
    )


class BoundSolidExecutionContext(SolidExecutionContext):
    """The solid execution context that is constructed during invocation once config and resources
    have been validated.
    """
