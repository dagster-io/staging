...
class StepExecutionContext(PlanExecutionContext, IStepContext):
    ...
    def _get_source_run_id_from_logs(self, step_output_handle: StepOutputHandle) -> str:
        ...
        # e.g. when parent run didn't yield a StepOutput for the given handle, it could reach here,
        #       but it should have been guarded earlier in the stack
        raise DagsterInvariantViolationError(
            "Could not find persistent artifacts from previous runs for step "
            f"'{step_output_handle.step_key}' output '{step_output_handle.output_name}'."
        )
