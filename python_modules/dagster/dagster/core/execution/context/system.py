...
class SystemStepExecutionContext(SystemExecutionContext):
    ...
    def get_output_context(self, step_output_handle):
        ...
        # outputs config will be a dict if any of the solid's output defs have output manager
        # config and a list otherwise
        if outputs_config:
...
class OutputContext(
    namedtuple("_StepOutputInfo", "step_key name metadata run_id pipeline_name config version")
):
    """
    Attributes:
