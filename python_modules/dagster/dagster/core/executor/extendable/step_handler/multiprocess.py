...
class MultiprocessStepHandler(StepHandler):
    ...
    def launch_steps(self, pipeline_context, step_context, known_state):
        self._term_events[step_context.step.key] = multiprocessing.Event()
        self._active_iters[step_context.step.key] = self._execute_step_out_of_process(
            step_context,
            step_context.step,
            known_state,
        )
        self._steps[step_context.step.key] = step_context.step

    def terminate_steps(self, step_keys: List[str]):
        for step_key in step_keys:
            self._term_events[step_key].set()

    def _execute_step_out_of_process(self, step_context, step, known_state):
        command = MultiprocessExecutorChildProcessCommand(
            run_config=step_context.run_config,
            pipeline_run=step_context.pipeline_run,
            step_key=step.key,
            instance_ref=step_context.instance.get_ref(),
            term_event=self._term_events[step.key],
        ...
        for ret in execute_child_process_command(command):
            if ret is None or isinstance(ret, DagsterEvent):
                ...
                if isinstance(ret, ChildProcessSystemErrorEvent):
                    self._errors[ret.pid] = ret.error_info
            else:
        ...
        for key, step_iter in self._active_iters.items():
            try:
                ...
                yield DagsterEvent.engine_event(
                    ...
                    step_handle=self._steps[key].handle,
                )
                step_failure_event = DagsterEvent.step_failure_event(
                    step_context=self._pipeline_context.for_step(self._steps[key]),
    ...
    def check_step_health(self, pipeline_context, step_context, known_state):
        pass
