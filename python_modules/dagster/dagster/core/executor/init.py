...
class InitExecutorContext(
    namedtuple("InitExecutorContext", "pipeline executor_def pipeline_run executor_config legacy")
):
    ...
    def ensure_multiprocess_safe(self):
        ...
        _check_non_ephemeral_instance(self.legacy["instance"])
        _check_persistent_storage_requirement(
            self.pipeline.get_definition(),
            self.legacy["mode_def"],
            self.legacy["intermediate_storage_def"],
            self.legacy["system_storage_def"],
        )
