...
from dagster.core.execution.plan.step import IExecutionStep
...
class StepDelegatingExecutor(Executor):
    def __init__(
        ...
        self._enable_step_isolation = enable_step_isolation

    @property
    def retries(self):
        return self._retries

    def _execute_isolated(self, pipeline_context, _execution_plan, active_execution):
        stopping = False
        running_steps: Dict[str, IExecutionStep] = {}
    ...
    def _execute_non_isolated(
        self, pipeline_context, execution_plan: ExecutionPlan, active_execution
    ):
        stopping = False
        running_steps: Dict[str, IExecutionStep] = {}

        steps = [
            execution_plan.step_dict[step_handle]
            for step_handle in execution_plan.step_handles_to_execute
        ]
        step_contexts = [pipeline_context.for_step(step) for step in steps]
    ...
    def execute(self, pipeline_context: PlanOrchestrationContext, execution_plan: ExecutionPlan):
        ...
        with execution_plan.start(retry_mode=self.retries) as active_execution:
            if self._enable_step_isolation:
                return self._execute_isolated(pipeline_context, execution_plan, active_execution)
            else:
                return self._execute_non_isolated(
                    pipeline_context, execution_plan, active_execution
                )
