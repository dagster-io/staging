...
class ScheduleDefinition(JobDefinition):
    ...
    def __init__(
        ...
        if execution_fn and (run_config_fn or tags_fn or should_execute or tags or run_config):
            ...
            def _execution_fn(context):
                ...
                yield RunParams(run_config=run_config_fn(context), tags=tags_fn(context))
    ...
    def get_execution_data(self, context):
        ...
        if len(result) == 1:
            check.is_list(result, of_type=(RunParams, RunSkippedData))
            data = result[0]

            if isinstance(data, RunSkippedData):
                return result
            check.inst(data, RunParams)
            return [
                RunParams(
        ...
        check.is_list(result, of_type=RunParams)

        # clone all the run params with the required schedule tags
        return [
            RunParams(
                run_config=data.run_config,
                tags=merge_dicts(data.tags, PipelineRun.tags_for_schedule(self)),
