...
class LoggerDefinition(AnonymousConfigurableDefinition):
    ...
    def initialize(self, init_context: "InitLoggerContext") -> logging.Logger:
        """Using the provided context, call the underlying `logger_fn` and return created logger."""
        from dagster.core.execution.context.logger import InitLoggerContext

        check.inst_param(init_context, "init_context", InitLoggerContext)
...
def build_init_logger_context(
    ...
    return InitLoggerContext(
        logger_config=logger_config, pipeline_def=pipeline_def, logger_def=None, run_id=None
    )
