...
def define_run_config_schema_type(creation_data: RunConfigSchemaCreationData) -> ConfigType:
    ...
    return Shape(
        ...
        field_substitutions={"ops": "solids"},
    )
...
def solid_config_field(fields: Dict[str, Optional[Field]], ignored: bool) -> Optional[Field]:
    ...
    if trimmed_fields:
        if ignored:
            return Field(
                Shape(trimmed_fields, field_substitutions={"ops": "solids"}),
                is_required=False,
                description="This solid is not present in the current solid selection, "
                "the config values are allowed but ignored.",
            )
        else:
            return Field(Shape(trimmed_fields, field_substitutions={"ops": "solids"}))
    else:
        return None
...
def define_solid_dictionary_cls(
    ...
    return Shape(fields, field_substitutions={"solids": "ops"})
