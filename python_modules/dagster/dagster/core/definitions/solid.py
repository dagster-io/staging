...
class CompositeSolidDefinition(ISolidDefinition, IContainSolids):
    ...
    @property
    def has_config_entry(self):
        # if the solid has been @configured, then we ignore child solid configs
        has_child_solid_config = (not self.is_preconfigured) and any(
            [solid.definition.has_config_entry for solid in self.solids]
        )
        return (
