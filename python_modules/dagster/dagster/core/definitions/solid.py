    ...
    def __call__(self, *args, **kwargs) -> Any:
        ...
        if is_in_composition():
            ...
            if self._context_arg_provided:
                check.invariant(
                    len(args) > 0
                    and (args[0] is None or isinstance(args[0], DirectSolidExecutionContext)),
                ...
                check.invariant(
                    len(args) == 0 or not isinstance(args[0], DirectSolidExecutionContext),
                    f"Compute function of solid '{self.name}' has no context argument, but context "
                    "was provided when invoking.",
                )
