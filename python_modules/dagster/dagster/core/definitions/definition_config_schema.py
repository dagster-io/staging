...
def convert_user_facing_definition_schema(potential_schema):
    return (
        None
...
class MappedDefinitionConfigSchema(IDefinitionConfigSchema):
    def __init__(self, current_field, config_mapping_fn):
        self._current_field = current_field
        self.config_mapping_fn = config_mapping_fn

    def as_field(self):
        return self._current_field

    @staticmethod
    def for_configured_definition(configured_definition, config_schema, resolvable_config):
        return MappedDefinitionConfigSchema(
            config_schema.as_field() if config_schema else None,
            _get_wrapped_config_mapping_fn(configured_definition, resolvable_config, config_schema),
        )
