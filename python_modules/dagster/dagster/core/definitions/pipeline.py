...
def _checked_resource_reqs_for_mode(
    ...
    for node_def in node_defs:
        for solid_def in node_def.iterate_solid_defs():
            ...
            # this could potentially be improved by only requiring the root input manager
            # if the input is unsatisfied
            for input_def in solid_def.input_defs:
                if input_def.root_manager_key is not None:
                    resource_reqs.add(input_def.root_manager_key)
                    if input_def.root_manager_key not in mode_resources:
                        raise DagsterInvalidDefinitionError(
                            f'Root input manager "{input_def.root_manager_key}" is required by '
                            f"input {input_def.name} of solid def {solid_def.name}, but is not "
                            f"provided by mode {mode_def.name}"
                        )
