                ...
                if required_resource not in mode_resources:
                    raise DagsterInvalidDefinitionError(
                        (
                            ...
                            f'Resource must be one of {{{", ".join(map(lambda resource: f"{resource}", mode_resources))}}}.'
                        )
                    )
                ...
                if output_def.io_manager_key not in mode_resources:
                    raise DagsterInvalidDefinitionError(
                        (
                            ...
                            f'IO manager must be one of {{{", ".join(map(lambda io_manager: f"{io_manager}", mode_io_managers))}}}.'
                        )
                    )
                ...
                if input_def.root_manager_key:
                    ...
                    if input_def.root_manager_key not in mode_def.resource_defs:
                        raise DagsterInvalidDefinitionError(
                            ...
                            f'provided by mode "{mode_def.name}".'
                            f'Root input manager must be one of {{{", ".join(map(lambda input_manager: f"{input_manager}", mode_root_input_managers))}}}.'
                        )
