...
class EventMetadataEntry(
    ...
    def fspath(
        ...
        if label is not None:
            return EventMetadataEntry.path(path, label)
        else:
            path = cast(str, check.str_param(path, "path"))
            description = cast(str, check.str_param(description, "description"))
            return EventMetadataEntry.path(last_file_comp(path), description)
