            ...
            for input_name, node in invocation.input_bindings.items():
                if isinstance(node, InvokedSolidOutputHandle):
                    ...
                    deps[input_name] = MultiDependencyDefinition(
                        [DependencyDefinition(call.node_name, call.output_name) for call in node]
                    )
                else:
...
class CallableNode(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        # TODO: what does this new one represent
        ...
        check.invariant(
            new_node_name == node_name,
            "old {node_name} new {new_node_name}".format(
                node_name=node_name, new_node_name=new_node_name
            ),
        )
        ...
        if len(self.node_def.output_defs) == 1:
            ...
            return InvokedSolidOutputHandle(new_node_name, output_name)
        ...
        return namedtuple("_{node_def}_outputs".format(node_def=self.node_def.name), outputs)(
            **{output: InvokedSolidOutputHandle(new_node_name, output) for output in outputs}
        )
