    ...
    def _process_argument_node(self, solid_name, output_node, input_name, input_bindings, arg_desc):
        ...
        if isinstance(output_node, (InvokedSolidOutputHandle, InputMappingNode)):
            ...
            raise DagsterInvalidDefinitionError(
                ...
                "output must be unpacked by invoking map or forEach."
            )
...
class InvokedSolidDynamicOutputWrapper:
    ...
    Must be unwrapped by invoking map or forEach.
    """
    ...
    def map(self, fn):
        ...
        if isinstance(result, InvokedSolidOutputHandle):
            ...
            check.failed(
                f"Could not handle output from map function invoked on {self}, received {result}"
            )
    ...
    def __iter__(self):
        raise DagsterInvariantViolationError(
            ...
            'from the solid "{solid}". Use the "map" or "forEach" method on this object to create '
