# This file, and the ability to import lambda_solid from the top level in dagster, will be removed
# in 0.11.0
import warnings
...
def lambda_solid(name=None, description=None, input_defs=None, output_def=None):
    ...
    """
    warnings.warn(
        "Importing @lambda_solid from the top level in dagster is deprecated, and will become "
        "impossible in 0.11.0. In that release, @lambda_solid will become an internal API and "
        "subject to breaking changes. If you want to continue using lambda solid, you should "
        "import it from dagster.core.definitions, rather than from dagster at the top level."
    )
    if callable(name):
