...
class SensorContext:
    ...
    def instance(self) -> DagsterInstance:
        # self._instance_ref should only ever be None when this SensorContext was
...
class SensorDefinition:
    ...
    Args:
        ...
        evaluation_fn (Callable[[SensorContext]]): The core evaluation function for the
            sensor, which is run at an interval to determine whether a run should be launched or
            not. Takes a :py:class:`~dagster.SensorContext`.
    ...
    def __call__(self, *args, **kwargs):
        ...
        if args:
            context = check.opt_inst_param(args[0], context_param_name, SensorContext)
        else:
            ...
            context = check.opt_inst_param(
                kwargs[context_param_name], context_param_name, SensorContext
            )
    ...
    def evaluate_tick(self, context: "SensorContext") -> "SensorExecutionData":
        """Evaluate sensor using the provided context.

        Args:
            context (SensorContext): The context with which to evaluate this sensor.
        Returns:
            SensorExecutionData: Contains list of run requests, or skip message if present.

        """

        check.inst_param(context, "context", SensorContext)
