...
from dagster.core.definitions.job import JobConfig, JobContext, JobDefinition, JobType
...
class SensorDefinition(JobDefinition):
    ...
    Args:
        ...
        job_config_fn (Callable[[SensorExecutionContext], JobConfig|List[JobConfig]]): A function
            that runs at an interval to determine whether a run should be launched or not. Takes a
            :py:class:`~dagster.SensorExecutionContext` and returns a JobConfig for each run that
            should be executed.
    ...
    def __init__(
        self, name, pipeline_name, job_config_fn, solid_selection=None, mode=None,
    ):
        ...
        self._job_config_fn = check.callable_param(job_config_fn, "job_config_fn")

    def get_job_config(self, context):
        check.inst_param(context, "context", SensorExecutionContext)
        result = self._job_config_fn(context)
        if isinstance(result, JobConfig):
            return [result]
        return check.is_list(result, of_type=JobConfig)
