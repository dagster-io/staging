...
def register_pipeline_hook(
    ...
    from dagster.core.definitions.job import PipelineHookRunSuccess
    ...
    def _pipeline_hook_dummy_sensor(context):
        ...
        for pipeline_run in runs:
            ...
            for event in events:
                ...
                run_key = f"{pipeline_run.run_id}:{pipeline_hook_def.dagster_event_type_value}"
                # TODO: may not need it
                if run_key == context.last_run_key:
                    continue
                ...
                with build_resources(
                    ...
                    try:
                        ...
                        # update state with failure status - this doesn't update run key, so when
                        # the hook execution error is fixed, we will backfill hook invocation
