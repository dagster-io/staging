...
class DecoratedSolidFunction(NamedTuple):
    ...
    decorator_takes_context: bool

    @lru_cache(maxsize=1)
    def has_context_arg(self) -> bool:
        return self.decorator_takes_context and is_context_provided(
            get_function_params(self.decorated_fn)
        )
...
class _Solid:
    ...
    def __call__(self, fn: Callable[..., Any]) -> SolidDefinition:
        ...
        compute_fn = DecoratedSolidFunction(
            decorated_fn=fn, decorator_takes_context=self.decorator_takes_context
        )
