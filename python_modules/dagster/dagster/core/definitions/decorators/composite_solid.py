...
def do_composition(
    ...
    actual_input_defs = (
        ...
        else infer_input_definitions_for_graph(decorator_name, name, fn)
    )

    actual_output_defs, explicit_outputs = (
        (provided_output_defs, True)
        if provided_output_defs is not None
        else (infer_output_definitions(decorator_name, name, fn), has_explicit_return_type(fn))
    )
