...
def get_output_manager_output_field(solid, output_def, resource_defs):
    # If using default manager key, mode definition does not need to provide resource def.
    # The system will infer to use the default output manager.
    if (
        output_def.manager_key == DEFAULT_MANAGER_KEY
        and output_def.manager_key not in resource_defs
    ):
        return None
