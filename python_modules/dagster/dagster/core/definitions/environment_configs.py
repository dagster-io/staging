...
def get_output_manager_output_field(solid, output_def, resource_defs):
    ...
    # TODO: remove the check for the default manager key once mem_asset_store is no longer default
    # behavior
    if output_def.manager_key != DEFAULT_MANAGER_KEY and not isinstance(
        resource_defs[output_def.manager_key], IOutputManagerDefinition
    ):
        raise DagsterInvalidDefinitionError(
            f'Output "{output_def.name}" for solid "{solid.name}" requires manager_key '
