...
class Partition(Generic[T]):
    ...
    def __init__(self, value: T, name: Optional[str] = None):
        self.value = value
        self.name = cast(str, check.opt_str_param(name, "name", str(value)))
...
class StaticPartitions(
    Partitions, NamedTuple("_StaticPartitions", [("partitions", List[Partition])])
):
    def __new__(cls, partitions: List[Partition]):
        return super(StaticPartitions, cls).__new__(
