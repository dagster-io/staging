...
from typing import Any, Callable, Dict, Generic, List, NamedTuple, Optional, TypeVar, cast
...
class Partitions(ABC, Generic[T]):
    @abstractmethod
    def get_partitions(self, current_time: Optional[datetime] = None) -> List[Partition[T]]:
        ...


class StaticPartitions(Partitions[T]):  # pylint: disable=unsubscriptable-object
...
class ScheduleTimeBasedPartitions(
    Partitions[datetime],  # pylint: disable=unsubscriptable-object
    NamedTuple(
        "_ScheduleTimeBasedPartitions",
        [
    ...
    def __new__(
        ...
        return super(ScheduleTimeBasedPartitions, cls).__new__(
            cls,
...
class DynamicPartitions(
    ...
    def __new__(cls, partition_fn: Callable[[Optional[datetime]], List[Partition]]):
        return super(DynamicPartitions, cls).__new__(
...
class PartitionSetDefinition(
    ...
    Args:
        ...
        partitions (Optional[Partitions]): A set of parameters used to construct the set
    ...
    def __new__(
        ...
        name: str,
        pipeline_name: str,
        partition_fn: Optional[Callable[[], List[Partition]]] = None,
        solid_selection: Optional[List[str]] = None,
        mode: Optional[str] = None,
        run_config_fn_for_partition: Callable[[Partition], Any] = lambda _partition: {},
        ...
        check.invariant(
            partition_fn is not None or partitions is not None,
            "One of `partition_fn` or `partitions` must be supplied.",
        )
        check.invariant(
            not (partition_fn and partitions),
            "Only one of `partition_fn` or `partitions` must be supplied.",
        )
        ...
        return super(PartitionSetDefinition, cls).__new__(
            ...
            partitions=check.opt_inst_param(
                ...
                "partitions",
                Partitions,
                default=DynamicPartitions(partition_fn=_wrap_partition_fn)
                if partition_fn is not None
                else None,
            ),
        )

    def run_config_for_partition(self, partition: Partition) -> Dict[str, Any]:
        return self.user_defined_run_config_fn_for_partition(partition)

    def tags_for_partition(self, partition: Partition) -> Dict[str, str]:
    ...
    def get_partitions(self, current_time: Optional[datetime] = None) -> List[Partition]:
        ...
        """
        return self.partitions.get_partitions(current_time)
...
class PartitionsConfig(Generic[T]):
    ...
    def __init__(
        self,
        partitions: Partitions[T],  # pylint: disable=unsubscriptable-object
    ...
    @property
    def partitions(self) -> Partitions[T]:  # pylint: disable=unsubscriptable-object
        return self._partitions
