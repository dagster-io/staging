...
def solid_invocation_result(
    ...
    **kwargs,
    ...
    context_arg_provided = is_context_provided(get_function_params(solid_def.compute_fn))
    ...
    result = (
        solid_def.compute_fn(context, **input_dict)
        if context_arg_provided
        else solid_def.compute_fn(**input_dict)
    )
...
def _type_check_output_wrapper(solid_def, result, context):
...
def _type_check_output(solid_def, output, context):
