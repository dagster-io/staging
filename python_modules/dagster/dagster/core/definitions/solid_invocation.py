    ...
    with user_code_error_boundary(
        ...
        try:
            ...
            raise DagsterInvariantViolationError(
                "Retry request denied. Retries are not supported when directly invoking solids."
            )
...
def _resolve_inputs(solid_def, args, kwargs):
    ...
    check.invariant(
        len(input_defs) >= len(args) + len(kwargs),
...
def _execute_and_retrieve_outputs(
    ...
    for output in compute_iterator:
        if not isinstance(output, AssetMaterialization):
            check.invariant(
                output.output_name not in outputs,
                f"Received output '{output.output_name}' more than once from solid '{solid_def.name}'.",
            )
            outputs[output.output_name] = output.value
        else:
            context.record_materialization(output)

    # Explicitly preserve the ordering of output defs
    return tuple([outputs[output_def.name] for output_def in solid_def.output_defs])
