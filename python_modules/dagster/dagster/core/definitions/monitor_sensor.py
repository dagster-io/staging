...
def pipeline_failure_monitor(
    ...
    def inner(
        fn: Callable[["MonitorSensorContext"], Union[SkipReason, MonitorRequest]]
    ) -> SensorDefinition:
        ...
        def _wrapped_fn(context: SensorExecutionContext):
            ...
            if context.cursor is None:
                context.update_cursor(pendulum.now("UTC").isoformat())
                yield SkipReason(
                    f"Initiating {sensor_name}. Set cursor to {datetime.fromisoformat(context.cursor)}"
                )
                return
            ...
            runs = context.instance.get_runs_by_timestamp(
                ...
                update_after=datetime.fromisoformat(context.cursor) if context.cursor else None,
                limit=5,
            )
