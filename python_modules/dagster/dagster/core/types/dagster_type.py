...
class PythonUnionDagsterType(DagsterType):
    '''Define a type in dagster whose typecheck is an isinstance check over a union defined as a tuple of types.
    ...
    def __init__(self, python_types, key=None, name=None, description=None, **kwargs):
        self.python_types = check.tuple_param(python_types, 'python_types', of_type=check.type_types)
        ...
        key = check.opt_str_param(key, 'key', name)
        description = check.opt_str_param(description, 'description', name)

        super(PythonUnionDagsterType, self).__init__(
            key=key,
            name=name,
            description=description,
            type_check_fn=self.type_check_method,
    ...
    def type_check_method(self, _context, value):
        if not isinstance(value, self.python_types):
            return TypeCheck(
                ...
                description=(
                    'Value of type {value_type} failed type check for Dagster type {dagster_type}, '
                    'expected value to be within Python type {expected_type}.'
                ).format(
