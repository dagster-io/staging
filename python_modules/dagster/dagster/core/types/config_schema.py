...
class DagsterTypeLoader(object):
    ...
    def compute_loaded_input_version(self, _context, _config_value):
        return None
...
class DagsterTypeLoaderFromDecorator(DagsterTypeLoader):
    ...
    def compute_loaded_input_version(self, context, config_value):
        version = ''
        if self._loader_version:
            version += str(self._loader_version)
        ext_version = self._external_version_fn(context, config_value)
        if ext_version:
...
def dagster_type_loader(
    ...
    Args:
        ...
        external_version_fn (Callable): A function that takes in the same parameters as the loader
            function (context, value) and returns a representation of the version of the external
