...
class DagsterInstance:
    ...
    def report_run_cancelling(self, run, message="Sending pipeline termination request."):

        from dagster.core.events import DagsterEvent, DagsterEventType
        from dagster.core.events.log import DagsterEventRecord

        cancelling_event = DagsterEvent(
            event_type_value=DagsterEventType.PIPELINE_CANCELLING.value,
        ...
        event_record = DagsterEventRecord(
            ...
            error_info=None,
            timestamp=time.time(),
            dagster_event=cancelling_event,
        )

        self.handle_new_event(event_record)

    def report_run_cancelled(self, pipeline_run):
        from dagster.core.events import DagsterEvent, DagsterEventType
        from dagster.core.events.log import DagsterEventRecord

        check.inst_param(pipeline_run, "pipeline_run", PipelineRun)
        message = (
            "This pipeline run has been marked as cancelled from outside the execution context."
        )

        dagster_event = DagsterEvent(
            event_type_value=DagsterEventType.PIPELINE_CANCELLED.value,
            pipeline_name=pipeline_run.pipeline_name,
            message=message,
        )
        event_record = DagsterEventRecord(
