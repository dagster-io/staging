...
class DagsterInstance:
    ...
    def enqueue_run(
        ...
        new_run = self._construct_run_with_snapshots(
            pipeline_name=pipeline_name,
            run_id=run_id,
            run_config=run_config,
            mode=mode,
        ...
        try:
            enqueued_run = self._run_queuer.enqueue_run(self, new_run, external_pipeline)
        except:
            from dagster.core.events import EngineEventData

            error = serializable_error_info_from_exc_info(sys.exc_info())
            self.report_engine_event(
                error.message, new_run, EngineEventData.engine_error(error),
            )
            self.report_run_failed(new_run)
            raise
