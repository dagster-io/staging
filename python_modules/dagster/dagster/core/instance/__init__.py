...
class DagsterInstance:
    ...
    def enqueue_run(
        ...
        try:
            enqueued_run = self._run_queuer.enqueue_run(self, new_run)
        except:
            from dagster.core.events import EngineEventData

            error = serializable_error_info_from_exc_info(sys.exc_info())
            self.report_engine_event(
                error.message, enqueued_run, EngineEventData.engine_error(error),
            )
            self.report_run_failed(enqueued_run)
            raise
