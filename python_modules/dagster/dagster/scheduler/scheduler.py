...
def launch_scheduled_runs(
    ...
    logger.info(
        "Checking for new runs at {end_datetime_utc} for the following schedules: {schedule_names}".format(
            end_datetime_utc=end_datetime_utc.isoformat(),
            schedule_names=repr([schedule.name for schedule in schedules]),
        )
    )
    ...
    if len(tick_times) == 1:
        logger.info(
            "Launching run for {schedule_name} at {time}".format(
                schedule_name=schedule_state.name, time=tick_times[0].isoformat()
            )
        )
    else:
        logger.info(
            "Launching {num_runs} runs for {schedule_name} at the following times:{times}".format(
                ...
                times=[time.isoformat() for time in tick_times],
            )
        )
