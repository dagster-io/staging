...
def execute_sensor_iteration_loop(instance, logger, daemon_shutdown_event, until=None):
    ...
    with ExitStack() as stack:
        while (not daemon_shutdown_event or not daemon_shutdown_event.is_set()) and (
            not until or start_time < until
        ):
            if (
                not handle_manager
            ...
            logger.info("Restarting sensor evaluation loop.", sleep_time)
            start_time = pendulum.now("UTC").timestamp()
