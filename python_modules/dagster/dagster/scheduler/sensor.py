...
def launch_runs_for_sensor(
    ...
    assert isinstance(sensor_runtime_data, SensorTickData)
    ...
    for job_config in sensor_runtime_data.run_params:
        execution_plan_errors = []
        execution_plan_snapshot = None
        if job_config.execution_key and instance.has_job_tick(
            external_sensor.get_external_origin_id(),
            job_config.execution_key,
            [JobTickStatus.SUCCESS],
        ):
            context.logger.info(
                "Found existing run for sensor {sensor_name} with execution_key `{execution_key}`, skipping.".format(
                    sensor_name=external_sensor.name, execution_key=job_config.execution_key
                )
            )
            context.add_state(JobTickStatus.SKIPPED, execution_key=job_config.execution_key)
            continue
        ...
        context.add_state(
            JobTickStatus.SUCCESS, run_id=run.run_id, execution_key=job_config.execution_key
        )
