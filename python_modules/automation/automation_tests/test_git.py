...
def test_set_git_tag():
    ...
    with pytest.raises(Exception):
        git.set_git_tag("0.1.0", signed=True, dry_run=False)


def test_commit_updates(tmpdir):
    assert "Initial commit" in subprocess.check_output(["git", "log", "--pretty=oneline"]).decode(
        "utf-8"
    )
    assert "New commit" not in subprocess.check_output(["git", "log", "--pretty=oneline"]).decode(
        "utf-8"
    )
    ...
    assert "New commit" in subprocess.check_output(["git", "log", "--pretty=oneline"]).decode(
        "utf-8"
    )
