...
@contextmanager
def get_test_project_workspace(container_image=None):
    with Workspace(TestProjectLoadTarget(container_image)) as workspace:
        yield workspace


@contextmanager
def get_test_project_external_pipeline_hierarchy(pipeline_name, container_image=None):
    with get_test_project_workspace(container_image) as workspace:
...
def get_test_project_external_repo(container_image=None):
    with get_test_project_workspace() as workspace:
        location = workspace.get_repository_location(workspace.repository_location_names[0])
...
def get_test_project_external_pipeline(pipeline_name, container_image=None):
    with get_test_project_external_pipeline_hierarchy(
        pipeline_name, container_image
    ) as workspace, location, repo, pipeline:
        yield pipeline


@contextmanager
def get_test_project_workspace_and_external_pipeline(pipeline_name, container_image=None):
    with get_test_project_external_pipeline_hierarchy(
        pipeline_name, container_image
    ) as workspace, location, repo, pipeline:
