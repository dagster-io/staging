...
def overwrite_partition(df: DataFrame, path: str, partition_name: str) -> None:
    pass


def overwrite_partitions(
    df: DataFrame, path: str, partition_name: str, num_partitions: int
) -> None:
    pass


def read_partition(path: str, partition_name: str) -> DataFrame:
    pass


def read_partitions(path: str, latest_partition_name: str, num_partitions: int) -> DataFrame:
    pass
...
def impure_drop_recreate():
    ...
    @solid(input_defs=[InputDefinition(dagster_type=Nothing)])
    def solid2(_):
        with db_connection() as con:
            con.execute(
                """
...
def impure_single_partition():
    ...
    @solid(input_defs=[InputDefinition(dagster_type=Nothing)], config_schema={"partition": str})
    def solid2(context):
        with db_connection() as con:
            con.execute(
                f"""
