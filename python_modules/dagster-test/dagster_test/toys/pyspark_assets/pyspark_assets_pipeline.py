...
@solid(
    ...
    required_resource_keys={"rootdir", "savedir"},
)
def get_max_temp_per_station(context):
    fpath = os.path.join(context.resources.rootdir, context.solid_config["temperature_file"])
...
def get_consolidated_location(context):
    fpath = os.path.join(context.resources.rootdir, context.solid_config["station_file"])
...
@pipeline(mode_defs=[ModeDefinition(resource_defs={"rootdir": rootdir, "savedir": savedir})])
...
if __name__ == "__main__":
    run_config = {
        ...
        "resources": {
            "rootdir": {"config": {"dir": "asset_pipeline_files"}},
