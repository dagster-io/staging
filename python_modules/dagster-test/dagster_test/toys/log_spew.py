...
from dagster import In, MultiOut, Out, Output, graph, op


def nonce_op(name, n_inputs, n_outputs):
    """Creates a op with the given number of (meaningless) inputs and outputs.
    ...
    @op(
        ...
        out=MultiOut(outs=[Out(name=f"output_{i}") for i in range(n_outputs)]),
    )
...
@graph(description="Demo graph that spits out different types of log messages to the event log.")
def log_spew_graph():
