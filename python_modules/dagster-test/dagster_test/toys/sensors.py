...
def get_toys_sensors():
    ...
    from dagster_aws.s3.sensor import get_s3_updates, get_new_s3_keys

    @sensor(pipeline_name="log_s3_pipeline")
    def s3_bucket_update_sensor(context):
        if not bucket:
            raise Exception(
                "S3 bucket not specified at environment variable `DAGSTER_TOY_SENSOR_S3_BUCKET`."
            )

        updated_s3_keys = get_s3_updates(
            bucket,
            since=utc_datetime_from_timestamp(context.last_completion_time)
            if context.last_completion_time
            else None,
        )
        if not updated_s3_keys:
            yield SkipReason(f"No s3 updates found for bucket {bucket}.")
            return

        for s3_key in updated_s3_keys:
            yield RunRequest(
                run_key=s3_key,
                run_config={
    ...
    @sensor(pipeline_name="log_s3_pipeline")
    def s3_bucket_new_sensor(context):
        if not bucket:
            raise Exception(
                "S3 bucket not specified at environment variable `DAGSTER_TOY_SENSOR_S3_BUCKET`."
            )

        new_s3_keys = get_new_s3_keys(bucket, last_key=context.last_run_key)
        if not new_s3_keys:
            yield SkipReason(f"No s3 updates found for bucket {bucket}.")
            return
    ...
    return [toy_file_sensor, s3_bucket_update_sensor, s3_bucket_new_sensor]
