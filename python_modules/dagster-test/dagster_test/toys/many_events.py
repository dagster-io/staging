...
def create_raw_file_solid(name):
    ...
    def raw_file_solid(_context):
        yield AssetMaterialization(
            ...
            metadata={"table_path": EventMetadataEntry.path_data("/path/to/{}.raw".format(name))},
        )
    ...
    with open(file_relative_path(__file__, MARKDOWN_EXAMPLE), "r") as f:
        ...
        for table in raw_tables:
            yield AssetMaterialization(
                ...
                metadata={
                    "table_name": table,
                    "table_path": EventMetadataEntry.path_data(f"/path/to/{table}"),
                    "table_data": {"name": table},
                    "table_name_big": EventMetadataEntry.url_data(f"https://bigty.pe/{table}"),
                    "table_blurb": EventMetadataEntry.md_data(md_str),
...
def many_materializations_and_passing_expectations(_context):
    ...
    for table in tables:
        yield AssetMaterialization(
            asset_key="table_info",
            metadata={
                "table_path": EventMetadataEntry.path_data(f"/path/to/{table}.raw"),
            },
        )
